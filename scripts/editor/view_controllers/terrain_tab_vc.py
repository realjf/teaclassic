...
class TerrainTabVC(vc.ViewController):
    ...
    def __update_objects_for_height_change(self):
        for obj in globals.active_objects_list:
            obj.pos = (obj.pos[0], tc.map_height_at_point(
                obj.pos[0], obj.pos[2]), obj.pos[2])
    ...
    def __paint_selection(self):
        ...
        if tc.mouse_over_minimap():
            return

        global_r = self.selected_tile[0][0] * \
            tc.TILES_PER_CHUNK_HEIGHT + self.selected_tile[1][0]
        global_c = self.selected_tile[0][1] * \
            tc.TILES_PER_CHUNK_WIDTH + self.selected_tile[1][1]
        ...
        for r in range(-(self.view.brush_size_idx), (self.view.brush_size_idx) + 1):
            for c in range(-(self.view.brush_size_idx), (self.view.brush_size_idx) + 1):
                ...
                if self.view.blend_textures:
                    bm = tc.BLEND_MODE_BLUR
                else:
                    bm = tc.BLEND_MODE_NOBLEND
                ...
                if tile_coords is not None:
                    ...
                    if self.view.brush_type_idx == Brush.TEXTURE:
                        ...
                        globals.active_map.update_tile(
                            tile_coords, center_height, tc.TILETYPE_FLAT, side_mat, 0, bm, self.view.blend_normals)
                    elif self.view.brush_type_idx == Brush.SHALLOW_WAT:
                        globals.active_map.update_tile(
                            tile_coords, SHALLOW_WAT_ELEV, tc.TILETYPE_FLAT, side_mat, 0, bm, self.view.blend_normals)
                    elif self.view.brush_type_idx == Brush.DEEP_WAT:
                        globals.active_map.update_tile(
                            tile_coords, DEEP_WAT_ELEV, tc.TILETYPE_FLAT, side_mat, 0, bm, self.view.blend_normals)
    ...
    def __smoothed_tile(self, tile_coords, dir):
        """
        ...
        global_r = tile_coords[0][0] * \
            tc.TILES_PER_CHUNK_HEIGHT + tile_coords[1][0]
        global_c = tile_coords[0][1] * \
            tc.TILES_PER_CHUNK_WIDTH + tile_coords[1][1]

        tile_for_case = [
            tc.TILETYPE_FLAT,
            tc.TILETYPE_CORNER_CONCAVE_NE,
            tc.TILETYPE_CORNER_CONCAVE_NW,
            tc.TILETYPE_RAMP_NS,
            tc.TILETYPE_CORNER_CONCAVE_SW,
            tc.TILETYPE_FLAT,  # ambiguous case
            tc.TILETYPE_RAMP_WE,
            tc.TILETYPE_CORNER_CONVEX_NW,
            tc.TILETYPE_CORNER_CONCAVE_SE,
            tc.TILETYPE_RAMP_EW,
            tc.TILETYPE_FLAT,  # ambiguous case
            tc.TILETYPE_CORNER_CONVEX_NE,
            tc.TILETYPE_RAMP_SN,
            tc.TILETYPE_CORNER_CONVEX_SE,
            tc.TILETYPE_CORNER_CONVEX_SW,
            tc.TILETYPE_FLAT
        ]
        ...
        nw_height = func([h for h in [
            getattr(globals.active_map.relative_tile(
                global_r, global_c,  0, -1), "top_right_height", None),
            getattr(globals.active_map.relative_tile(
                global_r, global_c, -1, -1), "bot_right_height", None),
            getattr(globals.active_map.relative_tile(
                global_r, global_c, -1,  0), "bot_left_height", None),
            getattr(tile, "top_left_height", None)
        ] if h is not None])

        ne_height = func([h for h in [
            getattr(globals.active_map.relative_tile(
                global_r, global_c, -1,  0), "bot_right_height", None),
            getattr(globals.active_map.relative_tile(
                global_r, global_c, -1,  1), "bot_left_height", None),
            getattr(globals.active_map.relative_tile(
                global_r, global_c,  0,  1), "top_left_height", None),
            getattr(tile, "top_right_height", None)
        ] if h is not None])

        se_height = func([h for h in [
            getattr(globals.active_map.relative_tile(
                global_r, global_c,  0,  1), "bot_left_height", None),
            getattr(globals.active_map.relative_tile(
                global_r, global_c,  1,  1), "top_left_height", None),
            getattr(globals.active_map.relative_tile(
                global_r, global_c,  1,  0), "top_right_height", None),
            getattr(tile, "bot_right_height", None)
        ] if h is not None])

        sw_height = func([h for h in [
            getattr(globals.active_map.relative_tile(
                global_r, global_c,  1,  0), "top_left_height", None),
            getattr(globals.active_map.relative_tile(
                global_r, global_c,  1, -1), "top_right_height", None),
            getattr(globals.active_map.relative_tile(
                global_r, global_c,  0, -1), "bot_right_height", None),
            getattr(tile, "bot_left_height", None)
        ] if h is not None])

        og_heights = [nw_height, ne_height, se_height, sw_height]
        heights = [h if h == max(og_heights) else min(og_heights)
                   for h in og_heights]
        ...
        for r in range(-radius, radius + 1):
            for c in range(-radius, radius + 1):
                ...
                if left_edge and not (top_edge or bot_edge) \
                    ...
                    results.append((tile_coords) +
                                   self.__smoothed_tile(tile_coords, dir))

        for r in results:
            globals.active_map.update_tile((r[0], r[1]), *r[2:])

        corner_tiles_coords = [
            globals.active_map.relative_tile_coords(
                global_r, global_c, -radius, -radius),
            globals.active_map.relative_tile_coords(
                global_r, global_c, -radius,  radius),
            globals.active_map.relative_tile_coords(
                global_r, global_c,  radius,  radius),
    ...
    def __on_brush_size_changed(self, event):
        tc.set_map_highlight_size(self.view.brush_size_idx + 1)

    def activate(self):
        tc.set_map_highlight_size(self.view.brush_size_idx + 1)
        tc.register_event_handler(
            tc.SDL_MOUSEBUTTONDOWN, TerrainTabVC.__on_mouse_pressed, self)
        tc.register_event_handler(
            tc.SDL_MOUSEBUTTONUP, TerrainTabVC.__on_mouse_released, self)
        tc.register_event_handler(
            tc.EVENT_SELECTED_TILE_CHANGED, TerrainTabVC.__on_selected_tile_changed, self)
        tc.register_event_handler(
            EVENT_TERRAIN_BRUSH_SIZE_CHANGED, TerrainTabVC.__on_brush_size_changed, self)

    def deactivate(self):
        tc.set_map_highlight_size(0)
        tc.unregister_event_handler(
            tc.SDL_MOUSEBUTTONDOWN, TerrainTabVC.__on_mouse_pressed)
        tc.unregister_event_handler(
            tc.SDL_MOUSEBUTTONUP, TerrainTabVC.__on_mouse_released)
        tc.unregister_event_handler(
            tc.EVENT_SELECTED_TILE_CHANGED, TerrainTabVC.__on_selected_tile_changed)
        tc.unregister_event_handler(
            EVENT_TERRAIN_BRUSH_SIZE_CHANGED, TerrainTabVC.__on_brush_size_changed)
