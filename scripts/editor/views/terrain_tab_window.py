...
class TerrainTabWindow(tc.Window):

    def __init__(self):
        vresx, vresy = (1920, 1080)
        super(TerrainTabWindow, self).__init__("TerrainTab",
                                               (0, UI_TAB_BAR_HEIGHT + 1, UI_LEFT_PANE_WIDTH, vresy -
                                                UI_TAB_BAR_HEIGHT - 1), tc.NK_WINDOW_BORDER, (vresx, vresy),
                                               resize_mask=tc.ANCHOR_X_LEFT | tc.ANCHOR_Y_TOP | tc.ANCHOR_Y_BOT)
    ...
    def update(self):
        ...
        self.layout_row_dynamic(20, 1)
        self.blend_textures = self.checkbox(
            "Blend Textures", self.blend_textures)
        if old_blend_textures != self.blend_textures:
            tc.global_event(EVENT_TERRAIN_TEX_BLEND_CHANGED,
                            self.blend_textures)
        ...
        if old_blend_normals != self.blend_normals:
            tc.global_event(EVENT_TERRAIN_NORMAL_BLEND_CHANGED,
                            self.blend_normals)
        ...
        if self.brush_size_idx != old_brush_size_idx:
            tc.global_event(EVENT_TERRAIN_BRUSH_SIZE_CHANGED,
                            self.brush_size_idx)
        ...
        if self.brush_type_idx != old_brush_type_idx:
            tc.global_event(EVENT_TERRAIN_BRUSH_TYPE_CHANGED,
                            self.brush_type_idx)
        ...
        if self.brush_type_idx == Brush.TEXTURE:
            ...
            def textures_group():
                ...
                for i in range(0, len(globals.active_map.materials)):
                    ...
                    on = self.selectable_label(globals.active_map.materials[i].name,
                                               tc.NK_TEXT_ALIGN_LEFT, i == self.selected_mat_idx)
                    if on:
                        self.selected_mat_idx = i
                    if self.selected_mat_idx != old:
                        tc.global_event(EVENT_TEXTURE_SELECTION_CHANGED, i)

            self.layout_row_static(400, UI_LEFT_PANE_WIDTH-30, 1)
            self.group("Texture:", tc.NK_WINDOW_BORDER, textures_group)
        else:
            # Elevation
            self.layout_row_dynamic(20, 1)
            ...
            if old_edges_type_idx != self.edges_type_idx:
                tc.global_event(EVENT_TERRAIN_EDGE_TYPE_CHANGED,
                                self.edges_type_idx)
            ...
            if self.brush_type_idx == Brush.ELEVATION:
                ...
                self.selected_height_idx = self.combo_box(
                    [str(h) for h in self.heights], self.selected_height_idx, 25, (UI_LEFT_PANE_WIDTH - 30, 200))
                if old_height_idx != self.selected_height_idx:
                    tc.global_event(EVENT_HEIGHT_SELECTION_CHANGED,
            ...
            self.selected_side_mat_idx = self.combo_box(
                [mat.name for mat in globals.active_map.materials], self.selected_side_mat_idx, 25, (UI_LEFT_PANE_WIDTH - 30, 200))
            if old_side_mat_idx != self.selected_side_mat_idx:
                tc.global_event(EVENT_SIDE_MAT_SEL_CHANGED,
                                globals.active_map.materials[self.selected_side_mat_idx])
