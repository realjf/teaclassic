...
class TabBarWindow(tc.Window):
    ...
    def __init__(self):
        ...
        super(TabBarWindow, self).__init__("TabBar", dims, tc.NK_WINDOW_NO_SCROLLBAR, (vresx, vresy),
                                           resize_mask=tc.ANCHOR_X_LEFT | tc.ANCHOR_X_RIGHT | tc.ANCHOR_Y_TOP)
        self.active_idx = 0
        self.labels = []
        self.child_windows = []
    ...
    def update(self):
        ...
        with btc.ButtonStyle(rounding=0.0):
            self.layout_row_begin(
                tc.NK_STATIC, UI_TAB_BAR_HEIGHT-10, UI_TAB_BAR_NUM_COLS)
            ...
            for idx in range(0, len(self.child_windows)):
                ...
                def on_tab_click():
                    ...
                    self.__show_active()
                    tc.global_event(
                        EVENT_TOP_TAB_SELECTION_CHANGED, self.active_idx)
