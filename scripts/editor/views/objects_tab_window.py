...
class ObjectsTabWindow(tc.Window):
    ...
    def update(self):
        ...
        if self.mode != old_mode:
            tc.global_event(EVENT_OBJECTS_TAB_MODE_CHANGED, self.mode)
        ...
        if self.mode == self.OBJECTS_MODE_PLACE:
            ...
            fac_names = [fac["name"] for fac in tc.get_factions_list()]
            self.selected_faction_idx = self.combo_box(
                fac_names, self.selected_faction_idx, 25, (UI_LEFT_PANE_WIDTH-30, 200))
            self.layout_row_dynamic(10, 1)
            ...
            def objects_group():
                ...
                for i in range(0, len(self.objects_list)):
                    ...
                    on = self.selectable_label(self.objects_list[i],
                                               tc.NK_TEXT_ALIGN_LEFT, i == self.selected_object_idx)
                    if on:
                        self.selected_object_idx = i
                    if self.selected_object_idx != old:
                        tc.global_event(EVENT_OBJECT_SELECTION_CHANGED, i)

            self.layout_row_static(400, UI_LEFT_PANE_WIDTH-30, 1)
            self.group("Objects", tc.NK_WINDOW_BORDER, objects_group)
            ...
            if len(sel_obj_list) > 1:
                def selection_group():
                    ...
                    for i in range(0, len(sel_obj_list)):
                        name = "{0} {1}".format(
                            sel_obj_list[i].name, format_str_for_numlist(sel_obj_list[i].pos))
                        on = self.selectable_label(
                            name, tc.NK_TEXT_ALIGN_LEFT, False)
                        if on:
                            tc.global_event(
                                EVENT_OBJECT_SELECTED_UNIT_PICKED, sel_obj_list[i])
                self.layout_row_static(400, UI_LEFT_PANE_WIDTH-30, 1)
                self.group("Selection", tc.NK_WINDOW_BORDER, selection_group)
            else:
                ...
                fac = next(f for f in tc.get_factions_list()
            ...
            def on_delete():
                tc.global_event(EVENT_OBJECT_DELETE_SELECTION, None)
