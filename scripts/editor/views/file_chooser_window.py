...
class FileChooser(tc.Window):
    ...
    def __init__(self, title):
        ...
        super(FileChooser, self).__init__(title,
                                          (vresx / 2 - FileChooser.WINDOW_WIDTH / 2, vresy / 2 - FileChooser.WINDOW_HEIGHT / 2,
                                           FileChooser.WINDOW_WIDTH, FileChooser.WINDOW_HEIGHT), tc.NK_WINDOW_BORDER | tc.NK_WINDOW_NO_SCROLLBAR | tc.NK_WINDOW_TITLE, (vresx, vresy))
    ...
    def update(self):
        ...
        self.mapstring = self.edit_string(tc.NK_EDIT_SIMPLE, self.mapstring)

        def on_okay():
            scenepath = self.scenestring if self.scene_flag else None
            tc.global_event(EVENT_FILE_CHOOSER_OKAY,
                            (tc.get_basedir() + "/" + self.mapstring, scenepath))

        def on_cancel():
            tc.global_event(EVENT_FILE_CHOOSER_CANCEL, None)

        self.layout_row_dynamic(30, 1)
        self.scene_flag = self.checkbox(
            self.title + " Scene:", self.scene_flag)
        self.layout_row_dynamic(30, 1)
        if self.scene_flag:
            self.scenestring = self.edit_string(
                tc.NK_EDIT_SIMPLE, self.scenestring)
        self.layout_row_static(15, FileChooser.WINDOW_WIDTH, 1)
