...
if len(sys.argv) > 1:
    tc.load_map(tc.get_basedir(), sys.argv[1], update_navgrid=False)
    globals.active_map = map.Map.from_filepath(
        tc.get_basedir() + "/" + sys.argv[1])
else:
    tc.load_map_string(globals.active_map.tcmap_str(), update_navgrid=False)

if len(sys.argv) > 2:
    globals.active_objects_list, = tc.load_scene(
        sys.argv[2], update_navgrid=False)
    globals.scene_filename = sys.argv[2]
    for obj in globals.active_objects_list:
        obj.selectable = True
else:
    tc.add_faction(DEFAULT_FACTION_NAME, DEFAULT_FACTION_COLOR)

############################################################
# Setup UI                                                 #
############################################################

minimap_pos = tc.get_minimap_position()
tc.set_minimap_position(UI_LEFT_PANE_WIDTH + minimap_pos[0], minimap_pos[1])
