...
def tile_to_string(tile):
    ...
    assert tile.blend_mode in [tc.BLEND_MODE_BLUR, tc.BLEND_MODE_NOBLEND]
...
class Chunk(object):
    ...
    def __init__(self):
        ...
        """
        self.rows = tc.TILES_PER_CHUNK_HEIGHT
        self.cols = tc.TILES_PER_CHUNK_WIDTH

        self.tiles = []
        for r in range(0, self.rows):
            row = [tc.Tile() for c in range(0, self.cols)]
    ...
    def from_lines(cls, lines):
        ...
        rows_read = 0
        while rows_read < tc.TILES_PER_CHUNK_HEIGHT:
            tiles_read = 0
            tiles_row = []
            while tiles_read < tc.TILES_PER_CHUNK_WIDTH:
...
class Map(object):
    ...
    def tcmap_str(self):
        ...
        for chunk_r in range(0, self.chunk_rows):
            for chunk_c in range(0, self.chunk_cols):
                ret += self.chunks[chunk_r][chunk_c].tcmap_str()
        return ret

    def write_to_file(self):
        if self.filename is not None:
            with open(self.filename, "w") as mapfile:
                mapfile.write(self.tcmap_str())
    ...
    def update_tile_mat(self, tile_coords, top_material, blend_mode, blend_normals):
        ...
        tc.update_tile(tile_coords[0], tile_coords[1], tile)
    ...
    def relative_tile_coords(self, global_r, global_c, dr, dc):
        ...
        if global_r + dr < 0 or global_r + dr >= self.chunk_rows * tc.TILES_PER_CHUNK_HEIGHT:
            return None

        if global_c + dc < 0 or global_c + dc >= self.chunk_cols * tc.TILES_PER_CHUNK_WIDTH:
            return None

        chunk_r = (global_r + dr) // tc.TILES_PER_CHUNK_HEIGHT
        chunk_c = (global_c + dc) // tc.TILES_PER_CHUNK_WIDTH
        tile_r = (global_r + dr) % tc.TILES_PER_CHUNK_HEIGHT
        tile_c = (global_c + dc) % tc.TILES_PER_CHUNK_WIDTH
