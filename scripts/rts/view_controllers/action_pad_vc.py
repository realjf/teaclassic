...
class ActionPadVC(vc.ViewController):
    ...
    def __on_selection_changed(self, event):
        ...
        controllable_sel = [
            ent for ent in sel if isinstance(ent, cont.Controllable)]

        if len(controllable_sel) > 0:
            first = controllable_sel[0]
            fac_list = tc.get_factions_list()
            if fac_list[first.faction_id]["controllable"]:
                self.__active_controllable = weakref.ref(first)
                self.__install_hotkeys(first)
                self.__view.actions = [first.action(i) for i in range(
                    0, ACTION_NUM_ROWS * ACTION_NUM_COLS)]

    def __on_keydown(self, event):
        scancode = event[0]
        if scancode in self.__hotkey_action_map and not tc.ui_text_edit_has_focus():
    ...
    def activate(self):
        ...
        tc.register_event_handler(
            tc.EVENT_UNIT_SELECTION_CHANGED, ActionPadVC.__on_selection_changed, self)
        tc.register_event_handler(
            tc.SDL_KEYDOWN, ActionPadVC.__on_keydown, self)
        self.__view.show()

    def deactivate(self):
        self.__view.hide()
        tc.unregister_event_handler(tc.SDL_KEYDOWN, ActionPadVC.__on_keydown)
        tc.unregister_event_handler(
            tc.EVENT_UNIT_SELECTION_CHANGED, ActionPadVC.__on_selection_changed)
