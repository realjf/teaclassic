...
class AnimCombatable(tc.AnimEntity, tc.CombatableEntity, cont.Controllable):
    """
    ...
    def __init__(self, path, tcobj, name, **kwargs):
        super(AnimCombatable, self).__init__(path, tcobj, name, **kwargs)
        self.attacking = False
        self.register(tc.EVENT_ATTACK_START,
                      AnimCombatable.on_attack_begin, weakref.ref(self))
        self.register(tc.EVENT_ATTACK_END,
                      AnimCombatable.on_attack_end, weakref.ref(self))
        self.register(tc.EVENT_ENTITY_DEATH,
                      AnimCombatable.on_death, weakref.ref(self))

    def __del__(self):
        self.unregister(tc.EVENT_ENTITY_DEATH, AnimCombatable.on_death)
        self.unregister(tc.EVENT_ATTACK_END, AnimCombatable.on_attack_end)
        self.unregister(tc.EVENT_ATTACK_START, AnimCombatable.on_attack_begin)
    ...
    def action(self, idx):
        ...
        if idx == 3:
            return action.ActionDesc(
                ...
                hotkey=tc.SDL_SCANCODE_A)
        return super(AnimCombatable, self).action(idx)

    @classmethod
    def __attack_action(cls):
        tc.set_attack_on_left_click()

    @classmethod
    def __hold_position_action(cls):
        for ent in tc.get_unit_selection():
            if isinstance(ent, tc.CombatableEntity):
                ent.hold_position()
