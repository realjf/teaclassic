...
class ActionPadWindow(tc.Window):
    ...
    def __init__(self):
        ...
        width = ActionPadWindow.BUTTON_WIDTH * ACTION_NUM_COLS \
            + (ACTION_NUM_COLS-1) * ActionPadWindow.BUTTON_PADDING + \
            (ACTION_NUM_COLS*2) + 4
        height = ActionPadWindow.BUTTON_WIDTH * ACTION_NUM_ROWS \
            + (ACTION_NUM_ROWS-1) * ActionPadWindow.BUTTON_PADDING + \
            (ACTION_NUM_ROWS*2) + 6
        vresx, vresy = (1920, 1080)
        super(ActionPadWindow, self).__init__("ActionPad", (vresx - width - 10, vresy - height - 10,
                                                            width, height), tc.NK_WINDOW_BORDER | tc.NK_WINDOW_NO_SCROLLBAR, (vresx, vresy),
                                              resize_mask=tc.ANCHOR_X_RIGHT | tc.ANCHOR_Y_BOT)
        self.spacing = (float(ActionPadWindow.BUTTON_PADDING),
                        float(ActionPadWindow.BUTTON_PADDING))
    ...
    def update(self):
        with btc.ButtonStyle(padding=(0.0, 0.0), rounding=0.0):
            ...
            def on_button_pressed(idx):
                tc.global_event(EVENT_UNIT_ACTION, idx)

            for r in range(0, ACTION_NUM_ROWS):
                self.layout_row_static(
                    ActionPadWindow.BUTTON_WIDTH, ActionPadWindow.BUTTON_WIDTH, ACTION_NUM_COLS)
