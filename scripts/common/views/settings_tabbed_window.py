...
class SettingsTabbedWindow(tc.Window):
    ...
    def __init__(self):
        ...
        super(SettingsTabbedWindow, self).__init__("Settings", dims,
                                                   tc.NK_WINDOW_NO_SCROLLBAR | tc.NK_WINDOW_MOVABLE | tc.NK_WINDOW_BORDER | tc.NK_WINDOW_TITLE | tc.NK_WINDOW_CLOSABLE, (vresx, vresy))
    ...
    def push_child(self, label, window):
        ...
        assert isinstance(window, tc.Window)

        self.labels.append(label)
        self.child_windows.append(window)

    def on_hide(self, manual):
        tc.global_event(EVENT_SETTINGS_HIDE, None)
    ...
    def update(self):
            ...
            with btc.ButtonStyle(rounding=0.0):
                ...
                for idx in range(0, len(self.child_windows)):
                    ...
                    def on_tab_click():
                        self.active_idx = idx
                        tc.global_event(
                            EVENT_SETTINGS_TAB_SEL_CHANGED, self.active_idx)
        ...
        BOT_PAD = 10
        self.layout_row_begin(tc.NK_STATIC, SettingsTabbedWindow.HEIGHT - self.header_height -
                              int(50 + 2 * self.group_padding[1]) - int(2 * self.spacing[1]) - BOT_PAD, 2)

        self.layout_row_push(SettingsTabbedWindow.TAB_COL_WIDTH)
        self.group("SettingsTabCol", tc.NK_WINDOW_NO_SCROLLBAR,
                   settings_tab_group)

        self.layout_row_push(SettingsTabbedWindow.WIDTH -
                             SettingsTabbedWindow.TAB_COL_WIDTH - int(2 * self.border + self.spacing[0]))
        self.group("SettingsSubWindow", tc.NK_WINDOW_NO_SCROLLBAR,
                   self.child_windows[self.active_idx].update)
        self.layout_row_end()
        ...
        def on_apply():
            tc.global_event(EVENT_SETTINGS_APPLY, None)
        ...
        def apply_done_group():
            ...
            if self.child_windows[self.active_idx].dirty:
                self.label_colored_wrap(
                    "You have unsaved changes on this page.", (255, 0, 0))
            else:
        ...
        self.group("SettingsApplyDone",
                   tc.NK_WINDOW_NO_SCROLLBAR, apply_done_group)
