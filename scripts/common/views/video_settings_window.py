...
class VideoSettingsWindow(tc.Window):
    ...
    def __init__(self):
        ...
        self.res_opt_strings = ["{}:{}".format(
            int(opt[0]), int(opt[1])) for opt in self.res_opts]
    ...
    def update(self):
        ...
        self.ar_idx = self.combo_box(
            self.__ar_opt_strings, self.ar_idx, 25, (VideoSettingsWindow.WIDTH - 40, 200))
        if old_ar_idx != self.ar_idx:
            tc.global_event(EVENT_AR_SETTING_CHANGED,
                            self.ar_opts[self.ar_idx])
        ...
        self.res_idx = self.combo_box(
            self.res_opt_strings, self.res_idx, 25, (VideoSettingsWindow.WIDTH - 40, 200))
        if old_res_idx != self.res_idx:
            tc.global_event(EVENT_RES_SETTING_CHANGED,
                            self.res_opts[self.res_idx])
        ...
        self.mode_idx = self.combo_box(
            self.mode_opt_strings, self.mode_idx, 25, (VideoSettingsWindow.WIDTH - 40, 200))
        if old_mode_idx != self.mode_idx:
            tc.global_event(EVENT_WINMODE_SETTING_CHANGED,
                            self.mode_opts[self.mode_idx])

        # Window Always On Top
        self.layout_row_dynamic(20, 1)
        self.label_colored_wrap(
            "Window Always On Top (Requires Restart):", (255, 255, 255))
        ...
        if self.win_on_top_idx != old_win_on_top_idx:
            tc.global_event(EVENT_WIN_TOP_SETTING_CHANGED,
                            self.win_on_top_opts[self.win_on_top_idx])
        ...
        if self.vsync_idx != old_vsync_idx:
            tc.global_event(EVENT_VSYNC_SETTING_CHANGED,
                            self.vsync_opts[self.vsync_idx])
        ...
        if self.shadows_idx != old_shadows_idx:
            tc.global_event(EVENT_SHADOWS_SETTING_CHANGED,
                            self.shadows_opts[self.shadows_idx])
        ...
        if self.water_reflect_idx != old_water_reflect_idx:
            tc.global_event(EVENT_WATER_REF_SETTING_CHANGED,
                            self.water_reflect_opts[self.water_reflect_idx])
