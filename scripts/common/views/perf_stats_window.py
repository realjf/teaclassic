...
class PerfStatsWindow(tc.Window):
    ...
    def __init__(self):
        ...
        super(PerfStatsWindow, self).__init__("Performance",
                                              (vresx/2 - PerfStatsWindow.WIDTH/2, vresy/2 - PerfStatsWindow.HEIGHT /
                                               2, PerfStatsWindow.WIDTH, PerfStatsWindow.HEIGHT),
                                              tc.NK_WINDOW_BORDER | tc.NK_WINDOW_MOVABLE | tc.NK_WINDOW_MINIMIZABLE | tc.NK_WINDOW_TITLE |
                                              tc.NK_WINDOW_CLOSABLE, (vresx, vresy))
        self.tickindex = 0
        self.ticksum_ms = 0
        ...
        self.paused = False
        self.trace_python = tc.settings_get("tc.debug.trace_python")
        self.trace_gpu = tc.settings_get("tc.debug.trace_gpu")
    ...
    def frame_perf_tab(self):
        ...
        def layout_children(children):
            ...
            for c in children:
                name = "{:}  [{} children]  [{:.6f} ms]".format(
                    c["name"], len(c["children"]), c["ms_delta"])
                self.tree_element(tc.NK_TREE_NODE, name, tc.NK_MINIMIZED,
                                  False, layout_children, (c["children"],))

        for name, perfdict in self.selected_perfstats.items():
            self.tree_element(tc.NK_TREE_NODE, name, tc.NK_MINIMIZED,
                              False, layout_children, (perfdict["children"],))

    def render_info_tab(self):
        render_info = tc.get_render_info()
        self.layout_row_dynamic(20, 1)
        self.label_colored_wrap("Renderer: %s" %
                                render_info["renderer"], (255, 255, 255))
        self.layout_row_dynamic(20, 1)
        self.label_colored_wrap("Version: %s" %
                                render_info["version"], (255, 255, 255))
        self.layout_row_dynamic(20, 1)
        self.label_colored_wrap("Shading Language Version: %s" %
                                render_info["shading_language_version"], (255, 255, 255))
        self.layout_row_dynamic(20, 1)
        self.label_colored_wrap("Vendor: %s" %
                                render_info["vendor"], (255, 255, 255))

    def nav_stats_tab(self):
        nav_stats = tc.get_nav_perfstats()
    ...
    def threads_tab(self):
        for name in self.frame_perfstats[self.tickindex]:
            ...
            self.simple_chart(tc.NK_CHART_LINES, (0, 200),
                              t_frame_times, lambda idx: None)
    ...
    def update(self):
        ...
        if not self.paused:
            newtick = tc.prev_frame_ms()
            newstats = tc.prev_frame_perfstats()
        ...
        self.simple_chart(tc.NK_CHART_LINES, (0, 200),
                          self.frame_times_ms, self.on_chart_click)
        ...
        self.label_colored_wrap("Avg. Frame Latency: {0:.1f} ms".format(
            avg_frame_latency_ms), (255, 255, 0))

        self.layout_row_dynamic(20, 1)
        self.label_colored_wrap("Max Frame Latency: {0} ms".format(
            self.max_frame_latency), (255, 255, 0))

        self.layout_row_dynamic(20, 1)
        old_trace_python = self.trace_python
        self.trace_python = True if self.checkbox(
            "Trace Python", self.trace_python) else False
        if old_trace_python != self.trace_python:
            tc.settings_set("tc.debug.trace_python",
                            self.trace_python, persist=False)

        self.layout_row_dynamic(20, 1)
        old_trace_gpu = self.trace_gpu
        self.trace_gpu = True if self.checkbox(
            "Trace GPU", self.trace_gpu) else False
        if old_trace_gpu != self.trace_gpu:
            tc.settings_set("tc.debug.trace_gpu",
                            self.trace_gpu, persist=False)
        ...
        self.tree(tc.NK_TREE_TAB, "Frame Performance",
                  tc.NK_MINIMIZED, self.frame_perf_tab)
        self.tree(tc.NK_TREE_TAB, "Threads", tc.NK_MINIMIZED, self.threads_tab)
        self.tree(tc.NK_TREE_TAB, "Renderer Info",
                  tc.NK_MINIMIZED, self.render_info_tab)
        self.tree(tc.NK_TREE_TAB, "Navigation Stats",
                  tc.NK_MINIMIZED, self.nav_stats_tab)
