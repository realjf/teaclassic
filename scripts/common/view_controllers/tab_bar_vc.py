...
class TabBarVC(view_controller.ViewController):
    ...
    def __on_tab_changed(self, event):
        assert self.__active_idx >= 0 and self.__active_idx < len(
            self.__children)
    ...
    def activate(self):
        tc.register_ui_event_handler(
            self.__tce, TabBarVC.__on_tab_changed, self)
    ...
    def deactivate(self):
        ...
        if len(self.__children) > 0:
            self.__children[self.__active_idx].deactivate()
        tc.unregister_event_handler(self.__tce, TabBarVC.__on_tab_changed)
