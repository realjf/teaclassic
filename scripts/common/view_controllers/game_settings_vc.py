...
class GameSettingsVC(vc.ViewController):
    ...
    def __load_selection(self):
        try:
            hb_saved = tc.settings_get("tc.game.healthbar_mode")
            if hb_saved == True:
                self.view.hb_idx = 0
            else:
                self.view.hb_idx = 1
    ...
    def __on_settings_apply(self, event):
        if self.view.hb_idx != self.__og_hb_idx:
            try:
                tc.settings_set("tc.game.healthbar_mode",
                                bool(self.view.hb_idx == 0))
                self.__og_hb_idx = self.view.hb_idx
            except Exception as e:
                print("Could not set tc.game.healthbar_mode:" + str(e))
    ...
    def activate(self):
        tc.register_ui_event_handler(
            EVENT_SETTINGS_APPLY, GameSettingsVC.__on_settings_apply, self)
        tc.register_ui_event_handler(
            EVENT_SETTINGS_HB_MODE_CHANGED, GameSettingsVC.__on_hb_mode_changed, self)

    def deactivate(self):
        tc.unregister_event_handler(
            EVENT_SETTINGS_APPLY, GameSettingsVC.__on_settings_apply)
        tc.unregister_event_handler(
            EVENT_SETTINGS_HB_MODE_CHANGED, GameSettingsVC.__on_hb_mode_changed)
