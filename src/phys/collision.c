...
static bool ray_triangle_intersect(vec3_t ray_origin, vec3_t ray_dir, vec3_t tri_verts[3], float *out_t)
{
    ...
    TCM_Vec3_Sub(&tri_verts[1], &tri_verts[0], &v0v1);
    TCM_Vec3_Sub(&tri_verts[2], &tri_verts[0], &v0v2);

    vec3_t n;
    TCM_Vec3_Cross(&v0v1, &v0v2, &n);

    float n_dot_ray_dir = TCM_Vec3_Dot(&n, &ray_dir);
    if(fabs(n_dot_ray_dir) < EPSILON) {
        /* Ray is parallel to the plane of the triangle */
        return false;
    }

    float d = TCM_Vec3_Dot(&n, &tri_verts[0]);
    float t = (d - TCM_Vec3_Dot(&n, &ray_origin)) / n_dot_ray_dir;
    ...
    TCM_Vec3_Scale(&ray_dir, t, &delta);
    TCM_Vec3_Add(&ray_origin, &delta, &P);
    ...
    TCM_Vec3_Sub(&tri_verts[1], &tri_verts[0], &edge0);
    TCM_Vec3_Sub(&P, &tri_verts[0], &vp0);
    TCM_Vec3_Cross(&edge0, &vp0, &C);

    if(TCM_Vec3_Dot(&n, &C) < 0.0f) {
        return false;
    }

    TCM_Vec3_Sub(&tri_verts[2], &tri_verts[1], &edge1);
    TCM_Vec3_Sub(&P, &tri_verts[1], &vp1);
    TCM_Vec3_Cross(&edge1, &vp1, &C);

    if(TCM_Vec3_Dot(&n, &C) < 0.0f) {
        return false;
    }

    TCM_Vec3_Sub(&tri_verts[0], &tri_verts[2], &edge2);
    TCM_Vec3_Sub(&P, &tri_verts[2], &vp2);
    TCM_Vec3_Cross(&edge2, &vp2, &C);
...
{
    vec3_t diff;
    TCM_Vec3_Sub(&point, (vec3_t*)&plane->point, &diff);
    return TCM_Vec3_Dot(&diff, (vec3_t*)&plane->normal);
}
...
static bool separating_axis_exists(vec3_t axis, const struct frustum *frustum, const vec3_t cuboid_corners[8])
{
    ...
    for(int i = 0; i < 8; i++)
        frust_axis_dots[i] = TCM_Vec3_Dot((vec3_t*)frust_points[i], &axis);

    for(int i = 0; i < 8; i++)
        cuboid_axis_dots[i] = TCM_Vec3_Dot((vec3_t*)&cuboid_corners[i], &axis);
                   ...
                   struct frustum *out)
{
    ...
    vec3_t tmp;
    vec3_t cam_right;
    TCM_Vec3_Cross(&up, (vec3_t*)&front, &cam_right);
    TCM_Vec3_Normal(&cam_right, &cam_right);

    vec3_t nc = pos;
    TCM_Vec3_Scale(&front, near_dist, &tmp);
    TCM_Vec3_Add(&nc, &tmp, &nc);

    vec3_t fc = pos;
    TCM_Vec3_Scale(&front, far_dist, &tmp);
    TCM_Vec3_Add(&nc, &tmp, &fc);

    vec3_t up_half_hfar;
    TCM_Vec3_Scale(&up, far_height/2.0f, &up_half_hfar);

    vec3_t right_half_wfar;
    TCM_Vec3_Scale(&cam_right, far_width/2.0f, &right_half_wfar);

    vec3_t up_half_hnear;
    TCM_Vec3_Scale(&up, near_height/2.0f, &up_half_hnear);

    vec3_t right_half_wnear;
    TCM_Vec3_Scale(&cam_right, near_width/2.0f, &right_half_wnear);


    /* Far Top Left corner */
    TCM_Vec3_Add(&fc, &up_half_hfar, &tmp);
    TCM_Vec3_Sub(&tmp, &right_half_wfar, &out->ftl);

    /* Far Top Right corner */
    TCM_Vec3_Add(&fc, &up_half_hfar, &tmp);
    TCM_Vec3_Add(&tmp, &right_half_wfar, &out->ftr);

    /* Far Bottom Left corner */
    TCM_Vec3_Sub(&fc, &up_half_hfar, &tmp);
    TCM_Vec3_Sub(&tmp, &right_half_wfar, &out->fbl);

    /* Far Bottom Right corner */
    TCM_Vec3_Sub(&fc, &up_half_hfar, &tmp);
    TCM_Vec3_Add(&tmp, &right_half_wfar, &out->fbr);

    /* Near Top Left corner */
    TCM_Vec3_Add(&nc, &up_half_hnear, &tmp);
    TCM_Vec3_Sub(&tmp, &right_half_wnear, &out->ntl);

    /* Near Top Right corner */
    TCM_Vec3_Add(&nc, &up_half_hnear, &tmp);
    TCM_Vec3_Add(&tmp, &right_half_wnear, &out->ntr);

    /* Near Bottom Left corner */
    TCM_Vec3_Sub(&nc, &up_half_hnear, &tmp);
    TCM_Vec3_Sub(&tmp, &right_half_wnear, &out->nbl);

    /* Near Bottom right corner */
    TCM_Vec3_Sub(&nc, &up_half_hnear, &tmp);
    TCM_Vec3_Add(&tmp, &right_half_wnear, &out->nbr);
    ...
    /* Far plane */
    vec3_t negative_dir;
    TCM_Vec3_Scale(&front, -1.0f, &negative_dir);
    ...
    TCM_Vec3_Scale(&cam_right, near_width / 2.0f, &tmp);
    TCM_Vec3_Add(&nc, &tmp, &tmp);
    TCM_Vec3_Sub(&tmp, &pos, &p_to_near_right_edge);
    TCM_Vec3_Normal(&p_to_near_right_edge, &p_to_near_right_edge);

    out->right.point = pos;
    TCM_Vec3_Cross(&p_to_near_right_edge, &up, &out->right.normal);

    /* Left plane */
    vec3_t p_to_near_left_edge;
    TCM_Vec3_Scale(&cam_right, near_width / 2.0f, &tmp);
    TCM_Vec3_Sub(&nc, &tmp, &tmp);
    TCM_Vec3_Sub(&tmp, &pos, &p_to_near_left_edge);
    TCM_Vec3_Normal(&p_to_near_left_edge, &p_to_near_left_edge);

    out->left.point = pos;
    TCM_Vec3_Cross(&up, &p_to_near_left_edge, &out->left.normal);

    /* Top plane */
    vec3_t p_to_near_top_edge;
    TCM_Vec3_Scale(&up, near_height / 2.0f, &tmp);
    TCM_Vec3_Add(&nc, &tmp, &tmp);
    TCM_Vec3_Sub(&tmp, &pos, &p_to_near_top_edge);
    TCM_Vec3_Normal(&p_to_near_top_edge, &p_to_near_top_edge);

    out->top.point = pos;
    TCM_Vec3_Cross(&cam_right, &p_to_near_top_edge, &out->top.normal);

    /* Bot plane */
    vec3_t p_to_near_bot_edge;
    TCM_Vec3_Scale(&up, near_height / 2.0f, &tmp);
    TCM_Vec3_Sub(&nc, &tmp, &tmp);
    TCM_Vec3_Sub(&tmp, &pos, &p_to_near_bot_edge);
    TCM_Vec3_Normal(&p_to_near_bot_edge, &p_to_near_bot_edge);

    out->bot.point = pos;
    TCM_Vec3_Cross(&p_to_near_bot_edge, &cam_right, &out->bot.normal);
}
...
bool C_RayIntersectsOBB(vec3_t ray_origin, vec3_t ray_dir, struct obb obb, float *out_t)
{
    ...
    vec3_t d;
    TCM_Vec3_Sub(&obb.center, &ray_origin, &d);

    for (int i = 0; i < 3; ++i) {

        GLfloat DA = TCM_Vec3_Dot(&ray_dir, &obb.axes[i]);
        GLfloat dA = TCM_Vec3_Dot(&d, &obb.axes[i]);
...
{
    float denom = TCM_Vec3_Dot(&ray_dir, &plane.normal);
    if(fabs(denom) > EPSILON) {

        vec3_t rp;
        TCM_Vec3_Sub(&plane.point, &ray_origin, &rp);
        float t = TCM_Vec3_Dot(&rp, &plane.normal) / denom;
        if(t >= 0.0f) {
            *out_t = t;
            return true;
        }
...
bool C_PointInsideOBB(vec3_t point, struct obb obb)
{
    ...
    for(int i = 0; i < 3; i++) {
        ...
        vec3_t relative;
        TCM_Vec3_Sub(&point, &obb.center, &relative);

        float dot = TCM_Vec3_Dot(&relative, &axis);
        float comp = dot / TCM_Vec3_Len(&axis);
...
{
    vec3_t delta;
    TCM_Vec3_Sub(&begin, &end, &delta);

    if(TCM_Vec3_Len(&delta) < EPSILON)
        return C_PointInsideOBB(begin, obb);

    vec3_t dir;
    TCM_Vec3_Normal(&delta, &dir);
    float t;

    if(!C_RayIntersectsOBB(begin, dir, obb, &t))
        return false;

    return (t >= 0 && t <= TCM_Vec3_Len(&delta));
}
...
bool C_FrustumAABBIntersectionExact(const struct frustum *frustum, const struct aabb *aabb)
{
    ...
    TCM_Vec3_Sub((vec3_t*)&frustum->ntr, (vec3_t*)&frustum->ntl, &frust_edges[0]);
    TCM_Vec3_Sub((vec3_t*)&frustum->ntl, (vec3_t*)&frustum->nbl, &frust_edges[1]);
    TCM_Vec3_Sub((vec3_t*)&frustum->ftl, (vec3_t*)&frustum->ntl, &frust_edges[2]);
    TCM_Vec3_Sub((vec3_t*)&frustum->ftr, (vec3_t*)&frustum->ntr, &frust_edges[3]);
    TCM_Vec3_Sub((vec3_t*)&frustum->fbr, (vec3_t*)&frustum->nbr, &frust_edges[4]);
    TCM_Vec3_Sub((vec3_t*)&frustum->fbl, (vec3_t*)&frustum->nbl, &frust_edges[5]);
    ...
    for(int i = 0; i < ARR_SIZE(aabb_axes); i++) {
        for(int j = 0; j < ARR_SIZE(frust_edges); j++) {
            ...
            TCM_Vec3_Cross(&aabb_axes[i], &frust_edges[j], &edge_cross_products[ncrosses]);
            if(TCM_Vec3_Len(&edge_cross_products[ncrosses]) > EPSILON) {
                TCM_Vec3_Normal(&edge_cross_products[ncrosses], &edge_cross_products[ncrosses]);
                ncrosses++;
            }
        }
    }
...
bool C_FrustumOBBIntersectionExact(const struct frustum *frustum, const struct obb *obb)
{
    ...
    for(int i = 0; i < ARR_SIZE(obb->axes); i++) {
        for(int j = 0; j < ARR_SIZE(frust_edges); j++) {
            ...
            TCM_Vec3_Cross((vec3_t*)&obb->axes[i], &frust_edges[j], &edge_cross_products[ncrosses]);
            if(TCM_Vec3_Len(&edge_cross_products[ncrosses]) > EPSILON) {
                TCM_Vec3_Normal(&edge_cross_products[ncrosses], &edge_cross_products[ncrosses]);
                ncrosses++;
            }
...
{
    vec2_t ap, ab, ad;
    TCM_Vec2_Sub(&point, &a, &ap);
    TCM_Vec2_Sub(&b, &a, &ab);
    TCM_Vec2_Sub(&d, &a, &ad);

    float ap_dot_ab = TCM_Vec2_Dot(&ap, &ab);
    float ap_dot_ad = TCM_Vec2_Dot(&ap, &ad);

    return (ap_dot_ab >= 0.0f && ap_dot_ab <= TCM_Vec2_Dot(&ab, &ab))
        && (ap_dot_ad >= 0.0f && ap_dot_ad <= TCM_Vec2_Dot(&ad, &ad));
}
...
bool C_PointInsideTriangle2D(vec2_t point, vec2_t a, vec2_t b, vec2_t c)
{
    ...
    TCM_Vec2_Sub(&c, &a, &v0);
    TCM_Vec2_Sub(&b, &a, &v1);
    TCM_Vec2_Sub(&point, &a, &v2);

    GLfloat dot00 = TCM_Vec2_Dot(&v0, &v0);
    GLfloat dot01 = TCM_Vec2_Dot(&v0, &v1);
    GLfloat dot02 = TCM_Vec2_Dot(&v0, &v2);
    GLfloat dot11 = TCM_Vec2_Dot(&v1, &v1);
    GLfloat dot12 = TCM_Vec2_Dot(&v1, &v2);
...
{
    vec2_t delta;
    TCM_Vec2_Sub(&point, &origin, &delta);
    return (TCM_Vec2_Len(&delta) <= radius);
}
...
float C_PointLineSegmentShortestDist(vec2_t point, struct line_seg_2d seg)
{
    ...
    vec2_t dir;
    TCM_Vec2_Sub(&b, &a, &dir);

    vec2_t to;
    TCM_Vec2_Sub(&point, &a, &to);
    float t = TCM_Vec2_Dot(&to, &dir) / len_sqrt;
    ...
    vec2_t proj, delta;
    TCM_Vec2_Scale(&dir, t, &delta);
    TCM_Vec2_Add(&a, &delta, &proj);
...
bool C_CircleRectIntersection(vec2_t center, float radius, struct box rect)
{
    ...
    for(int i = 0; i < ARR_SIZE(corners); i++) {
        vec2_t delta;
        TCM_Vec2_Sub(&corners[i], &center, &delta);
        if(TCM_Vec2_Len(&delta) <= radius)
            return true;
    }
