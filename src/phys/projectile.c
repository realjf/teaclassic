...
static quat_t phys_velocity_dir(vec3_t vel)
{
    ...
    mat4x4_t yrotmat;
    TCM_Mat4x4_MakeRotY(yrot, &yrotmat);

    mat4x4_t zrotmat;
    TCM_Mat4x4_MakeRotZ(zrot, &zrotmat);

    mat4x4_t rotmat;
    TCM_Mat4x4_Mult4x4(&yrotmat, &zrotmat, &rotmat);
...
static void phys_proj_update(struct projectile *proj)
{
    ...
    TCM_Vec3_Add(&proj->vel, &accel, &proj->vel);
    TCM_Vec3_Add(&proj->pos, &proj->vel, &proj->pos);

    mat4x4_t trans, scale, rot, tmp;
    quat_t qrot = phys_velocity_dir(proj->vel);

    TCM_Mat4x4_MakeTrans(proj->pos.x, proj->pos.y, proj->pos.z, &trans);
    TCM_Mat4x4_MakeScale(proj->scale.x, proj->scale.y, proj->scale.z, &scale);
    TCM_Mat4x4_RotFromQuat(&qrot, &rot);

    TCM_Mat4x4_Mult4x4(&scale, &rot, &tmp);
    TCM_Mat4x4_Mult4x4(&trans, &tmp, &proj->model);
}
...
static void phys_sweep_test(int front_idx)
{
    ...
    TCM_Vec3_Scale(&delta, -1.0f * s_simticks, &delta);
    TCM_Vec3_Add(&begin, &delta, &end);
    ...
    for(int i = 0; i < nents; i++) {
        ...
        if(C_LineSegIntersectsOBB(begin, end, obb)) {
            ...
            TCM_Vec3_Sub((vec3_t*)&proj->pos, &ent_pos, &diff);

            if(TCM_Vec3_Len(&diff) < min_dist) {
                min_dist = TCM_Vec3_Len(&diff);
                hit_ent = ent;
            }
        }
    }
                          ...
                          uint32_t cookie, int flags, struct proj_desc pd)
{
    ...
    struct projectile proj = (struct projectile){
        ...
        .render_private = AL_RenderPrivateForName(pd.basedir, pd.tcobj),
        .pos = origin,
        .vel = velocity,
        .scale = pd.scale,
    };
...
bool P_Projectile_VelocityForTarget(vec3_t src, vec3_t dst, float init_speed, vec3_t *out)
{
    ...
    if(TCM_Vec3_Len(&velocity) <= EPSILON) {
        return false;
    }
    assert(TCM_Vec3_Len(&velocity) > EPSILON);
    TCM_Vec3_Normal(&velocity, &velocity);
    TCM_Vec3_Scale(&velocity, v, &velocity);
...
bool P_Projectile_SaveState(struct SDL_RWops *stream)
{
    ...
    for(int i = 0; i < vec_size(&s_front); i++) {
        ...
        tc_strlcpy(basedir.val.as_string, dir, sizeof(basedir.val.as_string));
        CHK_TRUE_RET(Attr_Write(stream, &basedir, "basedir"));

        struct attr filename = (struct attr){
            .type = TYPE_STRING,
        };
        tc_strlcpy(filename.val.as_string, name, sizeof(filename.val.as_string));
...
bool P_Projectile_LoadState(struct SDL_RWops *stream)
{
    ...
    for(int i = 0; i < num_proj; i++) {
        ...
        tc_strlcpy(dir, attr.val.as_string, ARR_SIZE(dir));

        CHK_TRUE_RET(Attr_Parse(stream, &attr, true));
        CHK_TRUE_RET(attr.type == TYPE_STRING);
        tc_strlcpy(name, attr.val.as_string, ARR_SIZE(name));

        AL_PreloadTCObj(dir, name);
