  ...
  while (1) {
    ...
    case TS_REQ_DELAY: {
      struct delay_desc dd = (struct delay_desc){
          .tid = tid, .wake_tick = curr_tick + request.ticks};
...
static void nameserver_exit(void *arg) {
  ...
  kh_foreach(names, key, tid, { free((void *)key); });
...
static struct result nameserver_task(void *arg) {
  ...
  while (1) {
    ...
    case NS_REQ_REGISTER: {
      ...
      if (k == kh_end(names)) {
        int status;
        k = kh_put(tid, names, tc_strdup(request.name), &status);
    ...
    case NS_REQ_WHOIS: {
      ...
      if (resp == NULL_TID && request.blocking) {
        ...
        if (k == kh_end(waiters)) {
          ...
          int status;
          k = kh_put(tidq, waiters, tc_strdup(request.name), &status);
          assert(status != -1 && status != 0);
...
void Task_Send(uint32_t tid, void *msg, size_t msglen, void *reply,
               size_t replylen) {
...
void Task_SetDestructor(void (*destructor)(void *), void *darg) {
  Sched_Request((struct request){.type = SCHED_REQ_SET_DESTRUCTOR,
                                 .argv[0] = (uint64_t)destructor,
                                 .argv[1] = (uint64_t)darg});
}

uint32_t Task_Create(int prio, task_t code, void *arg, struct future *result,
                     int flags) {
...
bool Task_Wait(uint32_t tid) {
  return Sched_Request(
      (struct request){.type = SCHED_REQ_WAIT, .argv[0] = (uint64_t)tid});
}

void Task_Sleep(int ms) {
  struct ts_req tr = (struct ts_req){.type = TS_REQ_DELAY, .ticks = ms};
  int resp;
  Task_Send(s_ts_tid, &tr, sizeof(tr), &resp, sizeof(resp));
}

void Task_Register(const char *name) {
  struct ns_req nr = (struct ns_req){.type = NS_REQ_REGISTER, .name = name};
  int resp;
...
uint32_t Task_WhoIs(const char *name, bool blocking) {
  struct ns_req nr =
      (struct ns_req){.type = NS_REQ_WHOIS, .name = name, .blocking = blocking};
  uint32_t resp;
  Task_Send(s_ns_tid, &nr, sizeof(nr), &resp, sizeof(resp));
  return resp;
}
