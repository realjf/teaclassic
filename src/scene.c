...
__KHASH_IMPL(attr, extern, kh_cstr_t, struct attr, 1, kh_str_hash_func,
             kh_str_hash_equal)
...
static bool scene_load_entity(SDL_RWops *stream) {
  ...
  for (int i = 0; i < num_atts; i++) {
    ...
    int ret;
    khiter_t k = kh_put(attr, attr_table, tc_strdup(attr.key), &ret);
    assert(ret != -1 && ret != 0);
...
static bool scene_load_section(SDL_RWops *stream) {
  ...
  struct {
    const char *name;
    bool (*func)(SDL_RWops *);
  } map[] = {{"factions", scene_load_factions},
