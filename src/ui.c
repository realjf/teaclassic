...
static void ui_draw_text(const struct text_desc desc) {
  ...
  struct nk_rect rect =
      nk_rect(desc.rect.x, desc.rect.y, desc.rect.w, desc.rect.h);
  struct nk_color rgba =
      nk_rgba(desc.rgba.r, desc.rgba.g, desc.rgba.b, desc.rgba.a);
...
static void on_update_ui(void *user, void *event) {
  ...
  nk_style_push_style_item(&s_ctx, &s->window.fixed_background,
                           nk_style_item_color(nk_rgba(0, 0, 0, 0)));

  const vec2_t adj_vres = UI_ArAdjustedVRes(s_vres);
  struct rect adj_bounds =
      UI_BoundsForAspectRatio((struct rect){0, 0, s_vres.x, s_vres.y}, s_vres,
                              adj_vres, ANCHOR_DEFAULT);
...
static int left_x_point(struct rect from_bounds, vec2_t from_res, vec2_t to_res,
                        int resize_mask) {
  ...
  case ANCHOR_X_CENTER | ANCHOR_X_RIGHT: {
    int to_half_width =
        (from_bounds.x - from_right_margin) - (to_res.x / 2 + from_center_off);
...
static int right_x_point(struct rect from_bounds, vec2_t from_res,
...
static int top_y_point(struct rect from_bounds, vec2_t from_res, vec2_t to_res,
                       int resize_mask) {
  ...
  case ANCHOR_Y_CENTER | ANCHOR_Y_BOT: {
    int to_half_height =
        (to_res.y - from_bot_margin) - (to_res.y / 2 + from_center_off);
...
static int bot_y_point(struct rect from_bounds, vec2_t from_res, vec2_t to_res,
                       int resize_mask) {
  int y_res_mask = resize_mask & ANCHOR_Y_MASK;
  int from_top_margin = from_bounds.y;
  int from_bot_margin = from_res.y - (from_bounds.y + from_bounds.h);
...
static void *push_draw_list(const struct nk_draw_list *dl) {
  ...
  void *st_vbuff =
      R_PushArg(dl->vertices->memory.ptr, dl->vertices->memory.size);
  void *st_ebuff =
      R_PushArg(dl->elements->memory.ptr, dl->elements->memory.size);
...
static void ui_init_font_stash(struct nk_context *ctx) {
  ...
  tc_snprintf(fontdir, sizeof(fontdir), "%s/%s", g_basepath, "assets/fonts");
  ...
  int result;
  khiter_t k = kh_put(font, s_fontmap, tc_strdup("__default__"), &result);
  ...
  for (int i = 0; i < nfiles; i++) {
    ...
    if (files[i].is_dir)
      continue;
    if (!tc_endswith(files[i].name, ".ttf"))
      continue;

    char path[NK_MAX_PATH_LEN];
    tc_snprintf(path, sizeof(path), "%s/%s", fontdir, files[i].name);

    struct nk_font *font =
        nk_font_atlas_add_from_file(&s_atlas, path, 16, NULL);
    if (!font)
      continue;

    int result;
    khiter_t k = kh_put(font, s_fontmap, tc_strdup(files[i].name), &result);
    assert(result != -1 && result != 0);
...
static void ui_render(void *user, void *event) {
  ...
  static const struct nk_draw_vertex_layout_element vertex_layout[] = {
      {NK_VERTEX_POSITION, NK_FORMAT_FLOAT,
       NK_OFFSETOF(struct ui_vert, screen_pos)},
...
bool UI_Init(const char *basedir, SDL_Window *win) {
  ...
  E_Global_Register(EVENT_RENDER_UI, ui_render, NULL,
...
void UI_InputBegin(void) { nk_input_begin(&s_ctx); }

void UI_InputEnd(void) { nk_input_end(&s_ctx); }
...
void UI_HandleEvent(SDL_Event *evt) {
  if (evt->type == SDL_KEYUP || evt->type == SDL_KEYDOWN) {
    ...
    if (sym == SDLK_RSHIFT || sym == SDLK_LSHIFT)
      ...
      nk_input_key(&s_ctx, NK_KEY_TEXT_LINE_START,
                   down && state[SDL_SCANCODE_LCTRL]);
    else if (sym == SDLK_e)
      nk_input_key(&s_ctx, NK_KEY_TEXT_LINE_END,
                   down && state[SDL_SCANCODE_LCTRL]);
    else if (sym == SDLK_UP)
...
void UI_DrawText(const char *text, struct rect rect, struct rgba rgba) {
  ...
  tc_strlcpy(d.text, text, sizeof(d.text));
...
vec2_t UI_ArAdjustedVRes(vec2_t vres) {
  ...
  if (curr_ar < old_ar) { /* vertical compression */
    ...
    return (vec2_t){round(vres.x * (curr_ar / old_ar)), round(vres.y)};

  } else { /* horizontal compression */

    return (vec2_t){round(vres.x), round(vres.y * (old_ar / curr_ar))};
  }
}

vec2_t UI_GetTextVres(void) { return s_vres; }
...
struct rect UI_BoundsForAspectRatio(struct rect from_bounds, vec2_t from_res,
  ...
  return (struct rect){left_x, top_y, right_x - left_x, bot_y - top_y};
}
