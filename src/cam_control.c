...
static void fps_cam_on_update_end(void *unused1, void *unused2) {
  ...
  TCM_Vec3_Scale(&front, -1.0f, &back);

  /* Find a vector that is orthogonal to 'front' in the XZ plane */
  vec3_t xz = (vec3_t){front.z, 0.0f, -front.x};
  TCM_Vec3_Cross(&front, &xz, &up);
  TCM_Vec3_Normal(&up, &up);

  TCM_Vec3_Cross(&front, &up, &left);
  TCM_Vec3_Normal(&left, &left);
  TCM_Vec3_Scale(&left, -1.0f, &right);

  vec3_t dir = (vec3_t){0.0f, 0.0f, 0.0f};

  if (ctx->front_state != KEY_RELEASED)
    TCM_Vec3_Add(&dir, &front, &dir);
  if (ctx->left_state != KEY_RELEASED)
    TCM_Vec3_Add(&dir, &left, &dir);
  if (ctx->back_state != KEY_RELEASED)
    TCM_Vec3_Add(&dir, &back, &dir);
  if (ctx->right_state != KEY_RELEASED)
    TCM_Vec3_Add(&dir, &right, &dir);
...
static void rts_cam_on_mousedown(void *unused, void *event_arg) {
  ...
  if (ctx->scroll_up || ctx->scroll_down || ctx->scroll_left ||
      ctx->scroll_right)
    return;
...
static void rts_cam_on_update_end(void *unused1, void *unused2) {
   ...
   *          +Z
   *
   * We want the behavior in which the camera is always scrolled up, down, left,
   * right depending on which corner/edge of the screen the mouse is touching.
   * However, which direction is 'up' or 'left' depends completely on where the
   * camera is facing. For example, 'up' becomes 'down' when the camera pitch is
   * changed from 90* to 270*.
   */

  vec3_t up =
      (vec3_t){1.0f * cos(DEG_TO_RAD(yaw)), 0.0f, -1.0f * sin(DEG_TO_RAD(yaw))};
  vec3_t left =
      (vec3_t){1.0f * sin(DEG_TO_RAD(yaw)), 0.0f, 1.0f * cos(DEG_TO_RAD(yaw))};
  ...
  if (!ctx->pan_disabled) {
    ...
    if (ctx->scroll_up || ctx->front_state != KEY_RELEASED)
      TCM_Vec3_Add(&dir, &up, &dir);
    if (ctx->scroll_down || ctx->back_state != KEY_RELEASED)
      TCM_Vec3_Add(&dir, &down, &dir);
  }
...
void CamControl_UninstallActive(void) {
  if (s_cam_ctx.installed_on_keydown)
    E_Global_Unregister(SDL_KEYDOWN, s_cam_ctx.installed_on_keydown);
  if (s_cam_ctx.installed_on_keyup)
    E_Global_Unregister(SDL_KEYUP, s_cam_ctx.installed_on_keyup);
  if (s_cam_ctx.installed_on_mousemove)
    E_Global_Unregister(SDL_MOUSEMOTION, s_cam_ctx.installed_on_mousemove);
  if (s_cam_ctx.installed_on_mousedown)
    E_Global_Unregister(SDL_MOUSEBUTTONDOWN, s_cam_ctx.installed_on_mousedown);
  if (s_cam_ctx.installed_on_mouseup)
    E_Global_Unregister(SDL_MOUSEBUTTONUP, s_cam_ctx.installed_on_mouseup);
  if (s_cam_ctx.installed_on_update_end)
    E_Global_Unregister(EVENT_UPDATE_END, s_cam_ctx.installed_on_update_end);
