...
#define MARKER_OBJ "build-site-marker.tcobj"
#define CENTER_MARKER_DIR "assets/models/build_site"
#define CENTER_MARKER_OBJ "build-site.tcobj"
...
static uint64_t td_key(const struct tile_desc *td) {
  return (((uint64_t)td->chunk_r << 48) | ((uint64_t)td->chunk_c << 32) |
          ((uint64_t)td->tile_r << 16) | ((uint64_t)td->tile_c << 0));
}

static void building_mark_border(struct buildstate *bs, struct tile_desc *a,
                                 struct tile_desc *b) {
  ...
  vec2_t acenter =
      (vec2_t){abox.x - abox.width / 2.0f, abox.z + abox.height / 2.0f};
  vec2_t bcenter =
      (vec2_t){bbox.x - bbox.width / 2.0f, bbox.z + bbox.height / 2.0f};

  vec2_t center;
  TCM_Vec2_Add(&acenter, &bcenter, &center);
  TCM_Vec2_Scale(&center, 0.5f, &center);

  vec3_t marker_pos =
      (vec3_t){center.x, M_HeightAtPoint(s_map, center), center.z};

  struct entity *ent = AL_EntityFromTCObj(
      MARKER_DIR, MARKER_OBJ, "__build_site_marker__", Entity_NewUID());
  if (!ent)
    return;
...
static void building_mark_center(struct buildstate *bs,
                                 const struct entity *ent) {
  vec3_t pos = G_Pos_Get(ent->uid);

  struct entity *marker =
      AL_EntityFromTCObj(CENTER_MARKER_DIR, CENTER_MARKER_OBJ,
...
static void building_place_markers(struct buildstate *bs,
                                   const struct entity *ent) {
  ...
  size_t ntiles =
      M_Tile_AllUnderObj(M_GetPos(s_map), res, &obb, tds, ARR_SIZE(tds));
...
void G_Building_RemoveEntity(const struct entity *ent) {
  ...
  E_Entity_Unregister(EVENT_STORAGE_SITE_AMOUNT_CHANGED, ent->uid,
                      on_amount_changed);
...
bool G_Building_Found(struct entity *ent, bool blocking) {
  ...
  struct entity *progress = AL_EntityFromTCObj(ent->basedir, ent->filename,
                                               ent->name, Entity_NewUID());
  if (progress) {
  ...
  E_Entity_Register(EVENT_STORAGE_SITE_AMOUNT_CHANGED, ent->uid,
                    on_amount_changed, (void *)((uintptr_t)ent->uid),
                    G_RUNNING);
  return true;
}
...
bool G_Building_SaveState(struct SDL_RWops *stream) {
  ...
  kh_foreach(s_entity_state_table, uid, curr, {
    ...
    struct attr frac_done =
        (struct attr){.type = TYPE_FLOAT, .val.as_float = curr.frac_done};
    CHK_TRUE_RET(Attr_Write(stream, &frac_done, "building_frac_done"));

    struct attr building_hp =
        (struct attr){.type = TYPE_INT,
                      .val.as_int = (ent->flags & ENTITY_FLAG_COMBATABLE)
                                        ? G_Combat_GetCurrentHP(ent)
                                        : 0};
    CHK_TRUE_RET(Attr_Write(stream, &building_hp, "building_hp"));

    struct attr building_vis_range =
        (struct attr){.type = TYPE_FLOAT, .val.as_float = curr.vision_range};
    CHK_TRUE_RET(Attr_Write(stream, &building_vis_range, "building_vis_range"));

    struct attr building_blocking =
        (struct attr){.type = TYPE_BOOL, .val.as_bool = curr.blocking};
    CHK_TRUE_RET(Attr_Write(stream, &building_blocking, "building_blocking"));

    struct attr building_ss =
        (struct attr){.type = TYPE_BOOL, .val.as_bool = curr.is_storage_site};
    CHK_TRUE_RET(Attr_Write(stream, &building_ss, "building_ss"));

    struct attr num_required =
        (struct attr){.type = TYPE_INT, .val.as_int = kh_size(curr.required)};
    ...
    kh_foreach(curr.required, required_key, required_amount, {
      struct attr required_key_attr = (struct attr){
          .type = TYPE_STRING,
      };
      tc_strlcpy(required_key_attr.val.as_string, required_key,
                 sizeof(required_key_attr.val.as_string));
      CHK_TRUE_RET(Attr_Write(stream, &required_key_attr, "required_key"));

      struct attr required_amount_attr =
          (struct attr){.type = TYPE_INT, .val.as_int = required_amount};
      CHK_TRUE_RET(
          Attr_Write(stream, &required_amount_attr, "required_amount"));
    });
