...
__KHASH_IMPL(entity, extern, khint32_t, struct entity *, 1, kh_int_hash_func,
             kh_int_hash_equal)
__KHASH_IMPL(faction, extern, khint32_t, int, 1, kh_int_hash_func,
             kh_int_hash_equal)
__KHASH_IMPL(range, extern, khint32_t, float, 1, kh_int_hash_func,
             kh_int_hash_equal)
...
static vec2_t g_default_minimap_pos(void) {
  struct sval res =
      (struct sval){.type = ST_TYPE_VEC2, .as_vec2 = (vec2_t){1920, 1080}};
  const float PAD = 10.0f;
  int size = 256;

  return (vec2_t){
      (size + 2 * MINIMAP_BORDER_WIDTH) / cos(M_PI / 4.0f) / 2 + PAD,
      res.as_vec2.y - (size + 2 * MINIMAP_BORDER_WIDTH) / cos(M_PI / 4.0f) / 2 -
          PAD,
  };
}
...
static void g_shadow_pass(struct render_input *in) {
  ...
  R_PushCmd((struct rcmd){
      .func = R_GL_Batch_RenderDepthMap, .nargs = 1, .args = {in}});
  ...
  for (int i = 0; i < vec_size(&in->light_vis_anim); i++) {
    ...
    TCM_Mat4x4_Inverse(&curr->model, &model);
    TCM_Mat4x4_Transpose(&model, &normal);
...
static void g_draw_pass(struct render_input *in) {
  ...
  R_PushCmd((struct rcmd){.func = R_GL_Batch_Draw, .nargs = 1, .args = {in}});
...
static void g_render_healthbars(void) {
  ...
  ss_e status = Settings_Get("tc.game.healthbar_mode", &hb_setting);
...
static void g_sort_stat_list(vec_rstat_t *inout) {
  ...
  while (i < vec_size(inout)) {
    int j = i;
    while (j > 0 && vec_AT(inout, j - 1).translucent &&
           !vec_AT(inout, j).translucent) {
...
static void g_make_draw_list(vec_pentity_t ents, vec_rstat_t *out_stat,
                             vec_ranim_t *out_anim) {
  ...
  for (int i = 0; i < vec_size(&ents); i++) {
    ...
    if (curr->flags & ENTITY_FLAG_ANIMATED) {
      ...
      A_GetRenderState(curr->uid, &rstate.njoints, rstate.curr_pose,
                       &rstate.inv_bind_pose);
      vec_ranim_push(out_anim, rstate);
    } else {

      struct tile_desc td = {0};
      if (s_gs.map) {
        M_Tile_DescForPoint2D(res, M_GetPos(s_gs.map), G_Pos_GetXZ(curr->uid),
                              &td);
      }
...
static void g_create_render_input(struct render_input *out) {
  ...
  ss_e status = Settings_Get("tc.video.shadows_enabled", &shadows_setting);
  assert(status == SS_OKAY);
  ...
  g_make_draw_list(s_gs.light_visible, &out->light_vis_stat,
                   &out->light_vis_anim);
...
static bool g_save_anim_state(SDL_RWops *stream) {
  ...
  struct attr num_anim = (struct attr){.type = TYPE_INT, .val.as_int = nanim};
...
static bool g_pentities_equal(struct entity *const *a,
                              struct entity *const *b) {
  return ((*a) == (*b));
}
...
static void g_change_simstate(void) {
  ...
  E_Global_Notify(EVENT_GAME_SIMSTATE_CHANGED, (void *)s_gs.requested_ss,
                  ES_ENGINE);
...
static void g_create_settings(void) {
  ...
  status = Settings_Create((struct setting){
      .name = "tc.game.healthbar_mode",
      .val = (struct sval){.type = ST_TYPE_INT, .as_int = HB_MODE_ALWAYS},
      .prio = 0,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.game.fog_of_war_enabled",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = true},
      .prio = 0,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.navigation_layer",
      .val = (struct sval){.type = ST_TYPE_INT, .as_int = 0},
      .prio = 0,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_navigation_cost_base",
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_last_cmd_flow_field",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_first_sel_movestate",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_first_sel_combined_hrvo",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_enemy_seek_fields",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.enemy_seek_fields_faction_id",
      .val = (struct sval){.type = ST_TYPE_INT, .as_int = 0},
      .prio = 0,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_navigation_blockers",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_navigation_portals",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_navigation_island_ids",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_navigation_local_island_ids",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_chunk_boundaries",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_faction_vision",
      .val =
          (struct sval){
              .type = ST_TYPE_INT,
              .as_int =
                  -1 /* -1 for none, else the faction ID to show vision for */
          },
      .prio = 0,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_combat_targets",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.debug.show_combat_ranges",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = false},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
...
static void g_render_minimap_units(void) {
  ...
  kh_foreach(s_gs.active, key, curr, {
    ...
    TCM_Vec3_Scale(&norm_color, 1.0f / 255, &norm_color);
...
bool G_LoadMap(SDL_RWops *stream, bool update_navgrid) {
  ...
  s_gs.map = AL_MapFromTCMapStream(stream, update_navgrid);
  if (!s_gs.map)
    PERF_RETURN(false);
  ...
  struct map_resolution res;
  M_GetResolution(s_gs.map, &res);
  R_PushCmd((struct rcmd){.func = R_GL_Batch_AllocChunks,
                          .nargs = 1,
                          .args = {
...
void G_Update(void) {
  ...
  kh_foreach(s_gs.active, key, curr, {
    ...
    /* Note that there may be some false positives due to using the fast frustum
     * cull. */
    if (C_FrustumOBBIntersectionFast(&cam_frust, &obb) !=
    ...
    if (C_FrustumOBBIntersectionFast(&light_frust, &obb) !=
        VOLUME_INTERSEC_OUTSIDE) {
      if (!vis_checked) {
        vis = g_ent_visible(pm, curr, &obb);
      }
...
void G_Render(void) {
  ...
  status = Settings_Get("tc.video.water_refraction", &refract_setting);
  assert(status == SS_OKAY);

  struct sval reflect_setting;
  status = Settings_Get("tc.video.water_reflection", &reflect_setting);
  assert(status == SS_OKAY);
          ...
          .args =
              {
                  ...
                  R_PushArg(&g_seltype_color_map[sel_type],
                            sizeof(g_seltype_color_map[0])),
...
bool G_AddFaction(const char *name, vec3_t color) {
  ...
  E_Global_Notify(EVENT_UPDATE_FACTION, (void *)((uintptr_t)new_fac_id),
                  ES_ENGINE);
  return true;
}
...
bool G_RemoveFaction(int faction_id) {
  ...
  E_Global_Notify(EVENT_UPDATE_FACTION, (void *)((uintptr_t)faction_id),
                  ES_ENGINE);
  return true;
}
...
bool G_UpdateFaction(int faction_id, const char *name, vec3_t color,
  ...
  tc_strlcpy(s_gs.factions[faction_id].name, name,
             sizeof(s_gs.factions[0].name));
...
uint16_t G_GetFactions(char out_names[][MAX_FAC_NAME_LEN], vec3_t *out_colors,
  ...
  for (int i = 0; i < MAX_FACTIONS; i++) {
    ...
    if (out_names) {
      tc_strlcpy(out_names[i], s_gs.factions[i].name, MAX_FAC_NAME_LEN);
    }
    if (out_colors) {
...
bool G_GetDiplomacyState(int fac_id_a, int fac_id_b,
...
void G_MoveActiveCamera(vec2_t xz_ground_pos) {
  ...
  float offset_mag = cos(DEG_TO_RAD(Camera_GetPitch(s_gs.active_cam))) *
                     Camera_GetHeight(s_gs.active_cam);

  /* We position the camera such that the camera ray intersects the ground plane
   * (Y=0) at the specified xz position. */
  vec3_t new_pos = (vec3_t){
      xz_ground_pos.x -
          cos(DEG_TO_RAD(Camera_GetYaw(s_gs.active_cam))) * offset_mag,
      old_pos.y,
      xz_ground_pos.z +
          sin(DEG_TO_RAD(Camera_GetYaw(s_gs.active_cam))) * offset_mag};
...
bool G_SaveGlobalState(SDL_RWops *stream) {
  ...
  struct attr hasmap =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = (s_gs.map != NULL)};
  CHK_TRUE_RET(Attr_Write(stream, &hasmap, "has_map"));

  if (hasmap.val.as_bool && !M_AL_WriteTCMap(s_gs.map, stream))
    return false;
  ...
  for (int i = 0; i < MAX_FACTIONS; i++) {
    ...
    struct attr fac_id = (struct attr){.type = TYPE_INT, .val.as_int = i};
    CHK_TRUE_RET(Attr_Write(stream, &fac_id, "fac_id"));

    struct attr fac_color =
        (struct attr){.type = TYPE_VEC3, .val.as_vec3 = fac.color};
    CHK_TRUE_RET(Attr_Write(stream, &fac_color, "fac_color"));

    struct attr fac_name = (struct attr){.type = TYPE_STRING};
    tc_snprintf(fac_name.val.as_string, sizeof(fac_name.val.as_string), "%s",
                fac.name);
    CHK_TRUE_RET(Attr_Write(stream, &fac_name, "fac_name"));

    struct attr fac_controllable =
        (struct attr){.type = TYPE_BOOL, .val.as_bool = fac.controllable};
  ...
  for (int i = 0; i < MAX_FACTIONS; i++) {
    for (int j = 0; j < MAX_FACTIONS; j++) {
      ...
      struct attr dstate = (struct attr){
          .type = TYPE_INT, .val.as_int = s_gs.diplomacy_table[i][j]};
      CHK_TRUE_RET(Attr_Write(stream, &dstate, "diplomacy_state"));
    }
  }

  struct attr cam_speed = (struct attr){
      .type = TYPE_FLOAT, .val.as_float = Camera_GetSpeed(s_gs.active_cam)};
  CHK_TRUE_RET(Attr_Write(stream, &cam_speed, "cam_speed"));

  struct attr cam_sens = (struct attr){
      .type = TYPE_FLOAT, .val.as_float = Camera_GetSens(s_gs.active_cam)};
  CHK_TRUE_RET(Attr_Write(stream, &cam_sens, "cam_sensitivity"));

  struct attr cam_pitch = (struct attr){
      .type = TYPE_FLOAT, .val.as_float = Camera_GetPitch(s_gs.active_cam)};
  CHK_TRUE_RET(Attr_Write(stream, &cam_pitch, "cam_pitch"));

  struct attr cam_yaw = (struct attr){
      .type = TYPE_FLOAT, .val.as_float = Camera_GetYaw(s_gs.active_cam)};
  CHK_TRUE_RET(Attr_Write(stream, &cam_yaw, "cam_yaw"));

  struct attr cam_pos = (struct attr){
      .type = TYPE_VEC3, .val.as_vec3 = Camera_GetPos(s_gs.active_cam)};
  CHK_TRUE_RET(Attr_Write(stream, &cam_pos, "cam_position"));

  struct attr active_cam_mode =
      (struct attr){.type = TYPE_INT, .val.as_int = s_gs.active_cam_mode};
  CHK_TRUE_RET(Attr_Write(stream, &active_cam_mode, "active_cam_mode"));

  struct attr active_font = (struct attr){
      .type = TYPE_STRING,
  };
  tc_strlcpy(active_font.val.as_string, UI_GetActiveFont(),
             sizeof(active_font.val.as_string));
  CHK_TRUE_RET(Attr_Write(stream, &active_font, "active_font"));

  struct attr hide_healthbars =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = s_gs.hide_healthbars};
  CHK_TRUE_RET(Attr_Write(stream, &hide_healthbars, "hide_healthbars"));

  struct attr minimap_render_all =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = s_gs.minimap_render_all};
...
bool G_LoadGlobalState(SDL_RWops *stream) {
  ...
  for (int i = 0; i < num_factions; i++) {
    ...
    tc_snprintf(fac.name, sizeof(fac.name), "%s", attr.val.as_string);
