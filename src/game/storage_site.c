...
static bool ss_state_init(struct ss_state *hs) {
  ...
  if (!hs->capacity || !hs->curr || !hs->desired || !hs->alt_capacity ||
      !hs->alt_desired) {
...
static size_t ss_get_keys(struct ss_state *hs, const char **out,
                          size_t maxout) {
  size_t ret = 0;
...
static void on_update_ui(void *user, void *event) {
  ...
  ss_e status = Settings_Get("tc.game.storage_site_ui_mode", &ui_setting);
  ...
  kh_foreach(s_entity_state_table, key, curr, {
    ...
    char name[256];
    tc_snprintf(name, sizeof(name), "__storage_site__.%x", key);
    ...
    const int flags = NK_WINDOW_NOT_INTERACTIVE | NK_WINDOW_BORDER |
                      NK_WINDOW_BACKGROUND | NK_WINDOW_NO_SCROLLBAR;

    struct rect adj_bounds =
        UI_BoundsForAspectRatio((struct rect){pos.x, pos.y, width, height},
    ...
    if (nk_begin_with_vres(ctx, name,
                           (struct nk_rect){adj_bounds.x, adj_bounds.y,
                                            adj_bounds.w, adj_bounds.h},
                           flags, (struct nk_vec2i){adj_vres.x, adj_vres.y})) {

      for (int i = 0; i < nnames; i++) {

        int capacity = curr.use_alt
                           ? G_StorageSite_GetAltCapacity(key, names[i])
        ...
        tc_snprintf(curr, sizeof(curr), "%4d",
                    G_StorageSite_GetCurr(key, names[i]));
        tc_snprintf(cap, sizeof(cap), "%4d", capacity);
        tc_snprintf(des, sizeof(des), "(%4d)", desired);

        nk_layout_row_begin(ctx, NK_DYNAMIC, 16, 2);
        nk_layout_row_push(ctx, 0.30f);
        nk_label_colored(ctx, names[i],
                         NK_TEXT_ALIGN_LEFT | NK_TEXT_ALIGN_MIDDLE, s_font_clr);
        ...
        nk_layout_row_push(ctx, 0.30f);
        nk_label_colored(ctx, des, NK_TEXT_ALIGN_LEFT | NK_TEXT_ALIGN_MIDDLE,
                         s_font_clr);
      }
    }
    nk_end(ctx);
...
static bool save_color(struct nk_color clr, SDL_RWops *stream) {
  struct attr clr_r = (struct attr){.type = TYPE_INT, .val.as_int = clr.r};
  CHK_TRUE_RET(Attr_Write(stream, &clr_r, "clr_r"));

  struct attr clr_g = (struct attr){.type = TYPE_INT, .val.as_int = clr.g};
  CHK_TRUE_RET(Attr_Write(stream, &clr_g, "clr_g"));

  struct attr clr_b = (struct attr){.type = TYPE_INT, .val.as_int = clr.b};
  CHK_TRUE_RET(Attr_Write(stream, &clr_b, "clr_b"));

  struct attr clr_a = (struct attr){.type = TYPE_INT, .val.as_int = clr.a};
...
static bool save_global_resources(int i, SDL_RWops *stream) {
  struct attr num_global_resources = (struct attr){
      .type = TYPE_INT, .val.as_int = kh_size(s_global_resource_tables[i])};
  CHK_TRUE_RET(
      Attr_Write(stream, &num_global_resources, "num_global_resources"));

  const char *resource_key;
  int resource_amount;

  kh_foreach(s_global_resource_tables[i], resource_key, resource_amount, {
    struct attr resource_key_attr = (struct attr){
        .type = TYPE_STRING,
    };
    tc_strlcpy(resource_key_attr.val.as_string, resource_key,
               sizeof(resource_key_attr.val.as_string));
    CHK_TRUE_RET(Attr_Write(stream, &resource_key_attr, "resource_key"));

    struct attr resource_amount_attr =
        (struct attr){.type = TYPE_INT, .val.as_int = resource_amount};
...
static bool save_global_capacities(int i, SDL_RWops *stream) {
  struct attr num_global_capacities = (struct attr){
      .type = TYPE_INT, .val.as_int = kh_size(s_global_capacity_tables[i])};
  CHK_TRUE_RET(
      Attr_Write(stream, &num_global_capacities, "num_global_capacities"));

  const char *capacity_key;
  int capacity_amount;

  kh_foreach(s_global_capacity_tables[i], capacity_key, capacity_amount, {
    struct attr capacity_key_attr = (struct attr){
        .type = TYPE_STRING,
    };
    tc_strlcpy(capacity_key_attr.val.as_string, capacity_key,
               sizeof(capacity_key_attr.val.as_string));
    CHK_TRUE_RET(Attr_Write(stream, &capacity_key_attr, "capacity_key"));

    struct attr capacity_amount_attr =
        (struct attr){.type = TYPE_INT, .val.as_int = capacity_amount};
...
bool G_StorageSite_Init(void) {
  ...
  status = Settings_Create((struct setting){
      .name = "tc.game.storage_site_ui_mode",
      .val = (struct sval){.type = ST_TYPE_INT, .as_int = SS_UI_SHOW_ALWAYS},
      .prio = 0,
...
bool G_StorageSite_SetCurr(const struct entity *ent, const char *rname,
  ...
  if (delta) {
    ss->last_change = (struct ss_delta_event){
        .name = si_intern(rname, &s_stringpool, s_stridx), .delta = delta};
    E_Entity_Notify(EVENT_STORAGE_SITE_AMOUNT_CHANGED, ent->uid,
                    &ss->last_change, ES_ENGINE);
  }
...
void G_StorageSite_SetShowUI(bool show) { s_show_ui = show; }
...
bool G_StorageSite_SaveState(struct SDL_RWops *stream) {
  ...
  kh_foreach(s_entity_state_table, key, curr, {
    ...
    struct attr use_alt =
        (struct attr){.type = TYPE_BOOL, .val.as_bool = curr.use_alt};
    CHK_TRUE_RET(Attr_Write(stream, &use_alt, "use_alt"));

    struct attr num_capacity =
        (struct attr){.type = TYPE_INT, .val.as_int = kh_size(curr.capacity)};
    CHK_TRUE_RET(Attr_Write(stream, &num_capacity, "num_capacity"));

    const char *cap_key;
    int cap_amount;
    kh_foreach(curr.capacity, cap_key, cap_amount, {
      struct attr cap_key_attr = (struct attr){
          .type = TYPE_STRING,
      };
      tc_strlcpy(cap_key_attr.val.as_string, cap_key,
                 sizeof(cap_key_attr.val.as_string));
      CHK_TRUE_RET(Attr_Write(stream, &cap_key_attr, "cap_key"));

      struct attr cap_amount_attr =
          (struct attr){.type = TYPE_INT, .val.as_int = cap_amount};
      CHK_TRUE_RET(Attr_Write(stream, &cap_amount_attr, "cap_amount"));
    });

    struct attr num_curr =
        (struct attr){.type = TYPE_INT, .val.as_int = kh_size(curr.curr)};
    ...
    struct attr num_desired =
        (struct attr){.type = TYPE_INT, .val.as_int = kh_size(curr.desired)};
    ...
    kh_foreach(curr.desired, desired_key, desired_amount, {
      struct attr desired_key_attr = (struct attr){
          .type = TYPE_STRING,
      };
      tc_strlcpy(desired_key_attr.val.as_string, desired_key,
                 sizeof(desired_key_attr.val.as_string));
      CHK_TRUE_RET(Attr_Write(stream, &desired_key_attr, "desired_key"));

      struct attr desired_amount_attr =
          (struct attr){.type = TYPE_INT, .val.as_int = desired_amount};
      CHK_TRUE_RET(Attr_Write(stream, &desired_amount_attr, "desired_amount"));
    });

    struct attr num_alt_cap = (struct attr){
        .type = TYPE_INT, .val.as_int = kh_size(curr.alt_capacity)};
    ...
    kh_foreach(curr.alt_capacity, alt_cap_key, alt_cap_amount, {
      struct attr alt_cap_key_attr = (struct attr){
          .type = TYPE_STRING,
      };
      tc_strlcpy(alt_cap_key_attr.val.as_string, alt_cap_key,
                 sizeof(alt_cap_key_attr.val.as_string));
      CHK_TRUE_RET(Attr_Write(stream, &alt_cap_key_attr, "alt_cap_key"));

      struct attr alt_cap_amount_attr =
          (struct attr){.type = TYPE_INT, .val.as_int = alt_cap_amount};
      CHK_TRUE_RET(Attr_Write(stream, &alt_cap_amount_attr, "alt_cap_amount"));
    });

    struct attr num_alt_desired = (struct attr){
        .type = TYPE_INT, .val.as_int = kh_size(curr.alt_desired)};
    ...
    kh_foreach(curr.alt_desired, alt_desired_key, alt_desired_amount, {
      struct attr alt_desired_key_attr = (struct attr){
          .type = TYPE_STRING,
      };
      tc_strlcpy(alt_desired_key_attr.val.as_string, alt_desired_key,
                 sizeof(alt_desired_key_attr.val.as_string));
      CHK_TRUE_RET(
          Attr_Write(stream, &alt_desired_key_attr, "alt_desired_key"));

      struct attr alt_desired_amount_attr =
          (struct attr){.type = TYPE_INT, .val.as_int = alt_desired_amount};
      CHK_TRUE_RET(
          Attr_Write(stream, &alt_desired_amount_attr, "alt_desired_amount"));
    });

    struct attr do_not_take =
        (struct attr){.type = TYPE_BOOL, .val.as_bool = curr.do_not_take};
  ...
  /* save UI style
   */
  struct attr bg_style_type =
      (struct attr){.type = TYPE_INT, .val.as_int = s_bg_style.type};
  ...
  case NK_STYLE_ITEM_TEXPATH: {
    ...
    tc_strlcpy(bg_texpath.val.as_string, s_bg_style.data.texpath,
  ...
  struct attr ui_shown =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = s_show_ui};
...
bool G_StorageSite_LoadState(struct SDL_RWops *stream) {
  ...
  case NK_STYLE_ITEM_TEXPATH: {
    ...
    tc_strlcpy(s_bg_style.data.texpath, attr.val.as_string,
