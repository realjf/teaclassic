...
static bool region_intersects_chunk(const struct region *reg,
                                    struct map_resolution res,
                                    struct tile_desc td) {
  struct box chunk =
      M_Tile_ChunkBounds(res, M_GetPos(s_map), td.chunk_r, td.chunk_c);
  ...
  case REGION_RECTANGLE: {
    struct box bounds =
        (struct box){reg->pos.x + reg->xlen / 2.0, reg->pos.z - reg->zlen / 2.0,
                     reg->xlen, reg->zlen};
...
static bool compare_keys(const char **a, const char **b) { return *a == *b; }

static bool compare_uids(uint32_t *a, uint32_t *b) { return *a == *b; }
...
static void region_update_intersecting(const char *name,
                                       const struct region *reg, int op) {
  struct map_resolution res;
  M_GetResolution(s_map, &res);

  int delta = 0;
  int chunklen = MAX(X_COORDS_PER_TILE * res.chunk_w * res.tile_w,
                     Z_COORDS_PER_TILE * res.chunk_h * res.tile_h);
  ...
  case REGION_CIRCLE:
    ...
    break;
  case REGION_RECTANGLE:
    delta = MAX(ceil(reg->xlen / 2.0f / chunklen),
                ceil(reg->zlen / 2.0f / chunklen));
    break;
  default:
    assert(0);
  }
  ...
  for (int dr = -delta; dr <= delta; dr++) {
    for (int dc = -delta; dc <= delta; dc++) {
      ...
      vec_str_t *chunk =
          &s_intersecting[curr.chunk_r * res.chunk_w + curr.chunk_c];
...
static bool region_contains(const struct region *reg, vec2_t point) {
  ...
  case REGION_RECTANGLE: {
    ...
    return C_PointInsideRect2D(point, corners[0], corners[2], corners[1],
                               corners[3]);
  }
  default:
    return (assert(0), false);
...
static size_t regions_at_point(vec2_t point, size_t maxout,
                               struct region *out[static maxout],
...
static vec2_t region_ss_pos(vec2_t pos) {
  ...
  vec4_t clip, tmp;
  TCM_Mat4x4_Mult4x1(&view, &pos_homo, &tmp);
...
bool G_Region_AddCircle(const char *name, vec2_t pos, float radius) {
  struct region newreg = (struct region){
      .type = REGION_CIRCLE, .radius = radius, .shown = false, .pos = pos};
...
bool G_Region_AddRectangle(const char *name, vec2_t pos, float xlen,
                           float zlen) {
  struct region newreg = (struct region){.type = REGION_RECTANGLE,
                                         .xlen = xlen,
                                         .zlen = zlen,
                                         .shown = false,
                                         .pos = pos};
...
bool G_Region_SetPos(const char *name, vec2_t pos) {
  ...
  vec2_t delta;
  TCM_Vec2_Sub(&reg->pos, &pos, &delta);
...
void G_Region_SetRender(bool on) { s_render = on; }

bool G_Region_GetRender(void) { return s_render; }
...
bool G_Region_ExploreFog(const char *name, int faction_id) {
  ...
  switch (reg->type) {
  case REGION_RECTANGLE:
    G_Fog_ExploreRectangle(reg->pos, faction_id, reg->xlen / 2.0f,
                           reg->zlen / 2.0f);
    break;
  case REGION_CIRCLE:
...
bool G_Region_Explored(const char *name, uint16_t player_mask, bool *out) {
  ...
  case REGION_RECTANGLE:
    *out = G_Fog_CircleExplored(player_mask, reg->pos, reg->radius);
    break;
  case REGION_CIRCLE:
    *out = G_Fog_RectExplored(player_mask, reg->pos, reg->xlen / 2.0f,
                              reg->zlen / 2.0f);
    break;
  default:
    assert(0);
...
bool G_Region_SaveState(struct SDL_RWops *stream) {
  struct attr render =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = s_render};
  CHK_TRUE_RET(Attr_Write(stream, &render, "render"));

  struct attr num_regions =
      (struct attr){.type = TYPE_INT, .val.as_int = kh_size(s_regions)};
  ...
  kh_foreach(s_regions, name, curr, {
    struct attr reg_name = (struct attr){
        .type = TYPE_STRING,
    };
    tc_strlcpy(reg_name.val.as_string, name, sizeof(reg_name.val.as_string));
    CHK_TRUE_RET(Attr_Write(stream, &reg_name, "reg_name"));

    struct attr shown =
        (struct attr){.type = TYPE_BOOL, .val.as_bool = curr.shown};
    CHK_TRUE_RET(Attr_Write(stream, &shown, "shown"));

    struct attr pos = (struct attr){.type = TYPE_VEC2, .val.as_vec2 = curr.pos};
    CHK_TRUE_RET(Attr_Write(stream, &pos, "pos"));

    struct attr type = (struct attr){.type = TYPE_INT, .val.as_int = curr.type};
    ...
    case REGION_CIRCLE: {
      ...
      struct attr radius =
          (struct attr){.type = TYPE_FLOAT, .val.as_float = curr.radius};
    ...
    case REGION_RECTANGLE: {
      ...
      struct attr dims = (struct attr){
          .type = TYPE_VEC2, .val.as_vec2 = (vec2_t){curr.xlen, curr.zlen}};
  ...
  struct attr num_dirty =
      (struct attr){.type = TYPE_INT, .val.as_int = kh_size(s_dirty)};
...
bool G_Region_LoadState(struct SDL_RWops *stream) {
  ...
  for (int i = 0; i < num_regions; i++) {
    ...
    CHK_TRUE_RET(type.val.as_int == REGION_CIRCLE ||
                 type.val.as_int == REGION_RECTANGLE);
    ...
    case REGION_CIRCLE: {
      ...
      CHK_TRUE_RET(G_Region_AddCircle(name.val.as_string, pos.val.as_vec2,
                                      radius.val.as_float));
      break;
    }
    case REGION_RECTANGLE: {
