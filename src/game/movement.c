...
struct movestate {
   ...
   */
  vec2_t vdes;
  /* The newly computed velocity (the desired velocity constrained by flocking
   * forces)
   */
  vec2_t vnew;
   ...
   * 'target seek' field to get to path to our target. This kicks in once we
   * pass the distance 'low water' threshold and is turned off if we pass the
   * 'high water' threshold again - this is to prevent 'toggling' at a boundary
   * where we switch from one field to another.
   */
...
static bool flock_contains(const struct flock *flock,
...
static void entity_block(const struct entity *ent) {
  M_NavBlockersIncref(G_Pos_GetXZ(ent->uid), G_GetSelectionRadius(ent->uid),
...
static void vec2_truncate(vec2_t *inout, float max_len) {
  if (TCM_Vec2_Len(inout) > max_len) {

    TCM_Vec2_Normal(inout, inout);
    TCM_Vec2_Scale(inout, max_len, inout);
  }
}
...
static void entity_finish_moving(const struct entity *ent,
                                 enum arrival_state newstate) {
  struct movestate *ms = movestate_get(ent);
  assert(!ent_still(ms));

  E_Entity_Notify(EVENT_MOTION_END, ent->uid, NULL, ES_ENGINE);
  if (ent->flags & ENTITY_FLAG_COMBATABLE && (ms->state != STATE_TURNING)) {
...
static bool same_chunk_as_any_in_set(struct tile_desc desc,
                                     const struct tile_desc *set,
                                     size_t set_size) {
...
static void remove_from_flocks(const struct entity *ent) {
  /* Remove any flocks which may have become empty. Iterate vector in backwards
   * order so that we can delete while iterating, since the last element in the
   * vector takes the place of the deleted one.
   */
...
static void filter_selection_pathable(const vec_pentity_t *in_sel,
                                      vec_pentity_t *out_sel) {
...
static void
split_into_layers(const vec_pentity_t *sel,
                  vec_pentity_t layer_flocks[static NAV_LAYER_MAX]) {
...
static bool make_flock(const vec_pentity_t *units, vec2_t target_xz,
  ...
  /* The following won't be optimal when the entities in the unitsection are on
   * different 'islands'. Handling that case is not a top priority.
   */
  vec2_t first_ent_pos_xz = G_Pos_GetXZ(first->uid);
  target_xz =
      M_NavClosestReachableDest(s_map, layer, first_ent_pos_xz, target_xz);
  ...
  /* The flow fields will be computed on-demand during the next movement update
   * tick */
  new_flock.target_xz = target_xz;
  if (attack) {
    new_flock.dest_id = M_NavDestIDForPosAttacking(s_map, target_xz, layer,
                                                   G_GetFactionID(first->uid));
  } else {
  ...
  /* If there is another flock with the same dest_id, then we merge the two
   * flocks. */
  struct flock *merge_flock = flock_for_dest(new_flock.dest_id);
  if (merge_flock) {

    struct entity *curr;
    kh_foreach(new_flock.ents, (uint32_t){0}, curr,
               { flock_add(merge_flock, curr); });
    kh_destroy(entity, new_flock.ents);
...
static void make_flocks(const vec_pentity_t *sel, vec2_t target_xz,
                        bool attack) {
  vec_pentity_t fsel;
...
size_t adjacent_flock_members(const struct entity *ent,
                              const struct flock *flock, struct entity *out[]) {
  ...
  kh_foreach(flock->ents, key, curr, {
    ...
    TCM_Vec2_Sub(&ent_xz_pos, &curr_xz_pos, &diff);

    if (TCM_Vec2_Len(&diff) <= G_GetSelectionRadius(ent->uid) +
                                   G_GetSelectionRadius(curr->uid) +
                                   ADJACENCY_SEP_DIST)
      out[ret++] = curr;
  });
  return ret;
...
static void move_marker_add(vec3_t pos, bool attack) {
  ...
  struct entity *ent =
      attack ? AL_EntityFromTCObj("assets/models/arrow", "arrow-red.tcobj",
                                  "__move_marker__", uid)
             : AL_EntityFromTCObj("assets/models/arrow", "arrow-green.tcobj",
                                  "__move_marker__", uid);
  if (!ent)
    return;
...
static void on_mousedown(void *user, void *event) {
  ...
  for (int i = 0; i < vec_size(sel); i++) {
    ...
    if (curr->flags & ENTITY_FLAG_COMBATABLE) {
      G_Combat_SetStance(curr, attack ? COMBAT_STANCE_AGGRESSIVE
...
static void on_render_3d(void *user, void *event) {
  ...
  status = Settings_Get("tc.debug.navigation_layer", &setting);
  assert(status == SS_OKAY);
  layer = setting.as_int;

  status = Settings_Get("tc.debug.show_last_cmd_flow_field", &setting);
  assert(status == SS_OKAY);
  if (setting.as_bool && s_last_cmd_dest_valid) {
    M_NavRenderVisiblePathFlowField(s_map, cam, s_last_cmd_dest);
  }

  status = Settings_Get("tc.debug.show_first_sel_movestate", &setting);
  assert(status == SS_OKAY);
  ...
  if (setting.as_bool && vec_size(sel) > 0) {
    ...
    if (ms) {
      ...
      char strbuff[256];
      tc_snprintf(strbuff, ARR_SIZE(strbuff),
                  "Arrival State: %s Velocity: (%f, %f)",
      ...
      case STATE_SURROUND_ENTITY: {
        ...
        if (ms->using_surround_field) {
          M_NavRenderVisibleSurroundField(s_map, cam, Entity_NavLayer(ent),
                                          target);
          UI_DrawText("(Surround Field)", (struct rect){5, 75, 450, 50},
                      text_color);
        } else {
          M_NavRenderVisiblePathFlowField(s_map, cam, flock->dest_id);
          UI_DrawText("(Path Field)", (struct rect){5, 75, 450, 50},
                      text_color);
        }
        break;
      }
  ...
  status = Settings_Get("tc.debug.show_enemy_seek_fields", &setting);
  assert(status == SS_OKAY);
  if (setting.as_bool) {

    status = Settings_Get("tc.debug.enemy_seek_fields_faction_id", &setting);
    assert(status == SS_OKAY);

    M_NavRenderVisibleEnemySeekField(s_map, cam, layer, setting.as_int);
  }

  status = Settings_Get("tc.debug.show_navigation_blockers", &setting);
  assert(status == SS_OKAY);
  if (setting.as_bool) {
    M_NavRenderNavigationBlockers(s_map, cam, layer);
  }

  status = Settings_Get("tc.debug.show_navigation_portals", &setting);
  assert(status == SS_OKAY);
  if (setting.as_bool) {
    M_NavRenderNavigationPortals(s_map, cam, layer);
  }

  status = Settings_Get("tc.debug.show_navigation_cost_base", &setting);
  assert(status == SS_OKAY);
  if (setting.as_bool) {
    M_RenderVisiblePathableLayer(s_map, cam, layer);
  }

  status = Settings_Get("tc.debug.show_chunk_boundaries", &setting);
  assert(status == SS_OKAY);
  if (setting.as_bool) {
    M_RenderChunkBoundaries(s_map, cam);
  }

  status = Settings_Get("tc.debug.show_navigation_island_ids", &setting);
  assert(status == SS_OKAY);
  if (setting.as_bool) {
    M_NavRenderNavigationIslandIDs(s_map, cam, layer);
  }

  status = Settings_Get("tc.debug.show_navigation_local_island_ids", &setting);
  assert(status == SS_OKAY);
  if (setting.as_bool) {
...
static quat_t dir_quat_from_velocity(vec2_t velocity) {
  assert(TCM_Vec2_Len(&velocity) > EPSILON);

  float angle_rad = atan2(velocity.raw[1], velocity.raw[0]) - M_PI / 2.0f;
  return (quat_t){0.0f, 1.0f * sin(angle_rad / 2.0f), 0.0f,
                  cos(angle_rad / 2.0f)};
}
...
static vec2_t ent_desired_velocity(const struct entity *ent) {
  ...
  case STATE_SEEK_ENEMIES:
    return M_NavDesiredEnemySeekVelocity(s_map, Entity_NavLayer(ent), pos_xz,
                                         G_GetFactionID(ent->uid));
  ...
  case STATE_SURROUND_ENTITY: {
    ...
    if (!target) {
      return M_NavDesiredPointSeekVelocity(s_map, fl->dest_id, pos_xz,
                                           fl->target_xz);
    }
    ...
    if (ms->using_surround_field) {
      return M_NavDesiredSurroundVelocity(s_map, Entity_NavLayer(ent), pos_xz,
                                          target, G_GetFactionID(ent->uid));
    } else {
      return M_NavDesiredPointSeekVelocity(s_map, fl->dest_id, pos_xz,
                                           fl->target_xz);
    }
...
/* Seek behaviour makes the entity target and approach a particular destination
 * point.
 */
static vec2_t seek_force(const struct entity *ent, vec2_t target_xz) {
  ...
  TCM_Vec2_Sub(&target_xz, &pos_xz, &desired_velocity);
  TCM_Vec2_Normal(&desired_velocity, &desired_velocity);
  TCM_Vec2_Scale(&desired_velocity, ms->max_speed / MOVE_TICK_RES,
                 &desired_velocity);

  TCM_Vec2_Sub(&desired_velocity, &ms->velocity, &ret);
  return ret;
}

/* Arrival behaviour is like 'seek' but the entity decelerates and comes to a
 * halt when it is within a threshold radius of the destination point.
 *
 * When not within line of sight of the destination, this will steer the entity
...
static vec2_t arrive_force_point(const struct entity *ent, dest_id_t dest_id,
  ...
  if (M_NavHasDestLOS(s_map, dest_id, pos_xz)) {
    ...
    TCM_Vec2_Sub(&target_xz, &pos_xz, &desired_velocity);
    distance = TCM_Vec2_Len(&desired_velocity);
    TCM_Vec2_Normal(&desired_velocity, &desired_velocity);
    TCM_Vec2_Scale(&desired_velocity, ms->max_speed / MOVE_TICK_RES,
                   &desired_velocity);

    if (distance < ARRIVE_SLOWING_RADIUS) {
      TCM_Vec2_Scale(&desired_velocity, distance / ARRIVE_SLOWING_RADIUS,
                     &desired_velocity);
    }

  } else {

    TCM_Vec2_Scale(&ms->vdes, ms->max_speed / MOVE_TICK_RES, &desired_velocity);
  }
...
/* Alignment is a behaviour that causes a particular agent to line up with
 * agents close by.
 */
static vec2_t alignment_force(const struct entity *ent,
  ...
  kh_foreach(flock->ents, key, curr, {
    ...
    TCM_Vec2_Sub(&curr_xz_pos, &ent_xz_pos, &diff);
    if (TCM_Vec2_Len(&diff) < ALIGN_NEIGHBOUR_RADIUS) {

      struct movestate *ms = movestate_get(ent);
      assert(ms);

      if (TCM_Vec2_Len(&ms->velocity) < EPSILON)
        continue;

      TCM_Vec2_Add(&ret, &ms->velocity, &ret);
      neighbour_count++;
    }
  });
  ...
  TCM_Vec2_Scale(&ret, 1.0f / neighbour_count, &ret);
  TCM_Vec2_Sub(&ret, &ms->velocity, &ret);
  vec2_truncate(&ret, MAX_FORCE);
  return ret;
}

/* Cohesion is a behaviour that causes agents to steer towards the center of
 * mass of nearby agents.
 */
static vec2_t cohesion_force(const struct entity *ent,
                             const struct flock *flock) {
  ...
  kh_foreach(flock->ents, key, curr, {
    ...
    float t = (TCM_Vec2_Len(&diff) - COHESION_NEIGHBOUR_RADIUS * 0.75) /
              COHESION_NEIGHBOUR_RADIUS;
    float scale = exp(-6.0f * t);

    TCM_Vec2_Scale(&curr_xz_pos, scale, &curr_xz_pos);
    TCM_Vec2_Add(&COM, &curr_xz_pos, &COM);
    neighbour_count++;
  });
  ...
  vec2_t ret;
  TCM_Vec2_Scale(&COM, 1.0f / neighbour_count, &COM);
  TCM_Vec2_Sub(&COM, &ent_xz_pos, &ret);
  vec2_truncate(&ret, MAX_FORCE);
  return ret;
}

/* Separation is a behaviour that causes agents to steer away from nearby
 * agents.
 */
static vec2_t separation_force(const struct entity *ent, float buffer_dist) {
  ...
  int num_near =
      G_Pos_EntsInCircle(G_Pos_GetXZ(ent->uid), SEPARATION_NEIGHB_RADIUS,
  ...
  for (int i = 0; i < num_near; i++) {
    ...
    float radius = G_GetSelectionRadius(ent->uid) +
                   G_GetSelectionRadius(curr->uid) + buffer_dist;
    TCM_Vec2_Sub(&curr_xz_pos, &ent_xz_pos, &diff);
     ...
     */
    float t = (TCM_Vec2_Len(&diff) - radius * 0.85) / TCM_Vec2_Len(&diff);
    float scale = exp(-20.0f * t);
    TCM_Vec2_Scale(&diff, scale, &diff);

    TCM_Vec2_Add(&ret, &diff, &ret);
  }

  if (0 == num_near)
    return (vec2_t){0.0f};

  TCM_Vec2_Scale(&ret, -1.0f, &ret);
  vec2_truncate(&ret, MAX_FORCE);
  return ret;
}

static vec2_t point_seek_total_force(const struct entity *ent,
                                     const struct flock *flock) {
  struct movestate *ms = movestate_get(ent);
  assert(ms);
  ...
  TCM_Vec2_Scale(&arrive, MOVE_ARRIVE_FORCE_SCALE, &arrive);
  TCM_Vec2_Scale(&cohesion, MOVE_COHESION_FORCE_SCALE, &cohesion);
  TCM_Vec2_Scale(&separation, SEPARATION_FORCE_SCALE, &separation);

  vec2_t ret = (vec2_t){0.0f};
  assert(!ent_still(ms));

  TCM_Vec2_Add(&ret, &arrive, &ret);
  TCM_Vec2_Add(&ret, &separation, &ret);
  TCM_Vec2_Add(&ret, &cohesion, &ret);
...
static vec2_t new_pos_for_vel(const struct entity *ent, vec2_t velocity) {
  ...
  TCM_Vec2_Add(&xz_pos, &velocity, &new_pos);
  return new_pos;
}

/* Nullify the components of the force which would guide
 * the entity towards an impassable tile. */
static void nullify_impass_components(const struct entity *ent,
                                      vec2_t *inout_force) {
  vec2_t nt_dims = N_TileDims();
  enum nav_layer layer = Entity_NavLayer(ent);

  vec2_t left =
      (vec2_t){G_Pos_Get(ent->uid).x + nt_dims.x, G_Pos_Get(ent->uid).z};
  vec2_t right =
      (vec2_t){G_Pos_Get(ent->uid).x - nt_dims.x, G_Pos_Get(ent->uid).z};
  vec2_t top =
      (vec2_t){G_Pos_Get(ent->uid).x, G_Pos_Get(ent->uid).z + nt_dims.z};
  vec2_t bot =
      (vec2_t){G_Pos_Get(ent->uid).x, G_Pos_Get(ent->uid).z - nt_dims.z};

  if (inout_force->x > 0 && (!M_NavPositionPathable(s_map, layer, left) ||
                             M_NavPositionBlocked(s_map, layer, left)))
    inout_force->x = 0.0f;

  if (inout_force->x < 0 && (!M_NavPositionPathable(s_map, layer, right) ||
                             M_NavPositionBlocked(s_map, layer, right)))
    inout_force->x = 0.0f;

  if (inout_force->z > 0 && (!M_NavPositionPathable(s_map, layer, top) ||
                             M_NavPositionBlocked(s_map, layer, top)))
    inout_force->z = 0.0f;

  if (inout_force->z < 0 && (!M_NavPositionPathable(s_map, layer, bot) ||
                             M_NavPositionBlocked(s_map, layer, bot)))
    inout_force->z = 0.0f;
}

static vec2_t point_seek_vpref(const struct entity *ent,
                               const struct flock *flock) {
  struct movestate *ms = movestate_get(ent);
  assert(ms);
  ...
  for (int prio = 0; prio < 3; prio++) {
    ...
    switch (prio) {
    case 0:
      steer_force = point_seek_total_force(ent, flock);
      break;
    case 1:
      steer_force = separation_force(ent, SEPARATION_BUFFER_DIST);
      break;
    case 2:
      steer_force = arrive_force_point(ent, flock->dest_id, flock->target_xz);
      break;
    }

    nullify_impass_components(ent, &steer_force);
    if (TCM_Vec2_Len(&steer_force) > MAX_FORCE * 0.01)
      break;
  }

  vec2_t accel, new_vel;
  TCM_Vec2_Scale(&steer_force, 1.0f / ENTITY_MASS, &accel);

  TCM_Vec2_Add(&ms->velocity, &accel, &new_vel);
...
static vec2_t vel_sma(const struct movestate *ms) {
  ...
  for (int i = 0; i < VEL_HIST_LEN; i++)
    TCM_Vec2_Add(&ret, (vec2_t *)&ms->vel_hist[i], &ret);
  TCM_Vec2_Scale(&ret, 1.0f / VEL_HIST_LEN, &ret);
  return ret;
}
...
static vec2_t vel_wma(const struct movestate *ms) {
  ...
  for (int i = 0; i < VEL_HIST_LEN; i++) {
    ...
    TCM_Vec2_Scale(&term, VEL_HIST_LEN - i, &term);
    TCM_Vec2_Add(&ret, &term, &ret);
    denom += (VEL_HIST_LEN - i);
  }

  TCM_Vec2_Scale(&ret, 1.0f / denom, &ret);
  return ret;
}
...
static void entity_update(struct entity *ent, vec2_t new_vel) {
  ...
  if (TCM_Vec2_Len(&new_vel) > 0 &&
      M_NavPositionPathable(s_map, layer, new_pos_xz) &&
      !M_NavPositionBlocked(s_map, layer, new_pos_xz)) {

    vec3_t new_pos = (vec3_t){new_pos_xz.x, M_HeightAtPoint(s_map, new_pos_xz),
                              new_pos_xz.z};
    G_Pos_Set(ent, new_pos);
    ms->velocity = new_vel;

    /* Use a weighted average of past velocities ot set the entity's
     * orientation. This means that the entity's visible orientation lags behind
     * its' true orientation slightly. However, this greatly smooths the turning
     * of the entity, giving a more natural look to the movemment.
     */
    vec2_t wma = vel_wma(ms);
    if (TCM_Vec2_Len(&wma) > EPSILON) {
  ...
  /* If the entity's current position isn't pathable, simply keep it 'stuck'
   * there in the same state it was in before. Under normal conditions, no
   * entity can move from pathable terrain to non-pathable terrain, but an this
   * violation is possible by forcefully setting the entity's position from a
   * scripting call.
   */
  ...
  case STATE_MOVING: {
    ...
    TCM_Vec2_Sub((vec2_t *)&flock->target_xz, &xz_pos, &diff_to_target);
    float arrive_thresh = G_GetSelectionRadius(ent->uid) * 1.5f;

    if (TCM_Vec2_Len(&diff_to_target) < arrive_thresh ||
        (M_NavIsAdjacentToImpassable(s_map, layer, xz_pos) &&
         M_NavIsMaximallyClose(s_map, layer, xz_pos, flock->target_xz,
                               arrive_thresh))) {
     ...
     * after some time.
     */
    if (TCM_Vec2_Len(&ms->vdes) < EPSILON) {
  ...
  case STATE_SURROUND_ENTITY: {
    ...
    vec2_t delta;
    TCM_Vec2_Sub(&target_pos, &ms->surround_target_prev, &delta);
    if (TCM_Vec2_Len(&delta) > EPSILON ||
        TCM_Vec2_Len(&ms->velocity) < EPSILON) {

      bool hasdest = M_NavClosestReachableAdjacentPos(
          s_map, layer, G_Pos_GetXZ(ent->uid), target, &dest);
    ...
    vec2_t diff;
    TCM_Vec2_Sub(&flock->target_xz, &dest, &diff);
    ms->surround_target_prev = target_pos;
    ms->surround_nearest_prev = dest;

    if (TCM_Vec2_Len(&diff) > EPSILON) {
  ...
  case STATE_ENTER_ENTITY_RANGE: {
    ...
    vec2_t delta;
    TCM_Vec2_Sub(&xz_pos, &xz_target, &delta);

    if (TCM_Vec2_Len(&delta) <= ms->target_range ||
        (M_NavIsAdjacentToImpassable(s_map, layer, xz_pos) &&
    ...
    vec2_t target_delta;
    TCM_Vec2_Sub(&xz_target, &ms->target_prev_pos, &target_delta);

    if (TCM_Vec2_Len(&target_delta) > 5.0f) {
  ...
  case STATE_TURNING: {
    ...
    float angle_diff = TCM_Quat_PitchDiff(&ent_rot, &ms->target_dir);
    ...
    mat4x4_t rotmat;
    TCM_Mat4x4_MakeRotY(turn_rad, &rotmat);
    quat_t rot;
    TCM_Quat_FromRotMat(&rotmat, &rot);

    /* Turn */
    quat_t final;
    TCM_Quat_MultQuat(&rot, &ent_rot, &final);
    TCM_Quat_Normal(&final, &final);
...
static void find_neighbours(const struct entity *ent, vec_cp_ent_t *out_dyn,
  ...
  int num_near =
      G_Pos_EntsInCircle(G_Pos_GetXZ(ent->uid), CLEARPATH_NEIGHBOUR_RADIUS,
                         near_ents, ARR_SIZE(near_ents));
  ...
  for (int i = 0; i < num_near; i++) {
    ...
    vec2_t curr_xz_pos = G_Pos_GetXZ(curr->uid);
    struct cp_ent newdesc =
        (struct cp_ent){.xz_pos = curr_xz_pos,
                        .xz_vel = ms->velocity,
...
static void cp_vec_free(void *ptr) { /* no-op */ }
...
static void clearpath_work(int begin_idx, int end_idx) {
  for (int i = begin_idx; i <= end_idx; i++) {
    ...
    vec2_t new_vel = G_ClearPath_NewVelocity(in->ent, in->ent_uid,
                                             in->ent_des_v, in->dyn_neighbs,
                                             in->stat_neighbs, in->save_debug);
...
static void clearpath_finish_work(void) {
  ...
  for (int i = 0; i < s_cp_work.nwork; i++) {
    ...
    vec2_t vel_diff;
    TCM_Vec2_Sub(&ms->vnew, &ms->velocity, &vel_diff);

    TCM_Vec2_Add(&ms->velocity, &vel_diff, &ms->vnew);
...
static void clearpath_prepare_work(void) {
  ...
  s_cp_work.out =
      stalloc(&s_cp_work.mem, ndynamic * sizeof(struct cp_work_out));
}
...
static void clearpath_submit_work(void) {
  ...
  for (int i = 0; i < ntasks; i++) {
    ...
    struct cp_task_arg *arg =
        stalloc(&s_cp_work.mem, sizeof(struct cp_task_arg));
    size_t nitems = ceil((float)s_cp_work.nwork / ntasks);

    arg->begin_idx = nitems * i;
    arg->end_idx = MIN(nitems * (i + 1) - 1, s_cp_work.nwork - 1);

    SDL_AtomicSet(&s_cp_work.futures[s_cp_work.ntasks].status,
                  FUTURE_INCOMPLETE);
    s_cp_work.tids[s_cp_work.ntasks] =
        Sched_Create(4, clearpath_task, arg,
...
static void on_20hz_tick(void *user, void *event) {
  ...
  kh_foreach(G_GetDynamicEntsSet(), key, curr, {
    ...
    struct cp_ent curr_cp =
        (struct cp_ent){.xz_pos = G_Pos_GetXZ(curr->uid),
                        .xz_vel = ms->velocity,
                        .radius = G_GetSelectionRadius(curr->uid)};
...
void G_Move_Shutdown(void) {
  ...
  for (int i = 0; i < vec_size(&s_move_markers); i++) {
    E_Entity_Unregister(EVENT_ANIM_FINISHED, vec_AT(&s_move_markers, i)->uid,
                        on_marker_anim_finish);
...
void G_Move_SetDest(const struct entity *ent, vec2_t dest_xz, bool attack) {
  enum nav_layer layer = Entity_NavLayer(ent);
  dest_xz =
      M_NavClosestReachableDest(s_map, layer, G_Pos_GetXZ(ent->uid), dest_xz);
  ...
  dest_id_t dest_id;
  if (attack) {
    dest_id = M_NavDestIDForPosAttacking(s_map, dest_xz, layer,
                                         G_GetFactionID(ent->uid));
  } else {
...
void G_Move_SetEnterRange(const struct entity *ent, const struct entity *target,
  ...
  vec2_t delta;
  TCM_Vec2_Sub(&xz_src, &xz_dst, &delta);
  if (TCM_Vec2_Len(&delta) <= range) {
    G_Move_Stop(ent);
    return;
  }

  vec2_t xz_target = M_NavClosestReachableInRange(s_map, Entity_NavLayer(ent),
                                                  xz_src, xz_dst, range);
  G_Move_SetDest(ent, xz_target, false);
...
void G_Move_UpdatePos(const struct entity *ent, vec2_t pos) {
  ...
  M_NavBlockersDecref(ms->last_stop_pos, ms->last_stop_radius,
                      G_GetFactionID(ent->uid), s_map);
  M_NavBlockersIncref(pos, ms->last_stop_radius, G_GetFactionID(ent->uid),
                      s_map);
...
void G_Move_UpdateSelectionRadius(const struct entity *ent, float sel_radius) {
  ...
  M_NavBlockersDecref(ms->last_stop_pos, ms->last_stop_radius,
                      G_GetFactionID(ent->uid), s_map);
  M_NavBlockersIncref(ms->last_stop_pos, sel_radius, G_GetFactionID(ent->uid),
                      s_map);
...
void G_Move_SetClickEnabled(bool on) { s_click_move_enabled = on; }

bool G_Move_GetClickEnabled(void) { return s_click_move_enabled; }
...
bool G_Move_SaveState(struct SDL_RWops *stream) {
  struct attr click_move_enabled =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = s_click_move_enabled};
  CHK_TRUE_RET(Attr_Write(stream, &click_move_enabled, "click_move_enabled"));

  /* save flock info */
  struct attr num_flocks =
      (struct attr){.type = TYPE_INT, .val.as_int = vec_size(&s_flocks)};
  ...
  for (int i = 0; i < vec_size(&s_flocks); i++) {
    ...
    struct attr num_flock_ents = (struct attr){
        .type = TYPE_INT, .val.as_int = kh_size(curr_flock->ents)};
    ...
    kh_foreach(curr_flock->ents, uid, curr_ent, {
      struct attr flock_ent =
          (struct attr){.type = TYPE_INT, .val.as_int = uid};
      CHK_TRUE_RET(Attr_Write(stream, &flock_ent, "flock_ent"));
    });

    struct attr flock_target =
        (struct attr){.type = TYPE_VEC2, .val.as_vec2 = curr_flock->target_xz};
    CHK_TRUE_RET(Attr_Write(stream, &flock_target, "flock_target"));

    struct attr flock_dest =
        (struct attr){.type = TYPE_INT, .val.as_int = curr_flock->dest_id};
  ...
  kh_foreach(s_entity_state_table, key, curr, {
    ...
    struct attr max_speed =
        (struct attr){.type = TYPE_FLOAT, .val.as_float = curr.max_speed};
    CHK_TRUE_RET(Attr_Write(stream, &max_speed, "max_speed"));

    struct attr vdes =
        (struct attr){.type = TYPE_VEC2, .val.as_vec2 = curr.vdes};
    CHK_TRUE_RET(Attr_Write(stream, &vdes, "vdes"));

    struct attr velocity =
        (struct attr){.type = TYPE_VEC2, .val.as_vec2 = curr.velocity};
    CHK_TRUE_RET(Attr_Write(stream, &velocity, "velocity"));

    struct attr blocking =
        (struct attr){.type = TYPE_BOOL, .val.as_bool = curr.blocking};
    ...
    struct attr wait_prev =
        (struct attr){.type = TYPE_INT, .val.as_int = curr.wait_prev};
    CHK_TRUE_RET(Attr_Write(stream, &wait_prev, "wait_prev"));

    struct attr wait_ticks_left =
        (struct attr){.type = TYPE_INT, .val.as_int = curr.wait_ticks_left};
    CHK_TRUE_RET(Attr_Write(stream, &wait_ticks_left, "wait_ticks_left"));

    for (int i = 0; i < VEL_HIST_LEN; i++) {

      struct attr hist_entry =
          (struct attr){.type = TYPE_VEC2, .val.as_vec2 = curr.vel_hist[i]};
      CHK_TRUE_RET(Attr_Write(stream, &hist_entry, "hist_entry"));
    }

    struct attr vel_hist_idx =
        (struct attr){.type = TYPE_INT, .val.as_int = curr.vel_hist_idx};
    CHK_TRUE_RET(Attr_Write(stream, &vel_hist_idx, "vel_hist_idx"));

    struct attr surround_target_uid =
        (struct attr){.type = TYPE_INT, .val.as_int = curr.surround_target_uid};
    CHK_TRUE_RET(
        Attr_Write(stream, &surround_target_uid, "surround_target_uid"));

    struct attr surround_target_prev = (struct attr){
        .type = TYPE_VEC2, .val.as_vec2 = curr.surround_target_prev};
    CHK_TRUE_RET(
        Attr_Write(stream, &surround_target_prev, "surround_target_prev"));

    struct attr surround_nearest_prev = (struct attr){
        .type = TYPE_VEC2, .val.as_vec2 = curr.surround_nearest_prev};
    CHK_TRUE_RET(
        Attr_Write(stream, &surround_nearest_prev, "surround_nearest_prev"));

    struct attr using_surround_field = (struct attr){
        .type = TYPE_BOOL, .val.as_bool = curr.using_surround_field};
    CHK_TRUE_RET(
        Attr_Write(stream, &using_surround_field, "using_surround_field"));

    struct attr target_prev_pos =
        (struct attr){.type = TYPE_VEC2, .val.as_vec2 = curr.target_prev_pos};
    CHK_TRUE_RET(Attr_Write(stream, &target_prev_pos, "target_prev_pos"));

    struct attr target_range =
        (struct attr){.type = TYPE_FLOAT, .val.as_float = curr.target_range};
    CHK_TRUE_RET(Attr_Write(stream, &target_range, "target_range"));

    struct attr target_dir =
        (struct attr){.type = TYPE_QUAT, .val.as_quat = curr.target_dir};
