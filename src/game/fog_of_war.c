...
/* Holds a 32-bit value for every tile of the map. The chunks are stored in
 * row-major order. Within a chunk, the tiles are in row-major order. Each
 * 32-bit value encodes a 2-bit faction state for up to 16 factions. */
...
/* Cache all the entities that have been explored by the player, for faster
 * queries */
...
static void fog_set_state(uint32_t *inout_tileval, int faction_id,
                          enum fog_state state) {
...
static int td_index(struct tile_desc td) {
  ...
  return td.chunk_r * (res.chunk_w * tiles_per_chunk) +
         td.chunk_c * tiles_per_chunk + (td.tile_r * res.tile_w + td.tile_c);
}
...
static vec2_t tile_center_pos(struct tile_desc td) {
  ...
  return (vec2_t){box.x - box.width / 2.0f, box.z + box.height / 2.0f};
}
...
static void wf_create_blocked_line(int xrad, int zrad, bool wf[][xrad * 2 + 1],
                                   struct tile_desc origin, int delta_r,
                                   int delta_c) {
  struct map_resolution res;
  M_GetResolution(s_map, &res);
  ...
  vec2_t slope;
  TCM_Vec2_Sub(&origin_center, &corner_center, &slope);
  TCM_Vec2_Normal(&slope, &slope);
...
void td_delta(struct tile_desc a, struct tile_desc b, int *out_dr,
              int *out_dc) {
  struct map_resolution res;
  M_GetResolution(s_map, &res);
...
static void fog_update_visible(int faction_id, vec2_t xz_pos, float radius,
                               int delta) {
  if (radius == 0.0f)
    return;
  ...
  while (pq_size(&frontier) > 0) {
    ...
    for (int i = 0; i < num_neighbs; i++) {
      ...
      vec2_t origin_delta;
      TCM_Vec2_Sub(&neighb_pos, &origin_pos, &origin_delta);

      if (TCM_Vec2_Len(&origin_delta) > radius)
        continue;

      if (td_is_los_corner(neighbs[i], origin_height))
        wf_create_blocked_line(tile_x_radius, tile_z_radius, wf_blocked, origin,
                               dr, dc);
      ...
      pq_td_push(&frontier, TCM_Vec2_Len(&origin_delta), neighbs[i]);
    }
  }
...
static bool fog_obj_matches(uint16_t fac_mask, const struct obb *obj,
...
static bool fog_circle_matches(uint16_t fac_mask, vec2_t xz_center,
                               float radius, enum fog_state *states,
                               size_t nstates) {
  assert(Sched_UsingBigStack());
  ...
  struct tile_desc tds[4096];
  size_t ntiles = M_Tile_AllUnderCircle(res, xz_center, radius, M_GetPos(s_map),
                                        tds, ARR_SIZE(tds));
...
static bool fog_rect_matches(uint16_t fac_mask, vec2_t xz_center, float halfx,
                             float halfz, enum fog_state *states,
                             size_t nstates) {
  assert(Sched_UsingBigStack());
...
static void on_render_3d(void *user, void *event) {
  ...
  status = Settings_Get("tc.debug.show_faction_vision", &setting);
...
void G_Fog_ExploreCircle(vec2_t xz_pos, int faction_id, float radius) {
  ...
  struct tile_desc tds[4096];
  size_t ntiles = M_Tile_AllUnderCircle(res, xz_pos, radius, M_GetPos(s_map),
                                        tds, ARR_SIZE(tds));
...
void G_Fog_ExploreRectangle(vec2_t xz_pos, int faction_id, float halfx,
  ...
  struct tile_desc tds[4096];
  size_t ntiles = M_Tile_AllUnderAABB(res, xz_pos, halfx, halfz,
                                      M_GetPos(s_map), tds, ARR_SIZE(tds));
...
void G_Fog_RenderChunkVisibility(int faction_id, int chunk_r, int chunk_c,
  ...
  for (int r = 0; r < res.tile_h; r++) {
    for (int c = 0; c < res.tile_w; c++) {
      ...
      *corners_base++ =
          (vec2_t){square_x - square_x_len, square_z + square_z_len};
...
bool G_Fog_RectExplored(uint16_t fac_mask, vec2_t xz_pos, float halfx,
  ...
  enum fog_state states[] = {STATE_IN_FOG, STATE_VISIBLE};
  return fog_rect_matches(fac_mask, xz_pos, halfx, halfz, states,
                          ARR_SIZE(states));
}
...
void G_Fog_ClearExploredCache(void) { kh_clear(uid, s_explored_cache); }
...
bool G_Fog_SaveState(struct SDL_RWops *stream) {
  ...
  struct attr enabled =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = s_enabled};
  CHK_TRUE_RET(Attr_Write(stream, &enabled, "enabled"));

  struct attr ntiles_attr =
      (struct attr){.type = TYPE_INT, .val.as_int = ntiles};
  ...
  for (int i = 0; i < ntiles; i++) {
    ...
    struct attr tilestate = (struct attr){.type = TYPE_INT, .val.as_int = fs};
    ...
    for (int chunk_r = 0; chunk_r < res.chunk_h; chunk_r++) {
      ...
      for (int tile_c = 0; tile_c < res.tile_w; tile_c++) {
        for (int tile_r = 0; tile_r < res.tile_h; tile_r++) {
          ...
          struct tile_desc td =
              (struct tile_desc){chunk_r, chunk_c, tile_r, tile_c};
...
void G_Fog_Enable(void) { s_enabled = true; }

void G_Fog_Disable(void) { s_enabled = false; }

bool G_Fog_Enabled(void) { return s_enabled; }
