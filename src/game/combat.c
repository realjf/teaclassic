 ...
 *                 |    |(enter attack range)   |(leave attack range)  |(target
 *dies) |    |*attack begins*        |*attack ends*         |*attack ends* | |
 *+--------+----------------------+ |    V              |
 ...
 *|(attack time elapsed) (not animated)|  [STATE_ATTACK_ANIM_PLAYING]-+ | | |
 *                 +->[STATE_ATTACKING]----------------------------------+
 *
...
struct combatstats {
  ...
  float base_armour_pc; /* Percentage of damage blocked. Valid range: [0.0
                           - 1.0] */
  float attack_range;
};
...
static void combatstate_set(const struct entity *ent,
                            const struct combatstate *cs) {
...
static void combat_dying_remove(const struct entity *ent) {
  int idx =
      vec_pentity_indexof(&s_dying_ents, (struct entity *)ent, pentities_equal);
  if (idx == -1)
    return;
...
static struct proj_desc combat_default_proj(void) {
  return (struct proj_desc){
      .basedir = tc_strdup("assets/models/bow_arrow"),
      .tcobj = tc_strdup("arrow.tcobj"),
...
static bool enemies(const struct entity *a, const struct entity *b) {
  ...
  bool result =
      G_GetDiplomacyState(G_GetFactionID(a->uid), G_GetFactionID(b->uid), &ds);

  assert(result);
  return (ds == DIPLOMACY_STATE_WAR);
}

static bool enemies_in_bin(int faction_id, struct map_resolution binres,
                           struct tile_desc td) {
...
static bool maybe_enemy_near(const struct entity *ent) {
  ...
  struct map_resolution binres = (struct map_resolution){
      mapres.chunk_w, mapres.chunk_h, X_BINS_PER_CHUNK, Z_BINS_PER_CHUNK};
...
static void entity_move_in_range(const struct entity *ent,
...
static bool entity_can_attack_melee(const struct entity *ent,
                                    const struct entity *target) {
...
static bool entity_can_attack(const struct entity *ent,
                              const struct entity *target) {
  const struct combatstate *cs = combatstate_get(ent->uid);
  if (cs->stats.attack_range == 0.0f) {
  ...
  return (TCM_Vec2_Len(&delta) <= cs->stats.attack_range);
}
...
static quat_t quat_from_vec(vec2_t dir) {
  assert(TCM_Vec2_Len(&dir) > EPSILON);
...
static quat_t entity_turn_dir(struct entity *ent, const struct entity *target) {
  ...
  TCM_Vec2_Sub(&tar_pos_xz, &ent_pos_xz, &ent_to_target);

  if (TCM_Vec2_Len(&ent_to_target) < EPSILON) {
    return Entity_GetRot(ent->uid);
  }

  TCM_Vec2_Normal(&ent_to_target, &ent_to_target);
  quat_t curr = Entity_GetRot(ent->uid);
  return quat_from_vec(ent_to_target);
}

static void entity_turn_to_target(struct entity *ent,
                                  const struct entity *target) {
...
static void entity_die(struct entity *ent) {
  ...
  if (ent->flags & ENTITY_FLAG_ANIMATED &&
    ...
    E_Entity_Register(EVENT_ANIM_CYCLE_FINISHED, ent->uid, on_death_anim_finish,
                      ent, G_RUNNING);
...
static void entity_melee_attack(const struct entity *self,
                                struct entity *target) {
  struct combatstate *cs = combatstate_get(self->uid);
  struct combatstate *target_cs = combatstate_get(cs->target_uid);

  float dmg =
      G_Combat_GetBaseDamage(self) * (1.0f - G_Combat_GetBaseArmour(target));
...
static void entity_ranged_attack(const struct entity *self,
                                 struct entity *target) {
  ...
  vec3_t vel;
  if (!P_Projectile_VelocityForTarget(ent_pos, target_pos, cs->pd.speed,
                                      &vel)) {
    /* We resort to just shooting nothing when we can't hit our target. This
     * case should never be hit so long as the initial velocity is high enough
     */
    return;
  }

  P_Projectile_Add(ent_pos, vel, self->uid, G_GetFactionID(self->uid), ent_dmg,
                   PROJ_ONLY_HIT_COMBATABLE | PROJ_ONLY_HIT_ENEMIES, cs->pd);
}

static void on_death_anim_finish(void *user, void *event) {
  struct entity *self = user;
  assert(self);
  E_Entity_Unregister(EVENT_ANIM_CYCLE_FINISHED, self->uid,
                      on_death_anim_finish);
...
static void entity_combat_action(struct entity *ent) {
  ...
  float angle_diff = TCM_Quat_PitchDiff(&ent_rot, &target_dir);
...
static void entity_target_enemy(struct entity *ent,
                                const struct entity *enemy) {
  struct combatstate *cs = combatstate_get(ent->uid);
  assert(cs);
  ...
  if (cs->stance == COMBAT_STANCE_AGGRESSIVE &&
...
static void on_20hz_tick(void *user, void *event) {
  ...
  kh_foreach(s_entity_state_table, key, (struct combatstate){0}, {
    ...
    case STATE_CAN_ATTACK: {
      ...
      /* Our target could have 'died' or gotten out of combat range - check this
       * first. */
      if (entity_dead(target) || !entity_can_attack(ent, target)) {
        ...
        if (curr->stance == COMBAT_STANCE_HOLD_POSITION &&
            !entity_can_attack(ent, enemy)) {
          E_Entity_Notify(EVENT_ATTACK_END, ent->uid, NULL, ES_ENGINE);
          entity_stop_combat(ent);
          break;
        }
      ...
      if (ent->flags & ENTITY_FLAG_ANIMATED) {
        ...
        E_Entity_Register(EVENT_ANIM_CYCLE_FINISHED, ent->uid,
                          on_attack_anim_finish, ent, G_RUNNING);
      } else {
...
static void combat_render_targets(void) {
  ...
  kh_foreach(s_entity_state_table, key, curr, {
    ...
    case STATE_CAN_ATTACK: {
      ...
      TCM_Vec2_Sub(&target_pos, &ent_pos, &delta);

      float t = TCM_Vec2_Len(&delta);
      TCM_Vec2_Normal(&delta, &delta);
      vec3_t dir = (vec3_t){delta.x, 0.0f, delta.z};

      vec3_t raised_pos = (vec3_t){
          ent_pos.x,
          M_HeightAtPoint(G_GetPrevTickMap(), (vec2_t){ent_pos.x, ent_pos.z}) +
              5.0f,
          ent_pos.z};
...
static void on_render_3d(void *user, void *event) {
  ...
  status = Settings_Get("tc.debug.show_combat_targets", &setting);
  ...
  status = Settings_Get("tc.debug.show_combat_ranges", &setting);
  assert(status == SS_OKAY);
...
bool G_Combat_Init(const struct map *map) {
  ...
  for (int i = 0; i < MAX_FACTIONS; i++) {
    s_fac_refcnts[i] =
        calloc((res.chunk_w * X_BINS_PER_CHUNK) *
                   (res.chunk_h * Z_BINS_PER_CHUNK) * sizeof(uint16_t),
               1);
    if (!s_fac_refcnts[i])
      goto fail_refcnts;
  }
...
void G_Combat_RemoveEntity(const struct entity *ent) {
  ...
  if (cs->state == STATE_ATTACK_ANIM_PLAYING || cs->state == STATE_CAN_ATTACK) {
    E_Entity_Notify(EVENT_ATTACK_END, ent->uid, NULL, ES_ENGINE);
  }

  TC_FREE(cs->pd.basedir);
  TC_FREE(cs->pd.tcobj);
...
int G_Combat_CurrContextualAction(void) {
  ...
  if ((hovered->flags & ENTITY_FLAG_MARKER) ||
      (hovered->flags & ENTITY_FLAG_ZOMBIE))
    return CTX_ACTION_NONE;

  bool can_target = (hovered->flags & ENTITY_FLAG_MOVABLE) &&
...
void G_Combat_AttackUnit(const struct entity *ent,
                         const struct entity *target) {
  struct combatstate *cs = combatstate_get(ent->uid);
  assert(cs);
...
struct entity *G_Combat_ClosestEligibleEnemy(const struct entity *ent) {
  ...
  size_t nents =
      G_Pos_EntsInCircleWithPred(pos, TARGET_ACQUISITION_RANGE, ents,
  ...
  for (int i = 0; i < nents; i++) {
    ...
    vec2_t delta;
    TCM_Vec2_Sub(&pos, &enemy_pos, &delta);
    float dist = TCM_Vec2_Len(&delta);
...
void G_Combat_SetProjDesc(const struct entity *ent,
                          const struct proj_desc *pd) {
  struct combatstate *cs = combatstate_get(ent->uid);
  assert(cs);
...
bool G_Combat_SaveState(struct SDL_RWops *stream) {
  ...
  kh_foreach(s_entity_state_table, key, curr, {
    ...
    struct attr stance =
        (struct attr){.type = TYPE_INT, .val.as_int = curr.stance};
    ...
    struct attr sticky =
        (struct attr){.type = TYPE_BOOL, .val.as_int = curr.sticky};
    CHK_TRUE_RET(Attr_Write(stream, &sticky, "sticky"));

    struct attr target_uid =
    ...
    struct attr move_cmd_interrupted = (struct attr){
        .type = TYPE_BOOL, .val.as_bool = curr.move_cmd_interrupted};
    CHK_TRUE_RET(
        Attr_Write(stream, &move_cmd_interrupted, "move_cmd_interrupted"));

    struct attr move_cmd_attacking = (struct attr){
        .type = TYPE_BOOL, .val.as_bool = curr.move_cmd_attacking};
    CHK_TRUE_RET(Attr_Write(stream, &move_cmd_attacking, "move_cmd_attacking"));

    struct attr move_cmd_xz =
        (struct attr){.type = TYPE_VEC2, .val.as_vec2 = curr.move_cmd_xz};
    CHK_TRUE_RET(Attr_Write(stream, &move_cmd_xz, "move_cmd_xz"));

    struct attr attack_elapsed_ticks = (struct attr){
        .type = TYPE_INT, .val.as_int = curr_ticks - curr.attack_start_tick};
    CHK_TRUE_RET(
        Attr_Write(stream, &attack_elapsed_ticks, "attack_elapsed_ticks"));

    struct attr pd_basedir = (struct attr){
        .type = TYPE_STRING,
    };
    tc_strlcpy(pd_basedir.val.as_string, curr.pd.basedir,
               sizeof(pd_basedir.val.as_string));
    CHK_TRUE_RET(Attr_Write(stream, &pd_basedir, "pd_basedir"));

    struct attr pd_tcobj = (struct attr){
        .type = TYPE_STRING,
    };
    tc_strlcpy(pd_tcobj.val.as_string, curr.pd.tcobj,
               sizeof(pd_tcobj.val.as_string));
    CHK_TRUE_RET(Attr_Write(stream, &pd_tcobj, "pd_tcobj"));
  ...
  struct attr num_dying =
      (struct attr){.type = TYPE_INT, .val.as_int = vec_size(&s_dying_ents)};
  ...
  for (int i = 0; i < vec_size(&s_dying_ents); i++) {
    ...
    struct attr uid =
        (struct attr){.type = TYPE_INT, .val.as_int = curr_ent->uid};
...
bool G_Combat_LoadState(struct SDL_RWops *stream) {
  ...
  for (int i = 0; i < num_ents; i++) {
    ...
    cs->pd.basedir = tc_strdup(attr.val.as_string);

    CHK_TRUE_RET(Attr_Parse(stream, &attr, true));
    CHK_TRUE_RET(attr.type == TYPE_STRING);
    cs->pd.tcobj = tc_strdup(attr.val.as_string);
