...
static void compute_vo_edges(struct cp_ent ent, struct cp_ent neighb,
  ...
  TCM_Vec2_Sub(&neighb.xz_pos, &ent.xz_pos, &ent_to_nb);
  TCM_Vec2_Normal(&ent_to_nb, &ent_to_nb);

  right = (vec2_t){-ent_to_nb.z, ent_to_nb.x};
  TCM_Vec2_Scale(&right, neighb.radius + ent.radius + CLEARPATH_BUFFER_RADIUS,
                 &right);

  vec2_t right_tangent, left_tangent;
  TCM_Vec2_Add(&neighb.xz_pos, &right, &right_tangent);
  TCM_Vec2_Sub(&neighb.xz_pos, &right, &left_tangent);

  TCM_Vec2_Sub(&right_tangent, &ent.xz_pos, out_xz_right);
  TCM_Vec2_Normal(out_xz_right, out_xz_right);
  assert(fabs(TCM_Vec2_Len(out_xz_right) - 1.0f) < EPSILON);

  TCM_Vec2_Sub(&left_tangent, &ent.xz_pos, out_xz_left);
  TCM_Vec2_Normal(out_xz_left, out_xz_left);
  assert(fabs(TCM_Vec2_Len(out_xz_left) - 1.0f) < EPSILON);
}
...
static struct VO compute_vo(struct cp_ent ent, struct cp_ent neighb) {
  ...
  TCM_Vec2_Add(&ent.xz_pos, &neighb.xz_vel, &ret.xz_apex);
  return ret;
}
...
static struct RVO compute_rvo(struct cp_ent ent, struct cp_ent neighb) {
  ...
  vec2_t apex_off;
  TCM_Vec2_Add(&ent.xz_vel, &neighb.xz_vel, &apex_off);
  TCM_Vec2_Scale(&apex_off, 0.5f, &apex_off);
  TCM_Vec2_Add(&ent.xz_pos, &apex_off, &ret.xz_apex);
...
static struct HRVO compute_hrvo(struct cp_ent ent, struct cp_ent neighb) {
  ...
  vec2_t centerline;
  TCM_Vec2_Add(&rvo.xz_left_side, &rvo.xz_right_side, &centerline);

  vec2_t vo_apex;
  TCM_Vec2_Add(&ent.xz_pos, &neighb.xz_vel, &vo_apex);
  ...
  } else if (det < -EPSILON) { /* the entity velocity is right of the RVO
                                  centerline */
...
/* Points exactly 'on' the boundary will be considered as 'not inside' of the
 * PCR for our purposes. */
static bool inside_pcr(const struct line_2d *vo_lr_pairs, size_t n_rays,
                       vec2_t test) {
  assert(n_rays % 2 == 0);
  for (int i = 0; i < n_rays; i += 2) {

    assert(fabs(TCM_Vec2_Len(&vo_lr_pairs[i + 0].dir) - 1.0f) < EPSILON);
    const float left_dir_x = vo_lr_pairs[i + 0].dir.x;
    const float left_dir_z = vo_lr_pairs[i + 0].dir.z;

    vec2_t point_to_test;
    TCM_Vec2_Sub(&test, (vec2_t *)&vo_lr_pairs[i + 0].point, &point_to_test);
    TCM_Vec2_Normal(&point_to_test, &point_to_test);

    float left_det =
        (point_to_test.z * left_dir_x) - (point_to_test.x * left_dir_z);
    bool left_of_vo = (left_det < EPSILON);

    if (left_of_vo)
      continue;

    assert(fabs(TCM_Vec2_Len(&vo_lr_pairs[i + 1].dir) - 1.0f) < EPSILON);
    const float right_dir_x = vo_lr_pairs[i + 1].dir.x;
    const float right_dir_z = vo_lr_pairs[i + 1].dir.z;

    TCM_Vec2_Sub(&test, (vec2_t *)&vo_lr_pairs[i + 1].point, &point_to_test);
    TCM_Vec2_Normal(&point_to_test, &point_to_test);

    float right_det =
        (point_to_test.z * right_dir_x) - (point_to_test.x * right_dir_z);
...
static void rays_repr(const struct HRVO *hrvos, size_t n_hrvos,
                      const struct VO *vos, size_t n_vos, struct line_2d *out) {
  size_t rays_idx = 0;
...
static size_t compute_vdes_proj_points(struct line_2d *rays, size_t n_rays,
  ...
  for (int i = 0; i < n_rays; i++) {
    ...
    assert(fabs(TCM_Vec2_Len(&rays[i].dir) - 1.0f) < EPSILON);

    float len = TCM_Vec2_Dot(&rays[i].dir, &des_v);
    TCM_Vec2_Scale(&rays[i].dir, len, &proj);
    TCM_Vec2_Add(&rays[i].point, &proj, &proj);
...
static vec2_t compute_vnew(const vec_vec2_t *outside_points, vec2_t des_v,
                           vec2_t ent_xz_pos) {
  ...
  for (int i = 0; i < vec_size(outside_points); i++) {
    ...
    TCM_Vec2_Sub(&curr, &ent_xz_pos, &curr);

    TCM_Vec2_Sub(&des_v, &curr, &diff);
    if ((len = TCM_Vec2_Len(&diff)) < min_dist) {
...
static void remove_furthest(vec2_t xz_pos, vec_cp_ent_t *dyn_inout,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int j = 0; j < vec_size(curr_vec); j++) {
      ...
      TCM_Vec2_Sub(&xz_pos, &ent->xz_pos, &diff);
      if ((len = TCM_Vec2_Len(&diff)) > max_dist) {
        max_dist = len;
        del_vec = curr_vec;
        del_idx = j;
      }
...
static void on_render_3d(void *user, void *event) {
  ...
  vec3_t origin_pos =
      (vec3_t){cpent->xz_pos.x, M_HeightAtPoint(map, cpent->xz_pos) + 5.0f,
               cpent->xz_pos.z};

  vec2_t des_v = s_debug_saved.ent_des_v;
  vec3_t des_vel_dir = (vec3_t){des_v.x, 0.0f, des_v.z};
  TCM_Vec3_Normal(&des_vel_dir, &des_vel_dir);

  float t = TCM_Vec2_Len(&des_v) * MOVE_TICK_RES;
  R_PushCmd((struct rcmd){
      .func = R_GL_DrawRay,
      .nargs = 5,
      .args =
  ...
  TCM_Vec3_Normal(&vel_dir, &vel_dir);

  t = TCM_Vec2_Len(&v_new) * MOVE_TICK_RES;
  R_PushCmd((struct rcmd){
      .func = R_GL_DrawRay,
      .nargs = 5,
      .args =
  ...
  for (int i = 0; i < vec_size(&s_debug_saved.xpoints); i++) {
    ...
    R_PushCmd((struct rcmd){
        ...
        .args =
            {
                R_PushArg(&vec_AT(&s_debug_saved.xpoints, i),
                          sizeof(vec_AT(&s_debug_saved.xpoints, 0))),
                R_PushArg(&radius, sizeof(radius)),
                R_PushArg(&width, sizeof(width)),
                R_PushArg(&green, sizeof(green)),
                (void *)G_GetPrevTickMap(),
  ...
  char strbuff[256];
  tc_strlcpy(strbuff, "Desired Velocity in PCR:", sizeof(strbuff));
  tc_strlcat(strbuff, s_debug_saved.des_v_in_pcr ? "true" : "false",
             sizeof(strbuff));
  struct rgba text_color = s_debug_saved.des_v_in_pcr
                               ? (struct rgba){255, 0, 0, 255}
                               : (struct rgba){0, 255, 0, 255};
  UI_DrawText(strbuff, (struct rect){5, 50, 200, 50}, text_color);
}

static bool clearpath_new_velocity(struct cp_ent cpent, uint32_t ent_uid,
                                   vec2_t ent_des_v,
                                   const vec_cp_ent_t dyn_neighbs,
                                   const vec_cp_ent_t stat_neighbs,
                                   bool save_debug, vec2_t *out) {
  ...
  vec2_t des_v_ws;
  TCM_Vec2_Add(&cpent.xz_pos, &ent_des_v, &des_v_ws);
...
bool G_ClearPath_ShouldSaveDebug(uint32_t ent_uid) {
  ...
  struct sval setting;
  ss_e status = Settings_Get("tc.debug.show_first_sel_combined_hrvo", &setting);
...
vec2_t G_ClearPath_NewVelocity(struct cp_ent cpent, uint32_t ent_uid,
  ...
  do {
    vec2_t ret;
    bool found = clearpath_new_velocity(cpent, ent_uid, ent_des_v, dyn_neighbs,
                                        stat_neighbs, save_debug, &ret);
    if (found)
      PERF_RETURN(ret);
