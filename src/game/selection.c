...
static void on_mousedown(void *user, void *event) {
   ...
   * the screen (camera pan action) which is mutually exclusive to unit
   * selection. */
  if (mouse_event->x == 0 || mouse_event->x == w - 1 || mouse_event->y == 0 ||
      mouse_event->y == h - 1)
    return;
...
static void on_mousemove(void *user, void *event) { s_hovered_dirty = true; }
...
static void on_render_ui(void *user, void *event) {
  ...
  vec2_t signed_size = (vec2_t){mouse_x - s_ctx.mouse_down_coord.x,
                                mouse_y - s_ctx.mouse_down_coord.y};
...
static vec3_t sel_unproject_mouse_coords(struct camera *cam,
                                         vec2_t mouse_coords, float ndc_z) {
  int w, h;
  Engine_WinDrawableSize(&w, &h);

  vec3_t ndc = (vec3_t){-1.0f + 2.0 * (mouse_coords.raw[0] / (float)w),
                        1.0f - 2.0 * (mouse_coords.raw[1] / (float)h), ndc_z};
  ...
  TCM_Mat4x4_Mult4x4(&proj, &view, &tmp);
  TCM_Mat4x4_Inverse(&tmp, &view_proj_inverse);

  vec4_t ret_homo;
  TCM_Mat4x4_Mult4x1(&view_proj_inverse, &clip, &ret_homo);
  return (vec3_t){ret_homo.x / ret_homo.w, ret_homo.y / ret_homo.w,
                  ret_homo.z / ret_homo.w};
}

static void sel_make_frustum(struct camera *cam, vec2_t mouse_down,
                             vec2_t mouse_up, struct frustum *out) {
  ...
  TCM_Vec3_Sub(&out->ftl, &out->ntl, &tl_dir);
  TCM_Vec3_Sub(&out->fbl, &out->nbl, &bl_dir);
  TCM_Vec3_Sub(&out->ftr, &out->ntr, &tr_dir);
  TCM_Vec3_Sub(&out->fbr, &out->nbr, &br_dir);

  TCM_Vec3_Normal(&tl_dir, &tl_dir);
  TCM_Vec3_Normal(&bl_dir, &bl_dir);
  TCM_Vec3_Normal(&tr_dir, &tr_dir);
  TCM_Vec3_Normal(&br_dir, &br_dir);

  vec3_t up, left;
  TCM_Vec3_Sub(&out->ntl, &out->nbl, &up);
  TCM_Vec3_Normal(&up, &up);
  TCM_Vec3_Sub(&out->ntl, &out->ntr, &left);
  TCM_Vec3_Normal(&left, &left);

  out->top.point = out->ntl;
  TCM_Vec3_Cross(&tl_dir, &left, &out->top.normal);
  TCM_Vec3_Normal(&out->top.normal, &out->top.normal);

  out->bot.point = out->nbr;
  TCM_Vec3_Cross(&left, &bl_dir, &out->bot.normal);
  TCM_Vec3_Normal(&out->bot.normal, &out->bot.normal);

  out->right.point = out->ntr;
  TCM_Vec3_Cross(&tr_dir, &up, &out->right.normal);
  TCM_Vec3_Normal(&out->right.normal, &out->right.normal);

  out->left.point = out->nbl;
  TCM_Vec3_Cross(&up, &tl_dir, &out->left.normal);
  TCM_Vec3_Normal(&out->left.normal, &out->left.normal);
}
...
static void sel_compute_hovered(struct camera *cam,
                                const vec_pentity_t *visible,
  ...
  vec3_t ray_origin =
      sel_unproject_mouse_coords(cam, (vec2_t){mouse_x, mouse_y}, -1.0f);
  vec3_t ray_dir;

  vec3_t cam_pos = Camera_GetPos(cam);
  TCM_Vec3_Sub(&ray_origin, &cam_pos, &ray_dir);
  TCM_Vec3_Normal(&ray_dir, &ray_dir);
...
static void sel_filter_and_set_type(void) {
  ...
  for (int i = 0; i < vec_size(&s_selected); i++) {
    ...
    if (allied_to_player_controllabe(controllable, fac_mask,
                                     G_GetFactionID(curr->uid))) {
      has_allied = true;
    }
  }
  ...
  for (int i = vec_size(&s_selected) - 1; i >= 0; i--) {
    ...
    if (has_player && !controllable[G_GetFactionID(curr->uid)]) {
      ...
      vec_pentity_del(&s_selected, i);
               ...
               !allied_to_player_controllabe(controllable, fac_mask,
                                             G_GetFactionID(curr->uid))) {
...
/* Note that the selection is only changed if there is at least one entity in
 * the new selection. Otherwise (ex. if the player is left-clicking on an empty
 * part of the map), the previous selection is kept. */
void G_Sel_Update(struct camera *cam, const vec_pentity_t *visible,
  ...
  if (s_ctx.mouse_down_coord.x == s_ctx.mouse_up_coord.x &&
    ...
    /* Case 1: The mouse is pressed and released in the same spot, meaning we
     * can use a single ray to test against the OBBs
     *
     * The behaviour is that only a single entity can be selected with a 'click'
     * action, even if multiple OBBs intersect with the mouse ray. We pick the
     * one with the closest intersection point.
     */
    ...
    /* Case 2: The mouse is pressed and released in different spots, meaning the
     * OBBs must be tested against a frustum that is defined by the selection
     * box.*/
...
bool G_Sel_SaveState(struct SDL_RWops *stream) {
  struct attr installed =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = s_ctx.installed};
  CHK_TRUE_RET(Attr_Write(stream, &installed, "installed"));

  struct attr sel_type =
      (struct attr){.type = TYPE_INT, .val.as_int = s_ctx.type};
  CHK_TRUE_RET(Attr_Write(stream, &sel_type, "sel_type"));

  struct attr num_selected =
      (struct attr){.type = TYPE_INT, .val.as_int = vec_size(&s_selected)};
  CHK_TRUE_RET(Attr_Write(stream, &num_selected, "num_selected"));

  for (int i = 0; i < vec_size(&s_selected); i++) {

    struct attr selected_ent = (struct attr){
        .type = TYPE_INT, .val.as_int = vec_AT(&s_selected, i)->uid};
...
void G_Sel_MarkHoveredDirty(void) { s_hovered_dirty = true; }

bool G_Sel_IsSelected(const struct entity *ent) {
  int idx =
      vec_pentity_indexof(&s_selected, (struct entity *)ent, pentities_equal);
  return (idx != -1);
}
