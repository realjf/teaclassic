...
static bool al_read_joint(SDL_RWops *stream, struct joint *out,
                          struct SQT *out_bind) {
  ...
  tc_strtok_r(line, " \t", &saveptr);
  tc_strtok_r(NULL, " \t", &saveptr);
  tc_strtok_r(NULL, " \t", &saveptr);

  string = tc_strtok_r(NULL, " \t", &saveptr);
  if (!sscanf(string, "%f/%f/%f", &out_bind->scale.x, &out_bind->scale.y,
              &out_bind->scale.z))
    goto fail;

  string = tc_strtok_r(NULL, " \t", &saveptr);
  if (!sscanf(string, "%f/%f/%f/%f", &out_bind->quat_rotation.x,
              &out_bind->quat_rotation.y, &out_bind->quat_rotation.z,
              &out_bind->quat_rotation.w))
    goto fail;

  string = tc_strtok_r(NULL, " \t", &saveptr);
  if (!sscanf(string, "%f/%f/%f", &out_bind->trans.x, &out_bind->trans.y,
              &out_bind->trans.z))
    goto fail;
...
static bool al_read_anim_clip(SDL_RWops *stream, struct anim_clip *out,
  ...
  for (int f = 0; f < out->num_frames; f++) {
    for (int j = 0; j < header->num_joints; j++) {
      ...
      READ_LINE(stream, line, fail);
      if (!sscanf(
              line, "%d %f/%f/%f %f/%f/%f/%f %f/%f/%f", &joint_idx,
              &curr_joint_trans->scale.x, &curr_joint_trans->scale.y,
              &curr_joint_trans->scale.z, &curr_joint_trans->quat_rotation.x,
              &curr_joint_trans->quat_rotation.y,
              &curr_joint_trans->quat_rotation.z,
              &curr_joint_trans->quat_rotation.w, &curr_joint_trans->trans.x,
              &curr_joint_trans->trans.y, &curr_joint_trans->trans.z)) {
        goto fail;
      }
    }
...
size_t al_data_buffsize_from_header(const struct tcobj_hdr *header) {
  size_t ret = 0;
...
size_t A_AL_CtxBuffSize(void) { return sizeof(struct anim_ctx); }
...
void A_AL_DumpPrivate(FILE *stream, void *priv_data) {
  ...
  for (int i = 0; i < priv->skel.num_joints; i++) {
    ...
    TCM_Quat_ToEuler(&bind->quat_rotation, &roll, &pitch, &yaw);

    fprintf(stream, "j %d %s ", j->parent_idx + 1, j->name);
    fprintf(stream,
            "%.6f/%.6f/%.6f %.6f/%.6f/%.6f %.6f/%.6f/%.6f %.6f/%.6f/%.6f\n",
            bind->scale.x, bind->scale.y, bind->scale.z, roll, pitch, yaw,
            bind->trans.x, bind->trans.y, bind->trans.z, j->tip.x, j->tip.y,
            j->tip.z);
  }
  ...
  for (int i = 0; i < priv->num_anims; i++) {
    ...
    for (int f = 0; f < ac->num_frames; f++) {
      for (int j = 0; j < ac->skel->num_joints; j++) {
        ...
        float roll, pitch, yaw;
        TCM_Quat_ToEuler(&sqt->quat_rotation, &roll, &pitch, &yaw);

        fprintf(stream, "\t%d %f/%f/%f %f/%f/%f %f/%f/%f\n", j + 1,
                sqt->scale.x, sqt->scale.y, sqt->scale.z, roll, pitch, yaw,
