...
#include "../entity.h"
#include "../lib/public/tc_malloc.h"
...
static bool batch_alloc_vbo(struct gl_batch *batch)
{
    ...
    void *heap_meta = tc_metamalloc_init(MESH_BUFF_SZ);
    if(!heap_meta)
        return false;
...
static bool batch_append_mesh(struct gl_batch *batch, GLuint VBO)
{
    ...
    int vbo_offset = -1;
    do{
        vbo_offset = tc_metamemalign(batch->vbos[curr_vbo_idx].heap_meta, alignment, size);
        if(vbo_offset >= 0)
            break;
    ...
    if(status == -1) {
        tc_metafree(batch->vbos[curr_vbo_idx].heap_meta, vbo_offset);
        return false;
    }
...
static void batch_free_mesh(struct gl_batch *batch, GLuint VBO)
{
    ...
    tc_metafree(batch->vbos[md.vbo_idx].heap_meta, md.offset);
                                  ...
                                  struct draw_call_desc dcall, struct inst_group_desc *descs)
{
    ...
    for(int i = dcall.start_idx; i <= dcall.end_idx; i++) {
        ...
        for(int j = curr->start_idx; j <= curr->end_idx; j++) {
            ...
            TCM_Mat4x4_Inverse((mat4x4_t*)&ents[j].model, &model);
