...
static void tile_top_normals(const struct tile *tile, vec3_t out_tri_normals[static 2], bool *out_tri_left)
{
    ...
    TCM_Vec3_Normal(out_tri_normals, out_tri_normals);
    TCM_Vec3_Normal(out_tri_normals + 1, out_tri_normals + 1);
}
...
static void tile_smooth_normals_corner(struct tile *adj_cw[static 4], struct terrain_vert *inout)
{
    ...
    for(int i = 0; i < 4; i++) {
        ...
        case ADJ_CW_IDX_TOP_LEFT:
            TCM_Vec3_Add(&norm_total, normals + 1, &norm_total);
            TCM_Vec3_Add(&norm_total, normals + (top_tri_left_aligned ? 1 : 0), &norm_total);
            break;
        case ADJ_CW_IDX_TOP_RIGHT:
            TCM_Vec3_Add(&norm_total, normals + 1, &norm_total);
            TCM_Vec3_Add(&norm_total, normals + (top_tri_left_aligned ? 0 : 1), &norm_total);
            break;
        case ADJ_CW_IDX_BOT_RIGHT:
            TCM_Vec3_Add(&norm_total, normals + 0, &norm_total);
            TCM_Vec3_Add(&norm_total, normals + (top_tri_left_aligned ? 0 : 1), &norm_total);
            break;
    ...
    TCM_Vec3_Normal(&norm_total, &norm_total);
...
static void tile_smooth_normals_edge(struct tile *adj_lrtb[static 4], struct terrain_vert *inout)
{
    ...
    assert(TCM_Vec3_Len(&norm_total) > 0);
    TCM_Vec3_Normal(&norm_total, &norm_total);
    inout->normal = norm_total;
}
                           ...
                           const int *tiles_per_chunk_x, const int *tiles_per_chunk_z)
{
    ...
    TCM_Mat4x4_MakeScale(SCALE_FACTOR, SCALE_FACTOR, SCALE_FACTOR, &scale);
    ...
    TCM_Mat4x4_MakeTrans(-center.x, -center.y, -center.z, &trans);
    TCM_Mat4x4_MakeTrans( center.x,  center.y,  center.z, &trans_inv);

    TCM_Mat4x4_Mult4x4(&scale, &trans, &tmp1);
    TCM_Mat4x4_Mult4x4(&trans_inv, &tmp1, &tmp2);
    TCM_Mat4x4_Mult4x4(model, &tmp2, &final_model);
...
void R_GL_TilePatchVertsSmooth(void *chunk_rprivate, const struct map *map, const struct tile_desc *tile)
{
    ...
    TCM_Vec3_Add(&center_norm, normals + 0, &center_norm);
    TCM_Vec3_Add(&center_norm, normals + 1, &center_norm);
    TCM_Vec3_Normal(&center_norm, &center_norm);
...
int R_TileGetTriMesh(const struct map *map, struct tile_desc *td, mat4x4_t *model, vec3_t out[])
{
    ...
    for(; i < ARR_SIZE(verts); i++) {
        ...
        vec4_t ws_pos_homo;
        TCM_Mat4x4_Mult4x1(model, &pos_homo, &ws_pos_homo);
