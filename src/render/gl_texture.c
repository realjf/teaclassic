...
bool R_GL_Texture_GetForName(const char *basedir, const char *name, GLuint *out)
{
    ...
    char qualname[512];
    tc_snprintf(qualname, sizeof(qualname), "%s/%s", basedir, name);
...
bool R_GL_Texture_Load(const char *basedir, const char *name, GLuint *out)
{
    ...
    if(basedir) {
        tc_snprintf(texture_path, sizeof(texture_path), "%s/%s", basedir, name);
    }else{
        texture_path[0] = '\0';
    }
    tc_snprintf(texture_path_maps, sizeof(texture_path_maps), "%s/assets/map_textures/%s", g_basepath, name);
    ...
    int put_ret;
    k = kh_put(tex, s_name_tex_table, tc_strdup(texture_path), &put_ret);
...
bool R_GL_Texture_AddExisting(const char *name, GLuint id)
{
    ...
    int put_ret;
    k = kh_put(tex, s_name_tex_table, tc_strdup(name), &put_ret);
    assert(put_ret != -1 && put_ret != 0);
...
void R_GL_Texture_Free(const char *basedir, const char *name)
{
    ...
    if(basedir) {
        ...
        tc_strlcpy(qualname, name, sizeof(qualname));
    }
                               ...
                               struct texture_arr *out, GLuint tunit)
{
    ...
    for(int i = 0; i < num_textures; i++) {
        ...
        char path[512];
        tc_snprintf(path, sizeof(path), "%s/assets/map_textures/%s", g_basepath, texnames[i]);
