...
void draw_cam_frustum(const struct camera *cam, mat4x4_t *minimap_model, const struct map *map)
{
    ...
    TCM_Vec3_Sub(&cam_frust.ftr, &cam_frust.ntr, &tr_dir);
    TCM_Vec3_Sub(&cam_frust.ftl, &cam_frust.ntl, &tl_dir);
    TCM_Vec3_Sub(&cam_frust.fbr, &cam_frust.nbr, &br_dir);
    TCM_Vec3_Sub(&cam_frust.fbl, &cam_frust.nbl, &bl_dir);
    ...
    if(!C_RayIntersectsPlane(cam_pos, tr_dir, ground_plane, &t))
        t = (1e10);
    TCM_Vec3_Scale(&tr_dir, t, &tr_dir);
    TCM_Vec3_Add(&cam_pos, &tr_dir, &tr);

    if(!C_RayIntersectsPlane(cam_pos, tl_dir, ground_plane, &t))
        t = (1e10);
    TCM_Vec3_Scale(&tl_dir, t, &tl_dir);
    TCM_Vec3_Add(&cam_pos, &tl_dir, &tl);
    ...
    if(!C_RayIntersectsPlane(cam_pos, br_dir, ground_plane, &t))
        GL_PERF_RETURN_VOID();
    TCM_Vec3_Scale(&br_dir, t, &br_dir);
    TCM_Vec3_Add(&cam_pos, &br_dir, &br);

    if(!C_RayIntersectsPlane(cam_pos, bl_dir, ground_plane, &t))
        GL_PERF_RETURN_VOID();
    TCM_Vec3_Scale(&bl_dir, t, &bl_dir);
    TCM_Vec3_Add(&cam_pos, &bl_dir, &bl);
    ...
    TCM_Mat4x4_MakeTrans(-1.0f, -1.0f, 0.0f, &one_px_trans);
    TCM_Mat4x4_Mult4x4(&one_px_trans, minimap_model, &new_model);
...
static void setup_ortho_view_uniforms(const struct map *map)
{
    ...
    TCM_Vec3_Add(&map_center, &offset, &map_center);
                        ...
                        vec2_t *center_pos, const int *side_len_px, vec4_t *border_clr)
{
    ...
    TCM_Mat4x4_MakeRotZ(DEG_TO_RAD(-45.0f), &tilt);
    TCM_Mat4x4_MakeScale((*side_len_px)/2.0f, (*side_len_px)/2.0f, 1.0f, &scale);
    TCM_Mat4x4_MakeTrans(center_pos->x, center_pos->y, 0.0f, &trans);
    TCM_Mat4x4_Mult4x4(&scale, &tilt, &tmp);
    TCM_Mat4x4_Mult4x4(&trans, &tmp, &model);
    ...
    TCM_Mat4x4_MakeScale(scale_fac, scale_fac, scale_fac, &border_scale);
    TCM_Mat4x4_Mult4x4(&border_scale, &tilt, &tmp);
    TCM_Mat4x4_Mult4x4(&trans, &tmp, &border_model);
