...
bool R_GL_Shader_InitAll(const char *base_path)
{
    ...
    for(int i = 0; i < ARR_SIZE(s_shaders); i++){
        ...
        char path[512];
        tc_snprintf(path, sizeof(path), "%s/%s", base_path, res->vertex_path);
        ...
        if(res->geo_path)
            tc_snprintf(path, sizeof(path), "%s/%s", base_path, res->geo_path);
        ...
        tc_snprintf(path, sizeof(path), "%s/%s", base_path, res->frag_path);
