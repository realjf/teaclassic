...
static void exec_draw_commands(const struct nk_draw_list *dl, GLuint shader_prog)
{
    ...
    mat4x4_t ortho;
    TCM_Mat4x4_MakeOrthographic(0.0f, curr_vres.x, curr_vres.y, 0.0f, -1.0f, 1.0f, &ortho);
    ...
    for(cmd = nk__draw_list_begin(dl, dl->buffer); cmd;
        ...
        if(cmd->userdata.ptr) {
            ...
            case NK_COMMAND_SET_VRES: {
                ...
                TCM_Mat4x4_MakeOrthographic(0.0f, ud->vec2i.x, ud->vec2i.y, 0.0f, -1.0f, 1.0f, &ortho);
                R_GL_StateSet(GL_U_PROJECTION, (struct uval){
                    .type = UTYPE_MAT4,
                    .val.as_mat4 = ortho
                });
