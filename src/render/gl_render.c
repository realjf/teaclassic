...
void R_GL_SetScreenspaceDrawMode(void)
{
    ...
    mat4x4_t ortho;
    TCM_Mat4x4_MakeOrthographic(0.0f, width, height, 0.0f, -1.0f, 1.0f, &ortho);
    R_GL_SetProj(&ortho);

    mat4x4_t identity;
    TCM_Mat4x4_Identity(&identity);
...
void R_GL_DrawSkeleton(const struct entity *ent, const struct skeleton *skel, const struct camera *cam)
{
    ...
    for(int i = 0, vbuff_idx = 0; i < skel->num_joints; i++, vbuff_idx +=2) {
        ...
        mat4x4_t bind_pose;
        TCM_Mat4x4_Inverse(&skel->inv_bind_poses[i], &bind_pose);

        /* The root of the bone in object space */
        TCM_Mat4x4_Mult4x1(&bind_pose, &homo, &result);
        ...
        TCM_Mat4x4_Mult4x1(&model, &root_homo, &tmpa);
        TCM_Mat4x4_Mult4x1(&view, &tmpa, &tmpb);
        TCM_Mat4x4_Mult4x1(&proj, &tmpb, &clip);
                  ...
                  const vec3_t *color, const float *t)
{
    ...
    vbuff[0] = *origin;
    vec3_t dircopy = *dir;
    TCM_Vec3_Normal(&dircopy, &dircopy);
    TCM_Vec3_Scale(&dircopy, *t, &dircopy);
    TCM_Vec3_Add((vec3_t*)origin, &dircopy, &vbuff[1]);
                    ...
                    const vec3_t *color, const float *width)
{
    ...
    mat4x4_t ortho;
    TCM_Mat4x4_MakeOrthographic(0.0f, win_width, win_height, 0.0f, -1.0f, 1.0f, &ortho);
    R_GL_SetProj(&ortho);
                                 ...
                                 const vec3_t *color, const struct map *map)
{
    ...
    TCM_Vec2_Sub(&corners[1], &corners[0], &deltas[0]);
    TCM_Vec2_Sub(&corners[2], &corners[1], &deltas[1]);
    TCM_Vec2_Sub(&corners[3], &corners[2], &deltas[2]);
    TCM_Vec2_Sub(&corners[0], &corners[3], &deltas[3]);

    for(int i = 0; i < 4; i++) {
        lens[i] = TCM_Vec2_Len(&deltas[i]);
        tot_len += lens[i];
        TCM_Vec2_Normal(&deltas[i], &deltas[i]);
    }
    ...
    for(int i = 0; i < 4; i++) {
        ...
        TCM_Vec3_Scale(&pdir, *width/2.0f, &pdir);

        for(int j = 0; j < ceil(lens[i] / sample_dist) + 1; j++) {

            vec2_t dir = deltas[i];
            TCM_Vec2_Scale(&dir, MIN(j * sample_dist, lens[i]), &dir);

            vec2_t xz;
            TCM_Vec2_Add(&corners[i], &dir, &xz);
            ...
            TCM_Vec3_Scale(&nudge, PAD, &nudge);
            TCM_Vec3_Add(&point, &nudge, &nudged);

            TCM_Vec3_Sub(&nudged, &pdir, &vbuff[vbuff_idx++]);
            TCM_Vec3_Add(&nudged, &pdir, &vbuff[vbuff_idx++]);
        }
    }
...
void R_GL_DrawLine(vec2_t endpoints[static 2], const float *width, const vec3_t *color, const struct map *map)
{
    ...
    vec2_t delta;
    TCM_Vec2_Sub(&endpoints[1], &endpoints[0], &delta);
    const float len = TCM_Vec2_Len(&delta);

    vec2_t perp = (vec2_t){ delta.z, -delta.x };
    TCM_Vec2_Normal(&perp, &perp);
    assert(*width > 0.0f);
    TCM_Vec2_Scale(&perp, *width/2.0f, &perp);
    ...
    for(int i = 0; i <= NUM_SAMPLES * 2; i += 2) {
        ...
        TCM_Vec2_Scale(&delta, t, &delta);

        vec2_t point;
        TCM_Vec2_Add(&endpoints[0], &delta, &point);

        vec2_t point_left, point_right;
        TCM_Vec2_Add(&point, &perp, &point_left);
        TCM_Vec2_Sub(&point, &perp, &point_right);
                              ...
                              mat4x4_t *model, const struct map *map)
{
    ...
    for(int i = 0; i < *count; i++, xz_corners += 4, colors++) {
        ...
        for(int i = 0; i < ARR_SIZE(verts); i++) {
            ...
            vec4_t ws_xz_homo;
            TCM_Mat4x4_Mult4x1(model, &xz_homo, &ws_xz_homo);
                        ...
                        mat4x4_t *model, const struct map *map)
{
    ...
    for(size_t i = 0, line_vbuff_idx = 0; i < *count; i++, line_vbuff_idx += 2) {
        ...
        TCM_Vec2_Scale(&to_add, 2.5f, &to_add);
        TCM_Vec2_Add(&tip, &to_add, &tip);
        ...
        TCM_Mat4x4_Mult4x1(model, &base_homo, &base_ws_homo);
        base_ws_homo.x /= base_ws_homo.w;
        base_ws_homo.z /= base_ws_homo.w;

        TCM_Mat4x4_Mult4x1(model, &tip_homo, &tip_ws_homo);
                           ...
                           const size_t *num_vos, const struct map *map)
{
    ...
    mat4x4_t model;
    TCM_Mat4x4_Identity(&model); /* points are already in world space */
    ...
    for(int i = 0; i < *num_vos; i++) {
        for(int s = 0; s < NUM_SAMPLES-1; s++) {
            ...
            /* Left ray segment */
            assert(fabs(TCM_Vec2_Len(&left_rays[i]) - 1.0) < EPSILON);
            vec2_t xz_off_a = left_rays[i], xz_off_b = left_rays[i];
            TCM_Vec2_Scale(&xz_off_a, ((float)s / NUM_SAMPLES) * RAY_LEN, &xz_off_a);
            TCM_Vec2_Scale(&xz_off_b, ((float)(s + 1) / NUM_SAMPLES) * RAY_LEN, &xz_off_b);

            vec2_t xz_pos_a, xz_pos_b;
            TCM_Vec2_Add(&apexes[i], &xz_off_a, &xz_pos_a);
            TCM_Vec2_Add(&apexes[i], &xz_off_b, &xz_pos_b);
            ...
            /* Right ray segment */
            assert(fabs(TCM_Vec2_Len(&right_rays[i]) - 1.0) < EPSILON);
