...
static void ar_commit(const struct sval *new_val) {
  ...
  if (new_ratio > curr_ratio) {
    ...
    new_res.as_vec2 = (vec2_t){.x = res.as_vec2.x,
                               .y = res.as_vec2.y / (new_ratio / curr_ratio)};
  } else {

    new_res.as_vec2 = (vec2_t){.x = res.as_vec2.x / (curr_ratio / new_ratio),
                               .y = res.as_vec2.y};
  }

  status = Settings_SetNoValidate("tc.video.resolution", &new_res);
  assert(status == SS_OKAY);
}
...
static bool res_validate(const struct sval *new_val) {
  ...
  struct sval ar;
  ss_e status = Settings_Get("tc.video.aspect_ratio", &ar);
  if (status != SS_NO_SETTING) {
...
static bool dm_validate(const struct sval *new_val) {
  ...
  return new_val->as_int == TC_WF_FULLSCREEN ||
         new_val->as_int == TC_WF_BORDERLESS_WIN ||
         new_val->as_int == TC_WF_WINDOW;
}
...
static void render_set_swap(const bool *on) { SDL_GL_SetSwapInterval(*on); }

static void vsync_commit(const struct sval *new_val) {
  R_PushCmd(
      (struct rcmd){.func = render_set_swap,
                    .nargs = 1,
...
static void render_set_trace_gpu(const bool *on) { g_trace_gpu = *on; }

static void trace_gpu_commit(const struct sval *new_val) {
  R_PushCmd(
      (struct rcmd){.func = render_set_trace_gpu,
                    .nargs = 1,
                    .args = {R_PushArg(&new_val->as_bool, sizeof(bool))}});
}
...
static void render_init_ctx(struct render_init_arg *arg) {
  ...
  if (!R_GL_Shader_InitAll(g_basepath) || !R_GL_Texture_Init() ||
      !R_GL_StateInit() || !R_GL_Batch_Init()) {
  ...
  strncpy(s_info_vendor, (const char *)glGetString(GL_VENDOR),
          ARR_SIZE(s_info_vendor) - 1);
  strncpy(s_info_renderer, (const char *)glGetString(GL_RENDERER),
          ARR_SIZE(s_info_renderer) - 1);
  strncpy(s_info_version, (const char *)glGetString(GL_VERSION),
          ARR_SIZE(s_info_version) - 1);
  strncpy(s_info_sl_version,
          (const char *)glGetString(GL_SHADING_LANGUAGE_VERSION),
          ARR_SIZE(s_info_sl_version) - 1);
...
static void render_dispatch_cmd(struct rcmd cmd) {
  ...
  case 0:
    ...
    break;
  case 1:
    ((void (*)(void *))cmd.func)(cmd.args[0]);
    break;
  case 2:
    ((void (*)(void *, void *))cmd.func)(cmd.args[0], cmd.args[1]);
    break;
  case 3:
    ((void (*)(void *, void *, void *))cmd.func)(cmd.args[0], cmd.args[1],
                                                 cmd.args[2]);
    break;
  case 4:
    ((void (*)(void *, void *, void *, void *))cmd.func)(
        cmd.args[0], cmd.args[1], cmd.args[2], cmd.args[3]);
    break;
  case 5:
    ((void (*)(void *, void *, void *, void *, void *))cmd.func)(
        cmd.args[0], cmd.args[1], cmd.args[2], cmd.args[3], cmd.args[4]);
    break;
  case 6:
    ((void (*)(void *, void *, void *, void *, void *, void *))cmd.func)(
        cmd.args[0], cmd.args[1], cmd.args[2], cmd.args[3], cmd.args[4],
        cmd.args[5]);
    break;
  case 7:
    ((void (*)(void *, void *, void *, void *, void *, void *,
               void *))cmd.func)(cmd.args[0], cmd.args[1], cmd.args[2],
...
bool R_Init(const char *base_path) {
  ...
  status = Settings_Create((struct setting){
      .name = "tc.video.aspect_ratio",
      .val =
          (struct sval){.type = ST_TYPE_VEC2, .as_vec2 = (vec2_t){dm.w, dm.h}},
      .prio = 0,
  ...
  struct sval ar_pair;
  Settings_Get("tc.video.aspect_ratio", &ar_pair);
  ...
  status = Settings_Create((struct setting){
      .name = "tc.video.resolution",
      .val = (struct sval){.type = ST_TYPE_VEC2, .as_vec2 = res_default},
  ...
  status = Settings_Create((struct setting){
      .name = "tc.video.display_mode",
      .val = (struct sval){.type = ST_TYPE_INT, .as_int = TC_WF_BORDERLESS_WIN},
      .prio = 0,
  ...
  status = Settings_Create((struct setting){
      .name = "tc.video.window_always_on_top",
  ...
  status = Settings_Create((struct setting){
      .name = "tc.video.water_reflection",
  ...
  status = Settings_Create((struct setting){
      .name = "tc.video.water_refraction",
      .val = (struct sval){.type = ST_TYPE_BOOL, .as_bool = true},
      .prio = 0,
      .validate = bool_val_validate,
      .commit = NULL,
...
SDL_Thread *R_Run(struct render_sync_state *rstate) {
  ...
  /* Create the GL context in the main thread and then hand it off to the render
   * thread. Certain drivers crap out when trying to make the context in the
...
void *R_PushArg(const void *src, size_t size) {
  struct render_workspace *ws =
      (SDL_ThreadID() == g_render_thread_id) ? G_GetRenderWS() : G_GetSimWS();
...
const char *R_GetInfo(enum render_info attr) {
  switch (attr) {
  case RENDER_INFO_VENDOR:
    return s_info_vendor;
  case RENDER_INFO_RENDERER:
    return s_info_renderer;
  case RENDER_INFO_VERSION:
    return s_info_version;
  case RENDER_INFO_SL_VERSION:
    return s_info_sl_version;
  default:
    assert(0);
    return NULL;
  }
