                               ...
                               struct frustum *out, mat4x4_t *out_view_mat)
{
    ...
    TCM_Vec3_Normal(&light_dir, &light_dir);
    TCM_Vec3_Scale(&light_dir, -1.0f, &light_dir);

    vec3_t right = (vec3_t){-1.0f, 0.0f, 0.0f}, up;
    TCM_Vec3_Cross(&light_dir, &right, &up);

    t = fabs((cam_pos.y + LIGHT_EXTRA_HEIGHT)/ light_dir.y);
    vec3_t light_origin, delta;
    TCM_Vec3_Scale(&light_dir, -t, &delta);
    TCM_Vec3_Add(&cam_ray_ground_isec, &delta, &light_origin);

    vec3_t target;
    TCM_Vec3_Add(&light_origin, &light_dir, &target);
     ...
     */
    mat4x4_t light_view;
    TCM_Mat4x4_MakeLookAt(&light_origin, &target, &up, &light_view);
...
void R_GL_DepthPassBegin(const vec3_t *light_pos, const vec3_t *cam_pos, const vec3_t *cam_dir)
{
    ...
    mat4x4_t light_proj;
    TCM_Mat4x4_MakeOrthographic(-CONFIG_SHADOW_FOV, CONFIG_SHADOW_FOV,
    ...
    TCM_Mat4x4_Mult4x4(&light_proj, &light_view, &light_space_trans);
