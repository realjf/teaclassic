                           ...
                           char out_weights_line[static MAX_LINE_LEN])
{
    ...
    /* This really should have been after the material in the TCOBJ format, so
...
static bool al_read_material(SDL_RWops *stream, const char *basedir, struct material *out, bool *out_null)
{
    ...
    char *saveptr;
    char *mat_name = tc_strtok_r(line, " \t\n", &saveptr);
    mat_name = tc_strtok_r(NULL, " \t\n", &saveptr);
...
size_t al_priv_buffsize_from_header(const struct tcobj_hdr *header)
{
    size_t ret = 0;
...
void *R_AL_PrivFromStream(const char *base_path, const struct tcobj_hdr *header, SDL_RWops *stream)
{
    ...
    ss_e status = Settings_Get("tc.video.shadows_enabled", &sh_setting);
...
void R_AL_DumpPrivate(FILE *stream, void *priv_data)
{
    ...
    for(int i = 0; i < priv->num_materials; i++) {
        ...
        char name[32];
        tc_snprintf(name, sizeof(name), "Material.%d", i + 1);
