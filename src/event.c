...
struct handler_desc {
  ...
  void *user_arg;
  int simmask; /* Specifies during which simulation states the handler gets
                  invoked */
...
static inline bool e_handlers_equal(const struct handler_desc *a,
                                    const struct handler_desc *b) {
  if (a->type != b->type)
    return false;

  if (a->type == HANDLER_TYPE_SCRIPT) {
    return S_ObjectsEqual(a->handler.as_script_callable,
                          b->handler.as_script_callable);
  } else {
    return a->handler.as_function == b->handler.as_function;
  }
}

static inline bool e_ptrs_equal(const struct handler_desc *a,
                                const struct handler_desc *b) {
  if (a->type != b->type)
    return false;
...
static bool e_zombie_can_receive(enum eventtype type) {
  return (type == EVENT_ENTITY_DEATH) || (type == EVENT_ENTITY_DISAPPEARED);
}
...
static bool e_register_handler(uint64_t key, struct handler_desc *desc) {
  ...
  if (k == kh_end(s_event_handler_table)) {
    ...
    if (idx != -1)
      return false; /* Don't allow registering duplicate handlers for the same
                       event */
...
static bool e_unregister_handler(uint64_t key,
                                 const struct handler_desc *desc) {
  khiter_t k;
  k = kh_get(handler_desc, s_event_handler_table, key);
...
static void e_invoke(const struct handler_desc hd, struct event event) {
  if (hd.type == HANDLER_TYPE_ENGINE) {
    ...
    script_opaque_t script_arg =
        (event.source == ES_SCRIPT)
...
static void e_handle_event(struct event event, bool immediate) {
  ...
  if (event.receiver_id != GLOBAL_ID && G_EntityIsZombie(event.receiver_id) &&
...
static void e_notify_entities_update_start(uint32_t ticks, bool immediate) {
  ...
  kh_foreach(s_event_handler_table, key, curr, {
    ...
    if (uid == GLOBAL_ID)
      continue;
    e_handle_event(
        (struct event){EVENT_UPDATE_START, NULL, ES_ENGINE, uid, ticks},
        immediate);
  });
}
...
void E_Shutdown(void) {
  khiter_t k;
  for (k = kh_begin(s_event_handler_table); k != kh_end(s_event_handler_table);
       ++k) {
...
void E_ServiceQueue(void) {
  ...
  e_handle_event(
      (struct event){EVENT_UPDATE_START, NULL, ES_ENGINE, GLOBAL_ID, ticks},
      false);
  ...
  e_handle_event(
      (struct event){EVENT_UPDATE_END, NULL, ES_ENGINE, GLOBAL_ID, ticks},
      false);
...
void E_FlushEventQueue(void) {
  ...
  while (E_EventsQueued()) {
    ...
    e_handle_event(
        (struct event){EVENT_RENDER_FINISH, NULL, ES_ENGINE, GLOBAL_ID, ticks},
        true);
    e_handle_event(
        (struct event){EVENT_RENDER_UI, NULL, ES_ENGINE, GLOBAL_ID, ticks},
        true);
    e_handle_event(
        (struct event){EVENT_UPDATE_END, NULL, ES_ENGINE, GLOBAL_ID, ticks},
        true);
...
size_t E_GetScriptHandlers(size_t max_out, struct script_handler *out) {
  ...
  kh_foreach(s_event_handler_table, key, curr, {
    for (int i = 0; i < vec_size(&curr); i++) {
      ...
      out[ret] =
          (struct script_handler){.event = key & ~((uint32_t)0),
                                  .id = key >> 32,
                                  .simmask = hd.simmask,
...
void E_Global_Notify(enum eventtype event, void *event_arg,
                     enum event_source source) {
  struct event e =
      (struct event){event, event_arg, source, GLOBAL_ID, SDL_GetTicks()};
...
void E_Global_NotifyImmediate(enum eventtype event, void *event_arg,
                              enum event_source source) {
  struct event e =
      (struct event){event, event_arg, source, GLOBAL_ID, SDL_GetTicks()};
...
bool E_Entity_Register(enum eventtype event, uint32_t ent_uid,
                       handler_t handler, void *user_arg, int simmask) {
  struct handler_desc hd;
  hd.type = HANDLER_TYPE_ENGINE;
  hd.handler.as_function = handler;
  hd.user_arg = user_arg;
...
bool E_Entity_Unregister(enum eventtype event, uint32_t ent_uid,
                         handler_t handler) {
  struct handler_desc hd;
  hd.type = HANDLER_TYPE_ENGINE;
  hd.handler.as_function = handler;
...
void E_Entity_Notify(enum eventtype event, uint32_t ent_uid, void *event_arg,
                     enum event_source source) {
  struct event e =
      (struct event){event, event_arg, source, ent_uid, SDL_GetTicks()};
  queue_event_push(&s_event_queues[s_front_queue_idx], &e);
}

void E_Entity_NotifyImmediate(enum eventtype event, uint32_t ent_uid,
                              void *event_arg, enum event_source source) {
  struct event e =
      (struct event){event, event_arg, source, ent_uid, SDL_GetTicks()};
  e_handle_event(e, true);
}
...
const char *E_EngineEventString(enum eventtype event) {
  if (event <= (int)SDL_LASTEVENT)
    return NULL;
  if (event - EVENT_UPDATE_START >=
      sizeof(s_event_str_table) / sizeof(const char *))
    return NULL;
