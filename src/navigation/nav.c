                            ...
                            size_t ntiles, const struct tile_desc tds[static ntiles], vec2_t *out)
{
    ...
    for(int i = 0; i < ntiles; i++) {
        ...
        vec2_t delta;
        TCM_Vec2_Sub(&xz_src, &tile_center, &delta);

        if(TCM_Vec2_Len(&delta) < min_dist) {
                                  ...
                                  mat4x4_t *model, mat4x4_t *view, mat4x4_t *proj)
{
    ...
    vec4_t ws_pos_homo;
    TCM_Mat4x4_Mult4x1(model, &map_pos, &ws_pos_homo);
    ws_pos_homo.x /= ws_pos_homo.w;
    ws_pos_homo.z /= ws_pos_homo.w;

    vec4_t clip, tmp;
    TCM_Mat4x4_Mult4x1(view, &ws_pos_homo, &tmp);
                            ...
                            const struct obb *obb, enum nav_layer layer)
{
    ...
    for(int i = 0; i < ntiles; i++) {
        ...
        *corners_base++ = (vec2_t){square_x - square_x_len, square_z};
        ...
        TCM_Mat4x4_Mult4x1(chunk_model, &center_homo, &ws_center_homo);
                       ...
                       int chunk_r, int chunk_c, enum nav_layer layer)
{
    ...
    for(int c = 0; c < FIELD_RES_C; c++) {
        ...
        char text[8];
        tc_snprintf(text, sizeof(text), "%d", chunk->islands[r][c]);
                            ...
                            int chunk_r, int chunk_c, enum nav_layer layer)
{
    ...
    for(int c = 0; c < FIELD_RES_C; c++) {
        ...
        char text[8];
        tc_snprintf(text, sizeof(text), "%d", chunk->local_islands[r][c]);
        n_render_overlay_text(text, center_homo, chunk_model, &view, &proj);
    }}
}
                                  ...
                                  vec3_t map_pos, int faction_id)
{
    ...
    const struct flow_field *tcf = N_FC_FlowFieldAt(ffid);
    assert(tcf);
    ...
    int dir_idx = tcf->field[curr_tile.tile_r][curr_tile.tile_c].dir_idx;
    ...
    if(local_iid == ISLAND_NONE) {
        ...
        struct flow_field exist_ff = *tcf;
     ...
     */
    dir_idx = tcf->field[curr_tile.tile_r][curr_tile.tile_c].dir_idx;
    if(dir_idx == FD_NONE) {
                        ...
                        vec2_t xz_pos, vec2_t xz_dest, float tolerance)
{
    ...
    for(int i = 0; i < ntds; i++) {
        ...
        vec2_t delta;
        TCM_Vec2_Sub(&tile_center, &xz_pos, &delta);

        if(TCM_Vec2_Len(&delta) <= tolerance)
            PERF_RETURN(true);
    }
