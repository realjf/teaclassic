...
static bool effects_equal(const struct al_effect *a,
                          const struct al_effect *b) {
  return (a->uid == b->uid);
}
...
static void audio_update_active_set(vec_effect_t *active) {
  ...
  size_t npotential =
      qt_effect_inrange_circle(&s_effect_tree, center.x, center.z,
                               HEARING_RANGE, potential, ARR_SIZE(potential));
...
static void audio_create_settings(void) {
  ...
  status = Settings_Create((struct setting){
      .name = "tc.audio.effect_volume",
      .val = (struct sval){.type = ST_TYPE_FLOAT, .as_float = s_effect_volume},
      .prio = 0,
...
static bool audio_save_effect(SDL_RWops *stream,
                              const struct al_effect *effect) {
  ALint buffer;
  alGetSourcei(effect->source, AL_BUFFER, &buffer);

  const char *name = Audio_GetEffectName(buffer);
  struct attr name_attr = (struct attr){
      .type = TYPE_STRING,
  };
  tc_strlcpy(name_attr.val.as_string, name, sizeof(name_attr.val.as_string));
  CHK_TRUE_RET(Attr_Write(stream, &name_attr, "name"));

  struct attr uid_attr =
      (struct attr){.type = TYPE_INT, .val.as_int = effect->uid};
  CHK_TRUE_RET(Attr_Write(stream, &uid_attr, "uid"));

  struct attr pos_attr =
      (struct attr){.type = TYPE_VEC3, .val.as_vec3 = effect->pos};
  CHK_TRUE_RET(Attr_Write(stream, &pos_attr, "pos"));

  ALint offset;
  alGetSourcei(effect->source, AL_SAMPLE_OFFSET, &offset);
  struct attr offset_attr =
      (struct attr){.type = TYPE_INT, .val.as_int = offset};
  CHK_TRUE_RET(Attr_Write(stream, &offset_attr, "offset"));

  ALint state;
  alGetSourcei(effect->source, AL_SOURCE_STATE, &state);
  struct attr state_attr = (struct attr){.type = TYPE_INT, .val.as_int = state};
...
static bool audio_load_effect(SDL_RWops *stream) {
  ...
  if (Audio_GetEffectBuffer(name, &buffer)) {
    ...
    struct al_effect effect = (struct al_effect){.uid = uid,
                                                 .pos = pos,
...
bool Audio_Effect_Add(vec3_t pos, const char *track) {
  ...
  struct al_effect effect = (struct al_effect){.uid = Entity_NewUID(),
                                               .pos = pos,
                                               .start_tick = start_tick,
                                               .end_tick = end_tick,
                                               .source = source};
...
float Audio_EffectVolume(void) { return s_effect_volume; }
...
bool Audio_EffectSaveState(struct SDL_RWops *stream) {
  struct attr num_effects_attr =
      (struct attr){.type = TYPE_INT, .val.as_int = vec_size(&s_effects)};
