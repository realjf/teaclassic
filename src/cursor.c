...
bool Cursor_GetRTSMode(void) { return s_rts_mode; }
...
bool Cursor_LoadBMP(enum cursortype type, const char *path, int hotx,
                    int hoty) {
  if (type < 0 || type >= _CURSOR_MAX)
    return false;

  char fullpath[512];
  tc_snprintf(fullpath, sizeof(fullpath), "%s/%s", g_basepath, path);
  ...
  curr->hot_x = hotx;
  curr->hot_y = hoty;
  tc_snprintf(curr->path, sizeof(curr->path), path);
...
bool Cursor_NamedLoadBMP(const char *name, const char *path, int hotx,
                         int hoty) {
  char fullpath[512];
  tc_snprintf(fullpath, sizeof(fullpath), "%s/%s", g_basepath, path);
  ...
  struct cursor_resource entry = (struct cursor_resource){
      .cursor = cursor, .surface = surface, .hot_x = hotx, .hot_y = hoty};
  tc_snprintf(entry.path, sizeof(entry.path), path);

  int status;
  khiter_t k = kh_put(cursor, s_named_cursors, tc_strdup(name), &status);
  if (status == -1)
    goto fail;
...
bool Cursor_SaveState(struct SDL_RWops *stream) {
  struct attr rts_mode =
      (struct attr){.type = TYPE_BOOL, .val.as_bool = s_rts_mode};
  CHK_TRUE_RET(Attr_Write(stream, &rts_mode, "rts_mode"));

  struct attr ncursors =
      (struct attr){.type = TYPE_INT, .val.as_int = kh_size(s_named_cursors)};
  ...
  kh_foreach(s_named_cursors, key, curr, {
    ...
    tc_strlcpy(cursor_name.val.as_string, key,
               sizeof(cursor_name.val.as_string));
    CHK_TRUE_RET(Attr_Write(stream, &cursor_name, "cursor_name"));

    struct attr cursor_path = (struct attr){
        .type = TYPE_STRING,
    };
    tc_strlcpy(cursor_path.val.as_string, curr.path,
               sizeof(cursor_path.val.as_string));
    CHK_TRUE_RET(Attr_Write(stream, &cursor_path, "cursor_path"));

    struct attr hotx =
        (struct attr){.type = TYPE_INT, .val.as_int = curr.hot_x};
    CHK_TRUE_RET(Attr_Write(stream, &hotx, "hotx"));

    struct attr hoty =
        (struct attr){.type = TYPE_INT, .val.as_int = curr.hot_y};
  ...
  struct attr cursortype = (struct attr){.type = TYPE_INT, .val.as_int = type};
  ...
  struct attr nsystem_attr =
      (struct attr){.type = TYPE_INT, .val.as_int = nsystem};
  ...
  for (int i = 0; i < ARR_SIZE(s_cursors); i++) {
    ...
    struct attr type = (struct attr){.type = TYPE_INT, .val.as_int = i};
    CHK_TRUE_RET(Attr_Write(stream, &type, "type"));

    struct attr path = (struct attr){
        .type = TYPE_STRING,
    };
    tc_snprintf(path.val.as_string, sizeof(path.val.as_string), curr->path);
