...
static bool pstate_init(struct perf_state *out, const char *name) {
  ...
  tc_strlcpy(out->name, name, sizeof(out->name));
  out->perf_tree_idx = 0;
  return true;
...
static void pstate_destroy(struct perf_state *in) {
  ...
  kh_foreach(in->id_name_table, key, curr, { free((char *)curr); });
...
void Perf_Shutdown(void) {
  ...
  kh_foreach(s_thread_state_table, key, curr, { pstate_destroy(&curr); });
  kh_destroy(pstate, s_thread_state_table);
}
...
void Perf_Push(const char *name) {
  ...
  uint32_t parent_idx =
      ssize > 0 ? vec_AT(&ps->perf_stack, ssize - 1) : PARENT_NONE;

  vec_perf_push(&ps->perf_trees[ps->perf_tree_idx],
                (struct perf_entry){.pc_delta = SDL_GetPerformanceCounter(),
...
void Perf_PushGPU(const char *name, uint32_t cookie) {
  ...
  vec_perf_push(&ps->perf_trees[ps->perf_tree_idx],
                (struct perf_entry){.begin.gpu_cookie = cookie,
                                    .parent_idx = parent_idx,
                                    .name_id = name_id_get(name, ps)});
...
void Perf_BeginTick(void) {
  ...
  for (int i = 0; i < vec_size(&gpu_ps->perf_trees[write_idx]); i++) {
    ...
    R_PushCmd((struct rcmd){.func = R_GL_TimestampForCookie,
                            .nargs = 2,
                            .args = {
                                &pe->begin.gpu_cookie,
                                &pe->begin.gpu_ts,
...
void Perf_FinishTick(void) {
  ...
  for (khiter_t k = kh_begin(s_thread_state_table);
       k != kh_end(s_thread_state_table); k++) {
...
size_t Perf_Report(size_t maxout, struct perf_info **out) {
  ...
  for (khiter_t k = kh_begin(s_thread_state_table);
    ...
    struct perf_info *info =
        malloc(sizeof(struct perf_info) +
               vec_size(&ps->perf_trees[read_idx]) * sizeof(info->entries[0]));
    if (!info)
      break;

    tc_strlcpy(info->threadname, ps->name, sizeof(info->threadname));
