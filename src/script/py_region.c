...
typedef struct {
  PyObject_HEAD enum region_type type;
  const char *name;
} PyRegionObject;
...
static PyObject *PyRegion_pickle(PyRegionObject *self, PyObject *args,
                                 PyObject *kwargs);
static PyObject *PyRegion_unpickle(PyObject *cls, PyObject *args,
                                   PyObject *kwargs);

static PyObject *PyRegion_get_pos(PyRegionObject *self, void *closure);
static int PyRegion_set_pos(PyRegionObject *self, PyObject *value,
                            void *closure);
static PyObject *PyRegion_get_shown(PyRegionObject *self, void *closure);
static int PyRegion_set_shown(PyRegionObject *self, PyObject *value,
                              void *closure);
...
static PyMethodDef PyRegion_methods[] = {
    {"curr_ents", (PyCFunction)PyRegion_curr_ents, METH_NOARGS,
     "Get a list of all the entities currently within the region."},

    {"contains", (PyCFunction)PyRegion_contains, METH_VARARGS,
     "Returns True if the specified entity is currently within the region."},

    {"explore", (PyCFunction)PyRegion_explore, METH_VARARGS,
     "Explore the Fog of War in the region for the specified faction."},

    {"__pickle__", (PyCFunction)PyRegion_pickle, METH_KEYWORDS,
     "Serialize a Permafrost Engine region object to a string."},

    {"__unpickle__", (PyCFunction)PyRegion_unpickle,
     METH_VARARGS | METH_KEYWORDS | METH_CLASS,
     "Create a new tc.Region instance from a string earlier returned from a "
     "__pickle__ method."
...
static PyGetSetDef PyRegion_getset[] = {
    {"position", (getter)PyRegion_get_pos, (setter)PyRegion_set_pos,
     "The current worldspace position of the region", NULL},
    {"shown", (getter)PyRegion_get_shown, (setter)PyRegion_set_shown,
     "Boolean to control whether the region is rendered on the map surface",
     NULL},
    {"name", (getter)PyRegion_get_name, NULL, "The name of the region.", NULL},
    {"type", (getter)PyRegion_get_type, NULL,
     "The type (tc.REGION_CIRCLE or tc.REGION_RECTANGLE) of the region.", NULL},
    {"parameters", (getter)PyRegion_get_parameters, NULL,
...
static PyTypeObject PyRegion_type = {
    PyVarObject_HEAD_INIT(NULL, 0) "tc.Region", /* tp_name */
    ...
    "in its' constructor:                                           \n"
    "                                                               \n"
    "  - type {tc.REGION_CIRCLE, tc.REGION_RECTANGLE}               \n"
    "  - name (string)                                              \n"
...
static PyObject *PyRegion_new(PyTypeObject *type, PyObject *args,
                              PyObject *kwds) {
  static char *kwlist[] = {"type",   "name",       "position",
                           "radius", "dimensions", NULL};
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "is(ff)|f(ff)", kwlist, &regtype,
                                   &name, &position.x, &position.z, &radius,
                                   &dims.x, &dims.z)) {
  ...
  if (regtype != REGION_CIRCLE && regtype != REGION_RECTANGLE) {
    PyErr_SetString(PyExc_TypeError,
                    "regtype keyword argument must be one of "
                    "{tc.REGION_CIRCLE, tc.REGION_RECTANGLE}.");
    return NULL;
  }

  if (regtype == REGION_CIRCLE && (PyDict_GetItemString(kwds, "dimensions") ||
                                   !PyDict_GetItemString(kwds, "radius"))) {
    PyErr_SetString(PyExc_TypeError,
                    "CIRCLE regions must have a radius but no dimensions.");
    return NULL;
  }

  if (regtype == REGION_RECTANGLE &&
      (!PyDict_GetItemString(kwds, "dimensions") ||
       PyDict_GetItemString(kwds, "radius"))) {
    PyErr_SetString(PyExc_TypeError,
                    "RECTANGLE regions must have dimensions but no radius.");
    return NULL;
  }
  ...
  const char *copy = tc_strdup(name);
  if (!copy) {
    Py_DECREF(self);
    type->tp_free(self);
  ...
  if (!status) {
    ...
    tc_snprintf(errbuff, sizeof(errbuff),
                "Unable to create region (%s) of type (%d)\n", copy, regtype);
...
static PyObject *PyRegion_contains(PyRegionObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O", &obj) || !S_Entity_Check(obj)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must be a single tc.Entity instance.");
    return NULL;
  }
...
static PyObject *PyRegion_explore(PyRegionObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "i", &faction_id)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must be a single integer (faction ID).");
    return NULL;
  }
...
static PyObject *PyRegion_unpickle(PyObject *cls, PyObject *args,
  ...
  if (!type || !name) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Could not unpickle internal state of tc.Region instance");
    goto fail_unpickle;
  }

  if (!PyInt_Check(type)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unpickled 'type' field must be an integer type");
    goto fail_unpickle;
  }

  if (!PyString_Check(name)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unpickled 'name' field must be a string");
    goto fail_unpickle;
  }

  reg =
      (PyRegionObject *)((PyTypeObject *)cls)->tp_alloc((PyTypeObject *)cls, 0);
  if (!reg) {
    assert(PyErr_Occurred());
    goto fail_unpickle;
  }
  ...
  reg->name = tc_strdup(PyString_AS_STRING(name));
...
void S_Region_Shutdown(void) { kh_destroy(PyObject, s_name_pyobj_table); }

void S_Region_Clear(void) { Py_CLEAR(s_loaded); }
...
script_opaque_t S_Region_ObjFromAtts(const char *name, int type, vec2_t pos,
  ...
  switch (type) {
  case REGION_CIRCLE:
    kwargs = Py_BuildValue("{s:i, s:s, s:(ff), s:f}", "type", type, "name",
                           name, "position", pos.x, pos.z, "radius", radius);
    break;
  case REGION_RECTANGLE:
    kwargs =
        Py_BuildValue("{s:i, s:s, s:(ff), s:(ff)}", "type", type, "name", name,
                      "position", pos.x, pos.z, "dimensions", xlen, zlen);
    break;
  default:
    assert(0);
  }
