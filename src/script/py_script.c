...
static PyObject *PyPf_load_map(PyObject *self, PyObject *args,
                               PyObject *kwargs);
static PyObject *PyPf_load_map_string(PyObject *self, PyObject *args,
                                      PyObject *kwargs);
...
static PyObject *PyPf_set_minimap_render_all_ents(PyObject *self,
                                                  PyObject *args);
...
static PyObject *PyPf_set_storage_site_ui_border_color(PyObject *self,
                                                       PyObject *args);
static PyObject *PyPf_set_storage_site_ui_font_color(PyObject *self,
                                                     PyObject *args);
...
static PyObject *PyPf_nearest_ent(PyObject *self, PyObject *args,
                                  PyObject *kwargs);
static PyObject *PyPf_ents_in_circle(PyObject *self, PyObject *args,
                                     PyObject *kwargs);
static PyObject *PyPf_ents_in_rect(PyObject *self, PyObject *args,
                                   PyObject *kwargs);
...
static PyObject *PyPf_play_global_effect(PyObject *self, PyObject *args,
                                         PyObject *kwargs);
...
static PyMethodDef tc_module_methods[] = {

    {"load_map", (PyCFunction)PyPf_load_map, METH_VARARGS | METH_KEYWORDS,
     "Loads the map from the specified file."},

    {"load_map_string", (PyCFunction)PyPf_load_map_string,
     METH_VARARGS | METH_KEYWORDS,
     "Loads the map from the specified TCMAP string."},

    {"set_ambient_light_color", (PyCFunction)PyPf_set_ambient_light_color,
     METH_VARARGS,
     "Sets the global ambient light color (specified as an RGB multiplier) for "
     "the scene."},

    {"set_emit_light_color", (PyCFunction)PyPf_set_emit_light_color,
     METH_VARARGS,
     "Sets the color (specified as an RGB multiplier) for the global light "
     "source."},

    {"set_emit_light_pos", (PyCFunction)PyPf_set_emit_light_pos, METH_VARARGS,
     "Sets the position (in XYZ worldspace coordinates)"},

    {"load_scene", (PyCFunction)PyPf_load_scene, METH_VARARGS | METH_KEYWORDS,
     "Import list of entities from a TCSCENE file (specified as a path "
     "string). "
     "Returns a tuple of the following items: list of loaded entities, list of "
     "loaded regions."},

    {"register_event_handler", (PyCFunction)PyPf_register_event_handler,
     METH_VARARGS,
     "Adds a script event handler to be called when the specified global event "
     "occurs. "
     "Any weakref user arguments are automatically unpacked before being "
     "passed to the handler."},

    {"register_ui_event_handler", (PyCFunction)PyPf_register_ui_event_handler,
     METH_VARARGS,
     "Same as 'register_event_handler' but the handler will also be run when "
     "the simulation state "
     "is tc.G_PAUSED_UI_RUNNING. This is for UI callbacks that should still be "
     "invoked even when "
     "the game is in a 'paused' state."},

    {"unregister_event_handler", (PyCFunction)PyPf_unregister_event_handler,
     METH_VARARGS,
     "Removes a script event handler added by 'register_event_handler'."},

    {"global_event", (PyCFunction)PyPf_global_event, METH_VARARGS,
    ...
    {"get_ticks", (PyCFunction)PyPf_get_ticks, METH_NOARGS,
     "Get the current number of game ticks (milliseconsd) - only useful in "
     "finding elapsed "
     "times in relation to other ticks."},

    {"ticks_delta", (PyCFunction)PyPf_ticks_delta, METH_VARARGS,
     "Compute the MS delta between two tick values retried by 'get_ticks'. "
     "This handles unsigned overflow "
     "unlike regular Python integer arithmnetic."},

    {"flush_tasks", (PyCFunction)PyPf_flush_tasks, METH_VARARGS,
     "Run every single active scripting task until it either gets blocked or "
     "completes. Useful mainly "
     "for serialization during initialization."},

    {"get_active_camera", (PyCFunction)PyPf_get_active_camera, METH_NOARGS,
     "Get a tc.Camera object describing the active camera from whose point of "
     "view the scene is currently rendered."},

    {"set_active_camera", (PyCFunction)PyPf_set_active_camera, METH_VARARGS,
     "Set a tc.Camera object to be the active camera from whose point of view "
     "the scene is currently rendered."},

    {"prev_frame_ms", (PyCFunction)PyPf_prev_frame_ms, METH_NOARGS,
     "Get the duration of the previous game frame in milliseconds."},

    {"prev_frame_perfstats", (PyCFunction)PyPf_prev_frame_perfstats,
     METH_NOARGS,
     "Get a dictionary of the performance data for the previous frame."},

    {"get_resolution", (PyCFunction)PyPf_get_resolution, METH_NOARGS,
     "Get the currently set resolution of the game window."},

    {"get_native_resolution", (PyCFunction)PyPf_get_native_resolution,
     METH_NOARGS, "Returns the native resolution of the active monitor."},

    {"get_basedir", (PyCFunction)PyPf_get_basedir, METH_NOARGS,
     "Get the path to the top-level game resource folder (parent of "
     "'assets')."},

    {"get_render_info", (PyCFunction)PyPf_get_render_info, METH_NOARGS,
    ...
    {"get_nav_perfstats", (PyCFunction)PyPf_get_nav_perfstats, METH_NOARGS,
     "Returns a dictionary holding various performance couners for the "
     "navigation subsystem."},

    {"get_mouse_pos", (PyCFunction)PyPf_get_mouse_pos, METH_NOARGS,
     "Get the (x, y) cursor position on the screen."},

    {"mouse_over_ui", (PyCFunction)PyPf_mouse_over_ui, METH_NOARGS,
     "Returns True if the mouse cursor is within the bounds of any UI "
     "windows."},

    {"ui_text_edit_has_focus", (PyCFunction)PyPf_ui_text_edit_has_focus,
     METH_NOARGS,
     "Returns True if the mouse cursor is currently in an editable text field "
     "of a UI window, "
     "such that the field records the keystrokes."},

    {"get_active_window", (PyCFunction)PyPf_get_active_window, METH_NOARGS,
     "Get the currently active (having focus) window, or None."},

    {"get_file_size", (PyCFunction)PyPf_get_file_size, METH_VARARGS,
     "Get the size (in bytes) of a Python file object."},

    {"shift_pressed", (PyCFunction)PyPf_shift_pressed, METH_NOARGS,
     "Returns True if either of the SHIFT keys are currently pressed."},

    {"ctrl_pressed", (PyCFunction)PyPf_ctrl_pressed, METH_NOARGS,
     "Returns True if either of the CTRL keys are currently pressed."},

    {"get_key_name", (PyCFunction)PyPf_get_key_name, METH_VARARGS,
     "Returns the string name for an SDL_Keycode integer value."},

    {"get_active_font", (PyCFunction)PyPf_get_active_font, METH_NOARGS,
     "Get the name of the current active font."},

    {"set_active_font", (PyCFunction)PyPf_set_active_font, METH_VARARGS,
     "Set the current active font to that of the specified name."},

    {"show_regions", (PyCFunction)PyPf_show_regions, METH_NOARGS,
     "Show region outlines and names over the map surface."},

    {"hide_regions", (PyCFunction)PyPf_hide_regions, METH_NOARGS,
     "Hide region outlines and names."},

    {"enable_fog_of_war", (PyCFunction)PyPf_enable_fog_of_war, METH_NOARGS,
     "Enable the fog of war."},

    {"disable_fog_of_war", (PyCFunction)PyPf_disable_fog_of_war, METH_NOARGS,
     "Disable the fog of war."},

    {"explore_map", (PyCFunction)PyPf_explore_map, METH_VARARGS,
     "Set the entire map as having being 'explored' for a particular faction."},

    {"enable_unit_selection", (PyCFunction)PyPf_enable_unit_selection,
     METH_NOARGS,
     "Make it possible to select units with the mouse. Enable drawing of a "
     "selection box when dragging the mouse."},

    {"disable_unit_selection", (PyCFunction)PyPf_disable_unit_selection,
     METH_NOARGS,
     "Make it impossible to select units with the mouse. Disable drawing of a "
     "selection box when dragging the mouse."},

    {"clear_unit_selection", (PyCFunction)PyPf_clear_unit_selection,
     METH_NOARGS, "Clear the current unit seleciton."},

    {"get_unit_selection", (PyCFunction)PyPf_get_unit_selection, METH_NOARGS,
     "Returns a list of objects currently selected by the player."},

    {"set_unit_selection", (PyCFunction)PyPf_set_unit_selection, METH_VARARGS,
     "Overwrites the list of objects currently selected by the player with the "
     "specified entity list."},

    {"get_hovered_unit", (PyCFunction)PyPf_get_hovered_unit, METH_NOARGS,
     "Get the closest unit under the mouse cursor, or None."},

    {"entities_for_tag", (PyCFunction)PyPf_entities_for_tag, METH_VARARGS,
     "Get a tuple of entities that have the specific tag."},

    {"hide_healthbars", (PyCFunction)PyPf_hide_healthbars, METH_NOARGS,
     "Disable rendering of healthbars. Overrides the user-configurable dynamic "
     "setting."},

    {"show_healthbars", (PyCFunction)PyPf_show_healthbars, METH_NOARGS,
     "Re-enable showing the healthbars after a 'hide_healthbars' call. Note "
     "that the healthbars will only "
     "be rendered if the corresponding user-configurable setting is set."},

    {"get_resource_list", (PyCFunction)PyPf_get_resource_list, METH_NOARGS,
     "Returns a list of the names of all the resources that are present (or "
     "have ever been present) in the current session."},

    {"get_resource_stored", (PyCFunction)PyPf_get_resource_stored, METH_VARARGS,
     "Returns the total amount of a particular resource between all "
     "player-controlled storage sites."},

    {"get_resource_capacity", (PyCFunction)PyPf_get_resource_capacity,
     METH_VARARGS,
     "Returns the total capacity for storing a particular resource between all "
     "player-controlled storage sites."},

    {"get_factions_list", (PyCFunction)PyPf_get_factions_list, METH_NOARGS,
     "Returns a list of descriptors (dictionaries) for each faction in the "
     "game."},

    {"add_faction", (PyCFunction)PyPf_add_faction, METH_VARARGS,
     "Add a new faction with the specified name and color. By default, this "
     "faction is mutually at peace with "
     "every other existing faction. By default, new factions are "
     "player-controllable."},

    {"remove_faction", (PyCFunction)PyPf_remove_faction, METH_VARARGS,
     "Remove the faction with the specified faction_id. This will remove all "
     "entities belonging to that faction. "
     "This may change the values of some other entities' faction_ids."},

    {"update_faction", (PyCFunction)PyPf_update_faction, METH_VARARGS,
     "Updates the name and color of the faction with the specified "
     "faction_id."},

    {"set_faction_controllable", (PyCFunction)PyPf_set_faction_controllable,
     METH_VARARGS,
     "Sets whether units of this faction can be controlled by the player or "
     "not."},

    {"set_diplomacy_state", (PyCFunction)PyPf_set_diplomacy_state, METH_VARARGS,
     "Symmetrically sets the diplomacy state between two distinct factions "
     "(passed in as IDs)."},

    {"get_diplomacy_state", (PyCFunction)PyPf_get_diplomacy_state, METH_VARARGS,
     "Query the diplomacy state of the specified two faction IDs."},

    {"get_tile", (PyCFunction)PyPf_get_tile, METH_VARARGS,
     "Get the tc.Tile object describing the tile at the specified "
     "coordinates."},

    {"update_tile", (PyCFunction)PyPf_update_tile, METH_VARARGS,
     "Update the map tile at the specified coordinates to the new value."},

    {"set_map_highlight_size", (PyCFunction)PyPf_set_map_highlight_size,
     METH_VARARGS,
     "Determines how many tiles around the currently hovered tile are "
     "highlighted. (0 = none, "
     "1 = single tile highlighted, 2 = 3x3 grid highlighted, etc.)"},

    {"get_minimap_position", (PyCFunction)PyPf_get_minimap_position,
     METH_NOARGS,
     "Returns the current minimap position in virtual screen coordinates."},

    {"set_minimap_position", (PyCFunction)PyPf_set_minimap_position,
     METH_VARARGS,
     "Set the center position of the minimap in virtual screen coordinates."},

    {"set_minimap_resize_mask", (PyCFunction)PyPf_set_minimap_resize_mask,
     METH_VARARGS,
     "Set the anchor points for the minimap, to control its bounds as the "
     "screen resizes."},

    {"get_minimap_size", (PyCFunction)PyPf_get_minimap_size, METH_NOARGS,
     "Get the center position of the minimap in virtual screen coordinates."},

    {"set_minimap_size", (PyCFunction)PyPf_set_minimap_size, METH_VARARGS,
     "Set the center position of the minimap in virtual screen coordinates. "
     "A size of 0 fully hides the minimap."},

    {"set_minimap_border_clr", (PyCFunction)PyPf_set_minimap_border_clr,
     METH_VARARGS, "Set the border color for the minimap."},
    ...
    {"mouse_over_minimap", (PyCFunction)PyPf_mouse_over_minimap, METH_NOARGS,
     "Returns true if the mouse cursor is over the minimap, false otherwise."},

    {"map_height_at_point", (PyCFunction)PyPf_map_height_at_point, METH_VARARGS,
     "Returns the Y-dimension map height at the specified XZ coordinate. "
     "Returns None if the "
     "specified coordinate is outside the map bounds."},

    {"map_nearest_pathable", (PyCFunction)PyPf_map_nearest_pathable,
     METH_VARARGS,
     "Returns the closest XZ map position that is pathable and not currently "
     "blocked."},

    {"map_pos_under_cursor", (PyCFunction)PyPf_map_pos_under_cursor,
     METH_NOARGS,
     "Returns the XYZ coordinate of the point of the map underneath the "
     "cursor. Returns 'None' if "
     "the cursor is not over the map."},

    {"set_move_on_left_click", (PyCFunction)PyPf_set_move_on_left_click,
     METH_NOARGS,
     "Set the cursor to target mode. The next left click will issue a move "
     "command to the location "
     "under the cursor."},

    {"set_attack_on_left_click", (PyCFunction)PyPf_set_attack_on_left_click,
     METH_NOARGS,
     "Set the cursor to target mode. The next left click will issue an attack "
     "command to the location "
     "under the cursor."},

    {"set_build_on_left_click", (PyCFunction)PyPf_set_build_on_left_click,
     METH_NOARGS,
     "Set the cursor to target mode. The next left click will issue a build "
     "command to the building "
     "under the cursor."},

    {"set_gather_on_left_click", (PyCFunction)PyPf_set_gather_on_left_click,
     METH_NOARGS,
     "Set the cursor to target mode. The next left click will issue a gather "
     "command to the resource "
     "under the cursor."},

    {"set_pick_up_on_left_click", (PyCFunction)PyPf_set_pick_up_on_left_click,
     METH_NOARGS,
     "Set the cursor to target mode. The next left click will issue a 'pick "
     "up' command to the storage site "
     "under the cursor."},

    {"set_drop_off_on_left_click", (PyCFunction)PyPf_set_drop_off_on_left_click,
     METH_NOARGS,
     "Set the cursor to target mode. The next left click will issue a 'drop "
     "off' command to the storage site "
     "under the cursor."},

    {"set_transport_on_left_click",
     (PyCFunction)PyPf_set_transport_on_left_click, METH_NOARGS,
     "Set the cursor to target mode. The next left click will issue a "
     "'transport' command to the storage site "
     "under the cursor."},

    {"set_click_move_enabled", (PyCFunction)PyPf_set_click_move_enabled,
     METH_VARARGS, "Enable or disable issuing move orders by right-clicking."},

    {"draw_text", (PyCFunction)PyPf_draw_text, METH_VARARGS,
     "Draw a text label with the specified bounds (X, Y, W, H) )and with the "
     "specified color (R, G, B, A). "
     "The label lasts for one frame, meaning this should be called every tick "
     "to keep the label fixed."},

    {"set_storage_site_ui_style", (PyCFunction)PyPf_set_storage_site_ui_style,
     METH_VARARGS,
     "Set the storage site UI background to the specified color (R, G, B, A) "
     "or image (string)."},

    {"set_storage_site_ui_border_color",
     (PyCFunction)PyPf_set_storage_site_ui_border_color, METH_VARARGS,
     "Set the storage site UI border color to the specified color (R, G, B, "
     "A)."},
    ...
    {"storage_site_show_ui", (PyCFunction)PyPf_storage_site_show_ui,
     METH_VARARGS,
     "Set a flag controlling whether or not the UI windows are rendered over "
     "the storage sites."},

    {"settings_get", (PyCFunction)PyPf_settings_get, METH_VARARGS,
     "Returns the value of the setting with the specified name. Will throw an "
     "exception if the setting is "
     "not found."},

    {"settings_set", (PyCFunction)PyPf_settings_set,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"settings_create", (PyCFunction)PyPf_settings_create, METH_VARARGS,
     "Create a new setting, the value of which will be saved in the settings "
     "file and will be accessible "
     "in another session. Settings may hold the following types: int, float, "
     "string, bool, and tuple "
     "of 2 floats (vec2). Setting names beginning with 'tc' are reserved for "
     "the engine."},

    {"settings_delete", (PyCFunction)PyPf_settings_delete, METH_VARARGS,
     "Delete a setting with the specified name. Setting names beginning with "
     "'tc' are reserved for the "
     "engine and may not be deleted."},

    {"settings_flush", (PyCFunction)PyPf_settings_flush, METH_VARARGS,
     "Write the current settings to the settings file."},

    {"get_simstate", (PyCFunction)PyPf_get_simstate, METH_NOARGS,
     "Returns the current simulation state."},

    {"set_simstate", (PyCFunction)PyPf_set_simstate, METH_VARARGS,
     "Set the current simulation state."},

    {"set_system_cursor", (PyCFunction)PyPf_set_system_cursor, METH_VARARGS,
     "Set a BMP for one of the cursors used by the engine core."},

    {"set_named_cursor", (PyCFunction)PyPf_set_named_cursor, METH_VARARGS,
     "Load a BMP cursor and associate it with a particular name."},

    {"activate_system_cursor", (PyCFunction)PyPf_activate_system_cursor,
     METH_VARARGS,
     "Make a cursor (specified by CURSOR enum type) the currently displayed "
     "cursor."},

    {"activate_named_cursor", (PyCFunction)PyPf_activate_named_cursor,
     METH_VARARGS,
     "Make a cursor (specified by name) the currently displayed cursor."},

    {"set_cursor_rts_mode", (PyCFunction)PyPf_set_cursor_rts_mode, METH_VARARGS,
     "Activates or deactivates the cursor RTS mode which keeps the cursor icon "
     "updated based on which edge "
     "of the screen it is at."},

    {"get_cursor_rts_mode", (PyCFunction)PyPf_get_cursor_rts_mode, METH_NOARGS,
     "Returns the current state of the RTS cursor mode enablement."},

    {"multiply_quaternions", (PyCFunction)PyPf_multiply_quaternions,
     METH_VARARGS,
     "Returns the normalized result of multiplying 2 quaternions (specified as "
     "a list of 4 floats - XYZW order)."},

    {"rand", (PyCFunction)PyPf_rand, METH_VARARGS,
     "Return a pseudo-random number in the range of 0 to the integer "
     "argument."},

    {"pickle_object", (PyCFunction)PyPf_pickle_object, METH_VARARGS,
     "Returns an ASCII string holding the serialized representation of the "
     "object graph."},

    {"unpickle_object", (PyCFunction)PyPf_unpickle_object, METH_VARARGS,
     "Returns a new reference to an object built from its' serialized "
     "representation. The argument string must "
     "an earlier return value of 'tc.pickle_object'."},

    {"save_session", (PyCFunction)PyPf_save_session, METH_VARARGS,
     "Save the current state of the engine to the specified file. The session "
     "can then be loaded "
     "from the file with the 'load_session' call."},

    {"load_session", (PyCFunction)PyPf_load_session, METH_VARARGS,
     "Load a session previously saved with the 'save_session' call."},

    {"exec_", (PyCFunction)PyPf_exec, METH_VARARGS,
     "Replace the current subsession with one set up by the provided script. "
     "This is performed asynchronously. Failure is notified via an "
     "EVENT_SESSION_FAIL_LOAD event."},

    {"exec_push", (PyCFunction)PyPf_exec_push, METH_VARARGS,
     "Replace the current subsession with one set up by the provided script, "
     "saving the current subsession onto a stack. "
     "This is performed asynchronously. Failure is notified via an "
     "EVENT_SESSION_FAIL_LOAD event."},

    {"exec_pop", (PyCFunction)PyPf_exec_pop, METH_VARARGS,
     "Pop a previously saved subsession, using it to replace the current "
     "subsession. "
     "This is performed asynchronously. Failure is notified via an "
     "EVENT_SESSION_FAIL_LOAD event."},

    {"exec_pop_to_root", (PyCFunction)PyPf_exec_pop_to_root, METH_VARARGS,
     "Pop the root subsession, using it to replace the current subsession. "
     "This is performed asynchronously. Failure is notified via an "
     "EVENT_SESSION_FAIL_LOAD event."},

    {"session_stack_depth", (PyCFunction)PyPf_session_stack_depth, METH_VARARGS,
     "Returns the number of sessions currently on the sessin stack."},

    {"nearest_ent", (PyCFunction)PyPf_nearest_ent, METH_VARARGS | METH_KEYWORDS,
     "Returns the nearest entity to the specified 'position' - (X, Z) point or "
     "None. Takes an optional 'predicate' callable "
     "argument to filter the results and an optional 'max_range' (Float) "
     "argument."},

    {"ents_in_circle", (PyCFunction)PyPf_ents_in_circle,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"ents_in_rect", (PyCFunction)PyPf_ents_in_rect,
     METH_VARARGS | METH_KEYWORDS,
     "Returns a list of entities in the specified rectangle (defined by two "
     "(X, Z) points - "
     "the 'minimum' and 'maximum' corners. Takes an optional 'predicate' "
     "callable argument to "
     "filter the results."},

    {"play_music", (PyCFunction)PyPf_play_music, METH_VARARGS,
     "Set the specified audio track to loop in the background. The argument "
     "must be a name of a WAV file in the "
     "'assets/music' folder, without the file extension."},

    {"curr_music", (PyCFunction)PyPf_curr_music, METH_VARARGS,
     "Return the name of the currently playing music track, or None."},

    {"get_all_music", (PyCFunction)PyPf_get_all_music, METH_NOARGS,
     "Get a list of all the currently loaded music track names."},

    {"play_effect", (PyCFunction)PyPf_play_effect, METH_VARARGS,
     "Play a specified audio effect at the specified (X, Y, Z) position. The "
     "name must be a name of a WAV file in the "
     "'assets/sounds' folder, without the file extension."},

    {"play_global_effect", (PyCFunction)PyPf_play_global_effect,
     METH_VARARGS | METH_KEYWORDS,
     "Play a specified audio effect with global range. The name must be a name "
     "of a WAV file in the "
     "'assets/sounds' folder, without the file extension. If another global "
     "effect is already playing, "
     "the effect will be ignored, unless 'interrupt' is set to True."},

    {"spawn_projectile", (PyCFunction)PyPf_spawn_projectile, METH_VARARGS,
     "Spawn a projectile with the specified parameters at a map location. The "
     "projectile will travel "
     "with the specified velocity until it leaves the map bounds or hits a "
     "unit. The hit event can be "
     "handled."},
...
static struct py_err_ctx s_err_ctx = {
    false,
};
...
static void s_on_update(void *user, void *event) { S_Error_Update(&s_err_ctx); }

static PyObject *PyPf_load_map(PyObject *self, PyObject *args,
                               PyObject *kwargs) {
  static char *kwlist[] = {"dir", "tcmap", "update_navgrid", "absolute", NULL};
  const char *dir, *tcmap;
  int update_navgrid = true;
  int absolute = false;

  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "zs|ii", kwlist, &dir, &tcmap,
                                   &update_navgrid, &absolute)) {
    PyErr_SetString(
        PyExc_TypeError,
        "The first argument must be a string or NULL (base directory). "
        "The second argument must be a string (TCMAP filepath).");
    return NULL;
  }

  char tcmap_path[512] = {[0] = '\0'};
  if (!absolute) {
    tc_strlcat(tcmap_path, g_basepath, sizeof(tcmap_path));
    tc_strlcat(tcmap_path, "/", sizeof(tcmap_path));
  }
  if (dir && strlen(dir) > 0) {
    tc_strlcat(tcmap_path, dir, sizeof(tcmap_path));
    tc_strlcat(tcmap_path, "/", sizeof(tcmap_path));
  }
  tc_strlcat(tcmap_path, tcmap, sizeof(tcmap_path));

  SDL_RWops *stream = SDL_RWFromFile(tcmap_path, "r");
  if (!stream) {
    char errbuff[256];
    tc_snprintf(errbuff, sizeof(errbuff), "Unable to open TCMap file %s",
                tcmap_path);
  ...
  if (!G_LoadMap(stream, update_navgrid)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to load the specified map file.");
    SDL_RWclose(stream);
    return NULL;
  }
...
static PyObject *PyPf_load_map_string(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|i", kwlist, &mapstr,
                                   &update_navgrid)) {
...
static PyObject *PyPf_load_scene(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|ii", kwlist, &relpath,
                                   &update_navgrid, &absolute)) {
  ...
  char full_path[512];
  if (absolute) {
    tc_strlcpy(full_path, relpath, sizeof(full_path));
  } else {
    tc_snprintf(full_path, sizeof(full_path), "%s/%s", g_basepath, relpath);
  }

  if (!Scene_Load(full_path)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to load scene from the specified file.");
    goto fail_load;
  }
...
static PyObject *register_handler(PyObject *self, PyObject *args, int simmask) {
  ...
  if (!PyArg_ParseTuple(args, "iOO", &event, &callable, &user_arg)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must a tuple of an integer and two objects.");
    return NULL;
  }
...
static PyObject *PyPf_register_ui_event_handler(PyObject *self,
                                                PyObject *args) {
...
static PyObject *PyPf_unregister_event_handler(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "iO", &event, &callable)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must a tuple of an integer and one object.");
    return NULL;
  }
  ...
  if (!ret) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Could not unregister the specified event handler.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_ticks_delta(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "kk", &a, &b)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be two integers (tick values).");
    return NULL;
  }
...
static PyObject *PyPf_prev_frame_perfstats(PyObject *self) {
  ...
  for (int i = 0; i < nthreads; i++) {
    ...
    for (int j = 0; j < curr_info->nentries; j++) {
      ...
      int parent_idx = (curr_info->entries[j].parent_idx == ~((uint32_t)0))
                           ? 0
                           : curr_info->entries[j].parent_idx + 1;
      ...
      PyObject *pc_delta =
          PyLong_FromUnsignedLongLong(curr_info->entries[j].pc_delta);
      if (!pc_delta)
        goto fail;
...
static PyObject *PyPf_get_resolution(PyObject *self) {
  struct sval res;
  ss_e status = Settings_Get("tc.video.resolution", &res);
...
static PyObject *PyPf_get_render_info(PyObject *self) {
  ...
  int rval = 0;
  rval |= PyDict_SetItemString(
      ret, "version", Py_BuildValue("s", R_GetInfo(RENDER_INFO_VERSION)));
  rval |= PyDict_SetItemString(
      ret, "vendor", Py_BuildValue("s", R_GetInfo(RENDER_INFO_VENDOR)));
  rval |= PyDict_SetItemString(
      ret, "renderer", Py_BuildValue("s", R_GetInfo(RENDER_INFO_RENDERER)));
  rval |= PyDict_SetItemString(
      ret, "shading_language_version",
      Py_BuildValue("s", R_GetInfo(RENDER_INFO_SL_VERSION)));
  assert(0 == rval);
...
static PyObject *PyPf_get_nav_perfstats(PyObject *self) {
  ...
  int rval = 0;
  rval |=
      PyDict_SetItemString(ret, "los_used", Py_BuildValue("i", stats.los_used));
  rval |=
      PyDict_SetItemString(ret, "los_max", Py_BuildValue("i", stats.los_max));
  rval |= PyDict_SetItemString(ret, "los_hit_rate",
                               Py_BuildValue("f", stats.los_hit_rate));
  rval |= PyDict_SetItemString(ret, "los_invalidated",
                               Py_BuildValue("i", stats.los_invalidated));
  rval |= PyDict_SetItemString(ret, "flow_used",
                               Py_BuildValue("i", stats.flow_used));
  rval |=
      PyDict_SetItemString(ret, "flow_max", Py_BuildValue("i", stats.flow_max));
  rval |= PyDict_SetItemString(ret, "flow_hit_rate",
                               Py_BuildValue("f", stats.flow_hit_rate));
  rval |= PyDict_SetItemString(ret, "flow_invalidated",
                               Py_BuildValue("i", stats.flow_invalidated));
  rval |= PyDict_SetItemString(ret, "ffid_used",
                               Py_BuildValue("i", stats.ffid_used));
  rval |=
      PyDict_SetItemString(ret, "ffid_max", Py_BuildValue("i", stats.ffid_max));
  rval |= PyDict_SetItemString(ret, "ffid_hit_rate",
                               Py_BuildValue("f", stats.ffid_hit_rate));
  rval |= PyDict_SetItemString(ret, "grid_path_used",
                               Py_BuildValue("i", stats.grid_path_used));
  rval |= PyDict_SetItemString(ret, "grid_path_max",
                               Py_BuildValue("i", stats.grid_path_max));
  rval |= PyDict_SetItemString(ret, "grid_path_hit_rate",
                               Py_BuildValue("f", stats.grid_path_hit_rate));
  assert(0 == rval);
...
static PyObject *PyPf_get_file_size(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O", &file) || !PyFile_Check(file)) {
...
static PyObject *PyPf_get_key_name(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "i", &keysym)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must an integer value (SDL_Keysym).");
    return NULL;
  }
...
static PyObject *PyPf_explore_map(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "i", &faction_id) ||
    ...
    PyErr_SetString(PyExc_TypeError,
                    "Argument must a valid faction ID (integer).");
    return NULL;
  }
...
static PyObject *PyPf_set_unit_selection(PyObject *self, PyObject *args) {
  ...
  for (int i = 0; i < PyList_GET_SIZE(list); i++) {
    ...
    if (!S_Entity_Check(obj)) {
      PyErr_SetString(PyExc_TypeError,
                      "Argument must a list of tc.Entity objects.");
      return NULL;
    }
    uint32_t uid;
...
static PyObject *PyPf_get_factions_list(PyObject *self) {
  ...
  for (int i = 0; facs; facs >>= 1, i++) {
    ...
    PyObject *color = Py_BuildValue("(iiii)", (int)colors[i].x,
                                    (int)colors[i].y, (int)colors[i].z, 255);
    if (!color)
      goto fail_dict;
...
static PyObject *PyPf_add_faction(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s(iiii)", &name, &color_ints[0], &color_ints[1],
                        &color_ints[2], &color_ints[3])) {
    PyErr_SetString(PyExc_TypeError, "Arguments must be a string and a tuple "
                                     "of 4 ints (RGBA color descriptor).");
    return NULL;
  }
...
static PyObject *PyPf_remove_faction(PyObject *self, PyObject *args) {
  ...
  if (!G_RemoveFaction(faction_id)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to remove the specified faction.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_update_faction(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "is(iiii)", &faction_id, &name, &color_ints[0],
                        &color_ints[1], &color_ints[2], &color_ints[3])) {
  ...
  if (!G_UpdateFaction(faction_id, name, color, controllable[faction_id])) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to update the specified faction.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_set_faction_controllable(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "iO", &faction_id, &new_controllable)) {
    ...
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be an integer and a boolean expression.");
    return NULL;
  }
  ...
  if (!G_UpdateFaction(faction_id, names[faction_id], colors[faction_id],
                       PyObject_IsTrue(new_controllable))) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to update the specified faction.");
    return NULL;
  }
...
static PyObject *PyPf_set_diplomacy_state(PyObject *self, PyObject *args) {
  ...
  if (!G_SetDiplomacyState(fac_id_a, fac_id_b, ds)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to set the diplomacy state: invalid arguments.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_get_tile(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(ii)(ii)O", &desc.chunk_r, &desc.chunk_c,
                        &desc.tile_r, &desc.tile_c)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be two tuples of two integers.");
    return NULL;
  }
...
static PyObject *PyPf_update_tile(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(ii)(ii)O", &desc.chunk_r, &desc.chunk_c,
                        &desc.tile_r, &desc.tile_c, &tile_obj)) {
    PyErr_SetString(
        PyExc_TypeError,
        "Arguments must be two tuples of two integers and a tc.Tile object.");
    return NULL;
  }

  if (NULL == (tile = S_Tile_GetTile(tile_obj))) {
    PyErr_SetString(PyExc_TypeError, "Last argument must be of type tc.Tile.");
    return NULL;
  }
...
static PyObject *PyPf_get_minimap_position(PyObject *self, PyObject *args) {
  ...
  if (!G_GetMinimapPos(&x, &y)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to get minimap position. A map must be loaded.");
    return NULL;
  }
...
static PyObject *PyPf_set_minimap_position(PyObject *self, PyObject *args) {
  ...
  if (!G_SetMinimapPos(x, y)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to set minimap position. A map must be loaded.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_set_minimap_resize_mask(PyObject *self, PyObject *args) {
  ...
  if ((resize_mask & ANCHOR_X_MASK) == 0 ||
    ...
    PyErr_SetString(PyExc_RuntimeError,
                    "Invalid reisize mask: the window must have at least one "
                    "anchor in each dimension.");
    return NULL;
  }

  if (!G_SetMinimapResizeMask(resize_mask)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to set minimap resize mask. A map must be loaded.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_get_minimap_size(PyObject *self) {
  ...
  if (!G_GetMinimapSize(&size)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to get minimap size. A map must be loaded.");
    return NULL;
  }
...
static PyObject *PyPf_set_minimap_size(PyObject *self, PyObject *args) {
  ...
  if (!G_SetMinimapSize(size)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to set minimap size. A map must be loaded.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_set_minimap_border_clr(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "iiii", &r, &g, &b, &a)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must be a tuple of 4 integers (RGBA).");
    return NULL;
  }
...
static PyObject *PyPf_set_minimap_render_all_ents(PyObject *self,
  ...
  if (!PyArg_ParseTuple(args, "O", &val)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must be a single boolean expression.");
    return NULL;
  }
...
static PyObject *PyPf_map_nearest_pathable(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(ff)", &x, &z)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be a tuple of two floats.");
    return NULL;
  }
...
static PyObject *PyPf_set_click_move_enabled(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O", &arg)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must a single boolean expression.");
    return NULL;
  }
...
static PyObject *PyPf_draw_text(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s(iiii)(iiii)|(iiii)", &text, &rect.x, &rect.y,
                        &rect.w, &rect.h, &r, &g, &b, &a, &tint_r, &tint_g,
                        &tint_b, &tint_a)) {
    PyErr_SetString(PyExc_TypeError,
                    "Expecting 3 arguments: A text string, a tuple of 4 ints "
                    "(bounds) and a tuple of 4 ints (RGBA color). "
                    "Optionally an extra RGBA tuple can be supplied to "
                    "customize the tint RGBA color.");
    return NULL;
  }
  ...
  vec2_t center = (vec2_t){rect.x + rect.w / 2.0f, rect.y + rect.h / 2.0f};
  vec2_t ndc = (vec2_t){(center.x - width / 2.0f) / (width / 2.0f),
  ...
  struct rect adj_rect =
      (struct rect){adj_center.x - rect.w / 2.0f, adj_center.y - rect.h / 2.0f,
                    rect.w, rect.h};
  struct rect shifted_rect =
      (struct rect){adj_rect.x - 1, adj_rect.y - 1, adj_rect.w, adj_rect.h};

  UI_DrawText(text, shifted_rect,
              (struct rgba){tint_r, tint_g, tint_b, tint_a});
  UI_DrawText(text, adj_rect, (struct rgba){r, g, b, a});
  Py_RETURN_NONE;
}

static PyObject *PyPf_set_storage_site_ui_style(PyObject *self,
                                                PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O", &val)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must be an (R, G, B, A) tuple or an image path.");
    return NULL;
  }
  ...
  if (PyTuple_Check(val) && PyTuple_GET_SIZE(val) == 4 &&
    ...
    tc_strlcpy(style.data.texpath, PyString_AS_STRING(val),
...
static PyObject *PyPf_storage_site_show_ui(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O", &obj)) {
    PyErr_SetString(PyExc_TypeError,
                    "Expecting a single argument (True or False expression)");
    return NULL;
  }
...
static PyObject *PyPf_settings_get(PyObject *self, PyObject *args) {
  ...
  if (status == SS_NO_SETTING) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The setting with the given name does not exist.");
    return NULL;
  }

  switch (val.type) {
  case ST_TYPE_STRING:
    return Py_BuildValue("s", val.as_string);
  case ST_TYPE_FLOAT:
    return Py_BuildValue("f", val.as_float);
  case ST_TYPE_INT:
    return Py_BuildValue("i", val.as_int);
  case ST_TYPE_BOOL:
    if (val.as_bool)
      Py_RETURN_TRUE;
    else
      Py_RETURN_FALSE;
  case ST_TYPE_VEC2:
    return Py_BuildValue("(f, f)", val.as_vec2.x, val.as_vec2.y);
  default:
    assert(0);
    Py_RETURN_NONE;
  }
...
static PyObject *PyPf_settings_set(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "sO|i", kwlist, &sname, &nvobj,
                                   &persist)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be a string and an object.");
    return NULL;
  }
  ...
  if (!sval_from_pyobj(nvobj, &newval)) {
    PyErr_SetString(
        PyExc_TypeError,
        "The new value is not one of the allowed types for settings.");
    return NULL;
  }
  ...
  if (status == SS_NO_SETTING) {
    ...
    PyErr_SetString(PyExc_RuntimeError,
                    "The new value is not allowed for this setting.");
    return NULL;
  }
...
static PyObject *PyPf_settings_create(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "sO", &name, &val)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be a string (name) and an object (value).");
    return NULL;
  }

  if (0 == strncmp(name, "tc", 2)) {
    PyErr_SetString(
        PyExc_RuntimeError,
        "Settings beginning with 'tc' are reserved for the engine.");
    return NULL;
  }
  ...
  struct setting new_sett = (struct setting){
      .val = sett_val, .prio = 2, .validate = NULL, .commit = NULL};
...
static PyObject *PyPf_settings_delete(PyObject *self, PyObject *args) {
  ...
  if (status != SS_OKAY) {
    char errstr[256];
    tc_snprintf(errstr, sizeof(errstr), "Could not delete setting. [err: %d]",
                status);
    PyErr_SetString(PyExc_RuntimeError, errstr);
    return NULL;
  }
...
static PyObject *PyPf_settings_flush(PyObject *self) {
  ...
  if (status != SS_OKAY) {
    PyErr_SetString(
        PyExc_RuntimeError,
        "Failed to save the current settings to the settings file.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_set_simstate(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "i", &ss) ||
    ...
    PyErr_SetString(
        PyExc_TypeError,
        "Argument must be an integer (valid simulation state value)");
    return NULL;
  }
...
static PyObject *PyPf_set_system_cursor(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "isii", &type, &path, &hotx, &hoty)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must be an int (CURSOR type), a string (path), "
                    "and two ints (hotx, hoty).");
    return NULL;
  }

  if (!Cursor_LoadBMP(type, path, hotx, hoty)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to load cursor image for the specified type.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_set_named_cursor(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "ssii", &name, &path, &hotx, &hoty)) {
    PyErr_SetString(PyExc_TypeError,
                    "Argument must be a string (name), a string (path), and "
                    "two ints (hotx, hoty).");
    return NULL;
  }
  ...
  if (!Cursor_NamedLoadBMP(name, path, hotx, hoty)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to load cursor image for the specified name.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_activate_system_cursor(PyObject *self, PyObject *args) {
  ...
  if (type < 0 || type >= _CURSOR_MAX) {
    PyErr_SetString(PyExc_TypeError,
                    "Invalid CURSOR type. It must be a CURSOR enum value.");
    return NULL;
  }
...
static PyObject *PyPf_activate_named_cursor(PyObject *self, PyObject *args) {
  ...
  if (!Cursor_NamedSetActive(name)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to activate cursor with the specified name.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_set_cursor_rts_mode(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O", &value)) {
    PyErr_SetString(PyExc_TypeError,
                    "Expecting a single argument: boolean expression");
    return NULL;
  }
...
static PyObject *PyPf_multiply_quaternions(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!", &PyTuple_Type, &q1_list, &PyTuple_Type,
                        &q2_list)) {
    PyErr_SetString(PyExc_TypeError, "Arguments must be two tuples.");
    return NULL;
  }

  if (PyTuple_GET_SIZE(q1_list) != 4 || PyTuple_GET_SIZE(q2_list) != 4) {
    PyErr_SetString(PyExc_TypeError, "The argument tuples must be of size 4.");
    return NULL;
  }

  if (!PyArg_ParseTuple(q1_list, "ffff", &q1.raw[0], &q1.raw[1], &q1.raw[2],
                        &q1.raw[3]))
    return NULL;
  if (!PyArg_ParseTuple(q2_list, "ffff", &q2.raw[0], &q2.raw[1], &q2.raw[2],
                        &q2.raw[3]))
    return NULL;

  TCM_Quat_MultQuat(&q1, &q2, &ret);
  TCM_Quat_Normal(&ret, &ret);
...
static PyObject *PyPf_pickle_object(PyObject *self, PyObject *args) {
  ...
  SDL_RWops *vops = TCSDL_VectorRWOps();
...
static PyObject *PyPf_save_session(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s", &str)) {
    PyErr_SetString(
        PyExc_TypeError,
        "Argument must be a string (path of the file to save the session to).");
    return NULL;
  }
  ...
  if (!file) {
    char buff[256];
    tc_snprintf(buff, sizeof(buff), "Unable to open file (%s) for writing.\n",
                str);
  ...
  SDL_RWops *stream =
      SDL_RWFromFP(file, true); /* file will be closed when stream is */
  assert(stream);
...
static PyObject *PyPf_load_session(PyObject *self, PyObject *args) {
  const char *str;
  if (!PyArg_ParseTuple(args, "s", &str)) {
    PyErr_SetString(PyExc_TypeError, "Argument must be a string (path of the "
                                     "file to load the session from).");
    return NULL;
  }
...
static int s_tracefunc(PyObject *obj, struct _frame *frame, int what,
                       PyObject *arg) {
  char name[128];

  switch (what) {
  case PyTrace_CALL: {
    tc_snprintf(name, sizeof(name), "[Py] %s",
  ...
  case PyTrace_C_CALL: {
    ...
    tc_snprintf(name, sizeof(name), "[PyC] %s", func->m_ml->ml_name);
    Perf_Push(name);
    break;
  }
...
static void trace_enable_commit(const struct sval *new_val) {
  ...
  E_Global_Register(EVENT_UPDATE_START, on_event_start,
                    (void *)((uintptr_t)new_val->as_bool),
...
static PyObject *PyPf_exec(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "sO", &scriptname, &sargs) ||
      !PyTuple_Check(sargs)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be a string and a tuple or arguments.");
    return NULL;
  }
...
static PyObject *PyPf_exec_push(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s|O", &scriptname, &sargs) ||
    ...
    PyErr_SetString(
        PyExc_TypeError,
        "Argument must be a string (and optionally a tuple of arguments).");
    return NULL;
  }
...
static bool s_pred_any(const struct entity *ent, void *arg) { return true; }
...
static PyObject *PyPf_nearest_ent(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "(ff)|Of", kwlist, &xz_pos.x,
                                   &xz_pos.z, &predicate, &max_range)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be an (X, Z) float tuple and "
  ...
  if (predicate) {
    nearest =
        G_Pos_NearestWithPred(xz_pos, s_pred_callable, predicate, max_range);
  } else {
...
static PyObject *PyPf_ents_in_circle(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "(ff)f|O", kwlist, &xz_pos.x,
                                   &xz_pos.z, &radius, &predicate)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be an (X, Z) float tuple, a float and "
                    "an optional callable object.");
    return NULL;
  }
  ...
  if (predicate) {
    ninside = G_Pos_EntsInCircleWithPred(
        xz_pos, radius, inside, ARR_SIZE(inside), s_pred_callable, predicate);
  } else {
    ninside = G_Pos_EntsInCircleWithPred(xz_pos, radius, inside,
                                         ARR_SIZE(inside), s_pred_any, NULL);
  }
...
static PyObject *PyPf_ents_in_rect(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "(ff)(ff)|O", kwlist,
                                   &xz_min.x, &xz_min.z, &xz_max.x, &xz_max.z,
                                   &predicate)) {
    PyErr_SetString(PyExc_TypeError,
                    "Arguments must be two (X, Z) float tuples, and an "
...
static PyObject *PyPf_play_music(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "z", &name)) {
    PyErr_SetString(
        PyExc_TypeError,
        "Expecting a single argument - name of the WAV file or 'None'.");
    return NULL;
  }

  if (!Audio_PlayMusic(name)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to play the specified music track.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_play_effect(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s(fff)", &name, &pos.x, &pos.y, &pos.z)) {
    PyErr_SetString(
        PyExc_TypeError,
        "Arugment must be a string (name) and a tuple of 3 floats (position).");
    return NULL;
  }
  ...
  if (!Audio_Effect_Add(pos, name)) {
    PyErr_SetString(
        PyExc_RuntimeError,
        "Unable to play the specified effect at the specified position.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_play_global_effect(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|ii", kwlist, &name,
                                   &interrupt, &channel)) {
    PyErr_SetString(PyExc_TypeError,
                    "Expecting a string (effect name), an optional boolean "
                    "(interrupt) argument, "
  ...
  if (!Audio_PlayForegroundEffect(name, interrupt, channel)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unable to play the specified global effect.");
    return NULL;
  }
  Py_RETURN_NONE;
}
...
static PyObject *PyPf_spawn_projectile(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(fff)(fff)IIII(ss(fff)f)", &origin.x, &origin.y,
                        &origin.z, &velocity.x, &velocity.y, &velocity.z,
                        &ent_parent, &faction_id, &cookie, &flags, &pd.basedir,
                        &pd.tcobj, &pd.scale.x, &pd.scale.y, &pd.scale.z,
                        &pd.speed)) {

    PyErr_SetString(PyExc_RuntimeError,
                    "The following arugment list is expected: "
                    ...
                    "projectile model TCOBJ filename (string), "
                    "projectile model scale (tuple of 3 floats), "
                    "projectile speed (float)].");
    return NULL;
  }

  uint32_t ret = P_Projectile_Add(origin, velocity, ent_parent, faction_id,
                                  cookie, flags, pd);
  return PyInt_FromLong(ret);
}
...
PyMODINIT_FUNC inittc(void) {
  PyObject *module;
  module = Py_InitModule("tc", tc_module_methods);
  if (!module)
    return;
...
bool S_Init(const char *progname, const char *base_path,
  ...
  tc_snprintf(script_dir, sizeof(script_dir), "%s/%s", g_basepath, "scripts");
  ...
  if (0 !=
      PyList_Append(PySys_GetObject("path"), Py_BuildValue("s", script_dir)))
    return false;

  tc_snprintf(script_dir, sizeof(script_dir), "%s/%s", g_basepath,
              "scripts/stdlib");
  if (0 !=
      PyList_Append(PySys_GetObject("path"), Py_BuildValue("s", script_dir)))
    return false;
  ...
  PyObject *module = PyDict_GetItemString(PySys_GetObject("modules"), "tc");
  assert(module);
  /* Initialize the pickler after registering all the built-ins, so that they
   * can be indexed. */
...
bool S_RunFile(const char *path, int argc, char **argv) {
  ...
  /* The directory of the script file won't be automatically added by
   * 'PyRun_SimpleFile'. We add it manually to sys.path ourselves. */
  ...
  PyObject *result =
      PyRun_File(script, path, Py_file_input, global_dict, global_dict);
  ret = (result != NULL);
  Py_XDECREF(result);
...
void S_RunEventHandler(script_opaque_t callable, script_opaque_t user_arg,
  ...
  args = PyTuple_New(2);
  /* PyTuple_SetItem steals references! However, we wish to hold on to the
   * user_arg. The event_arg is DECREF'd once after all the handlers for the
  ...
  /* Make sure to retain the callable object - PyObject_CallObject will not do
   * this for us. The reason is that the invoked handler may unregister itself,
   * thus removing the last living reference to it. */
  Py_INCREF(callable);
...
void S_Retain(script_opaque_t obj) { Py_XINCREF(obj); }

void S_Release(script_opaque_t obj) { Py_XDECREF(obj); }

script_opaque_t S_WrapEngineEventArg(int eventnum, void *arg) {
  switch (eventnum) {
  case SDL_KEYDOWN:
  case SDL_KEYUP:
    return Py_BuildValue("(iii)", ((SDL_Event *)arg)->key.keysym.scancode,
                         ((SDL_Event *)arg)->key.keysym.sym,
                         ((SDL_Event *)arg)->key.keysym.mod);

  case SDL_MOUSEMOTION:
    return Py_BuildValue("(i,i), (i,i)", ((SDL_Event *)arg)->motion.x,
                         ((SDL_Event *)arg)->motion.y,
                         ((SDL_Event *)arg)->motion.xrel,
                         ((SDL_Event *)arg)->motion.xrel);

  case SDL_MOUSEBUTTONDOWN:
    return Py_BuildValue("(i, i)", ((SDL_Event *)arg)->button.button,
  ...
  case EVENT_SELECTED_TILE_CHANGED:
    if (!arg)
      Py_RETURN_NONE;
    return Py_BuildValue("(i,i), (i,i)", ((struct tile_desc *)arg)->chunk_r,
...
bool S_SaveState(SDL_RWops *stream) {
  ...
  struct script_handler *handlers =
      malloc(max_handlers * sizeof(struct script_handler));
  if (!handlers)
    return false;
  ...
  for (int i = 0; i < nhandlers; i++) {
    PyObject *val = Py_BuildValue(
        "lllOO", handlers[i].event, handlers[i].id, handlers[i].simmask,
        (PyObject *)handlers[i].handler, (PyObject *)handlers[i].arg);
    if (!val)
      goto fail_handlers;
  ...
  PyObject *state =
      Py_BuildValue("OOOOOOOOO", interp->modules, interp->sysdict,
                    interp->builtins, interp->modules_reloading,
                    interp->codec_search_path, interp->codec_search_cache,
                    interp->codec_error_registry, saved_handlers, tasks);
  if (!state)
    goto fail_state;
...
bool S_LoadState(SDL_RWops *stream) {
  ...
  for (int i = 0; i < PyTuple_GET_SIZE(handlers); i++) {
    ...
    if (!PyInt_Check(event) || !PyInt_Check(uid) || !PyInt_Check(simmask) ||
...
void S_ShowLastError(void) {
  ...
  PyErr_NormalizeException(&s_err_ctx.type, &s_err_ctx.value,
                           &s_err_ctx.traceback);
