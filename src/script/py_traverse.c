...
__KHASH_IMPL(str, extern, khint64_t, const char *, 1, kh_int_hash_func,
             kh_int_hash_equal)
__KHASH_IMPL(pobj, extern, kh_cstr_t, PyObject *, 1, kh_str_hash_func,
             kh_str_hash_equal)
...
static void s_traverse_df(PyObject *root, visitproc visit,
  ...
  for (int i = 0; i < PyList_Size(attrs); i++) {
     ...
     * means that it was a brand new object derived to satisfy this very
     * attribute lookup. An example of this is a long object's 'denominator'
     * method which returns a brand new heap-allocated PyObject to satisfy the
     ...
     * trapped in a cycle of infinite recursion if we traverse down it. So,
     * don't recurse down 'derived' attributes, which are fulfilled with a
     * unique object on each lookup.
     */
...
static void s_traverse_bf(PyObject *root, visitproc visit,
                          struct visit_ctx *ctx) {
  khiter_t k = kh_get(id, ctx->visited, (uintptr_t)root);
  if (k != kh_end(ctx->visited))
    return;
...
static int visit_index_qualname(PyObject *obj, void *ctx) {
  ...
  if (!vctx->parent) {
    ...
    qname = tc_strapp(qname, ".");
    qname = tc_strapp(qname, vctx->attrname);
  }
...
static bool s_traverse(PyObject *root, visitproc visit, void *user,
                       traversefunc t) {
  int ret = false;
  struct visit_ctx ctx;
...
bool S_Traverse_IndexQualnames(khash_t(str) * inout) {
  ...
  while (PyDict_Next(modules_dict, &pos, &key, &value)) {
    ...
    if (!PyModule_Check(value))
      continue;
    bool ret =
        s_traverse_bf_with_visited(value, visit_index_qualname, inout, visited);
    if (!ret)
      goto fail_traverse;
  }
