...
#define SET_EXC(_type, ...)                                                    \
  do {                                                                         \
    char errbuff[1024];                                                        \
    off_t written =                                                            \
        snprintf(errbuff, sizeof(errbuff), "%s:%d: ", __FILE__, __LINE__);     \
    written +=                                                                 \
        snprintf(errbuff + written, sizeof(errbuff) - written, __VA_ARGS__);   \
...
#define TRACE_OP(op, ctx)                                                      \
  do {                                                                         \
    PyObject *mod = PyDict_GetItemString(PySys_GetObject("modules"), "tc");    \
    ...
    printf("[U] %-14s: [stack size: %4u] [mark stack size: %4u] (%s:%d)\n",    \
           ...
           (unsigned)vec_size(&ctx->mark_stack), __FILE__, __LINE__);          \
    fflush(stdout);                                                            \
    Py_DECREF(flag);                                                           \
  } while (0)
...
#define TC_EXTEND                                                              \
  'x' /* Interpret the next opcode as a Permafrost Engine extension opcode */

/* The extension opcodes: */
#define TC_PROTO 'a' /* identify pickle protocol version */
#define TC_TRUE 'b'  /* push True */
#define TC_FALSE 'c' /* push False */
#define TC_GETATTR                                                             \
  'd' /* Get new reference to attribute(TOS) of object(TOS1) and push it on    \
         the stack */
#define TC_POPMARK 'e'  /* Discard the top mark on the mark stack */
#define TC_SETATTRS 'f' /* Set attributes of object on TOS1 with dict at TOS   \
                         */
#define TC_NOTIMPL 'g'  /* Push NotImplemented */
#define TC_ELLIPSIS 'h' /* Push Ellipsis */
#define TC_BUILTIN                                                             \
  'i' /* Push new reference to built-in that is identified by its'             \
         fully-qualified name */
#define TC_TYPE                                                                \
  'j' /* Push new class created from the top 4 stack items (name, bases, dict, \
         metaclass) */
#define TC_CODE                                                                \
  'k' /* Push code object from the 14 TOS items (the args to PyCode_New) */
#define TC_FUNCTION 'l'     /* Push function object from TOS items */
#define TC_EMPTY_CELL 'm'   /* Push empty cell */
#define TC_CELL 'n'         /* Push cell with TOS contents */
#define TC_BYTEARRAY 'o'    /* Push byte array from encoded string and type */
#define TC_SUPER 'p'        /* Push super from 3 TOS items */
#define TC_EMPTYFUNC 'q'    /* Push dummy function object */
#define TC_BASEOBJ 'r'      /* Push an 'object' instance */
#define TC_SYSLONGINFO 's'  /* Push a 'sys.long_info' instance */
#define TC_NULLIMPORTER 't' /* Push an imp.NullImporter instance */
#define TC_SYSFLOATINFO 'u' /* Push a 'sys.float_info' instance */
#define TC_SET 'v'          /* Push a set from TOS tuple */
#define TC_FROZENSET 'w'    /* Push a frozenset from TOS tuple and type */
#define TC_CLASS                                                               \
  'x' /* Push an old class created from the top 3 stack items (name, bases,    \
         methods) */
#define TC_INST 'y' /* Push 'instance' from 'classobj' and 'dict' on TOS */
#define TC_GETSETDESC                                                          \
  'z' /* Push 'getset_descriptor' instance from top 3 stack items */
#define TC_MODULE 'A' /* PUsh module with dict on TOS */
#define TC_NEWINST                                                             \
  'B' /* Create new-style instance with type on TOS, args on TOS1 and the      \
         outer-most builtin base on TOS2 */
#define TC_CLSMETHOD 'C'  /* Push a classmethod instance from TOS */
#define TC_INSTMETHOD 'D' /* Pusn an instancemethod instance from TOS */
#define TC_MEMDESC 'E'    /* Push a 'member_descriptor' instance */
#define TC_METHWRAP                                                            \
  'F' /* Push a 'method-wrapper' instance from top 2 stack items */
#define TC_RANGE 'G' /* Push range object from top 3 stack items */
#define TC_SLICE 'H' /* Push a slice object from top 3 stack items */
#define TC_STATMETHOD                                                          \
  'I'                  /* Push a staticmethod object from stack item and type */
#define TC_BUFFER 'J'  /* Push a buffer object from top 4 stack items */
#define TC_MEMVIEW 'K' /* Push a memoryview object from stack item */
#define TC_PROPERTY 'L'  /* Push a property object from top 5 stack items */
#define TC_ENUMERATE 'M' /* Push an enuerate object from top 5 stack items */
#define TC_LISTITER 'N'  /* Push listiterator object from top 2 stack items */
#define TC_COMPLEX 'O'   /* Push a 'complex' object from top 3 stack items */
#define TC_DICTPROXY 'P' /* Push a 'dictproxy' object from the TOS dict */
#define TC_REVERSED                                                            \
  'Q'                /* Push a 'reversed' object from the top 3 stack items */
#define TC_GEN 'R'   /* Push a generator object from top TOS */
#define TC_FRAME 'S' /* Push a frame object from variable number of TOS items  \
                      */
#define TC_NULLVAL                                                             \
  'T' /* Push a dummy object onto the stack, indicating a NULL value */
#define TC_TRACEBACK 'U' /* Push a traceback object from top 4 TOS items */
#define TC_EMPTYFRAME                                                          \
  'V' /* Push a dummy frame object with valuestack size from TOS */
#define TC_WEAKREF 'W'  /* Push a weakref.ref object from top 2 TOS items */
#define TC_EMPTYMOD 'X' /* Push a dummy module reference */
#define TC_PROXY 'Y'    /* Push a weakproxy instance from top 2 TOS items */
#define TC_STENTRY                                                             \
  'Z' /* Push a symtable entry instance from top 16 TOS items */
#define TC_DICTKEYS '1'  /* Push a dict_keys instance from TOS dict */
#define TC_DICTVALS '2'  /* Push a dict_values instance from TOS dict */
#define TC_DICTITEMS '3' /* Push a dict_items instance from TOS dict */
#define TC_CALLITER                                                            \
  '4' /* Push a callable-iterator instance from top 2 TOS items */
#define TC_SEQITER '5' /* Push an iterator instance from top 2 TOS items */
#define TC_BYTEARRITER                                                         \
  '6' /* Push a bytearray iterator instance from top 2 TOS items */
#define TC_TUPLEITER                                                           \
  '7' /* Push a tuple iterator instance from top 2 TOS items */
#define TC_LISTREVITER                                                         \
  '8' /* Push a reversed list iterator instance from top 2 TOS items */
#define TC_DICTKEYITER                                                         \
  '9' /* Push a dictionary-keyiterator instance from top 5 TOS items */
#define TC_DICTVALITER                                                         \
  '!' /* Push a dictionary-valueiterator instance from top 5 TOS items */
#define TC_DICTITEMITER                                                        \
  '@' /* Push a dictionary-itemiterator instance from top 5 TOS items */
#define TC_SETITER '#'       /* Push a setiterator from top 4 TOS items */
#define TC_FIELDNAMEITER '$' /* Push a fieldnameiterator from top 4 TOS items  \
                              */
#define TC_FORMATITER '%'    /* Push a formatteriterator from top 3 TOS items */
#define TC_EXCEPTION '^'     /* Push an Exception subclass from TOS item */
#define TC_METHOD_DESC                                                         \
  '&' /* Push a method_descriptor from the top 2 TOS items */
#define TC_BI_METHOD                                                           \
  '*' /* Push a 'built-in method' (variant of PyCFunctionObject) type from top \
         3 TOS items */
#define TC_OP_ITEMGET                                                          \
  '(' /* Push an operator.itemgetter instance from top 2 TOS items */
#define TC_OP_ATTRGET                                                          \
  ')' /* Push an operator.attrgetter instance from top 2 TOS items */
#define TC_OP_METHODCALL                                                       \
  '-' /* Push an operator.methodcaller instance from top 3 TOS items */
#define TC_CUSTOM                                                              \
  '+' /* Push an instance returned by an __unpickle__ static method of a type  \
       */
#define TC_ALLOC                                                               \
  ':' /* Allocate an object (call tp_alloc) using the type on TOS */
...
typedef int (*pickle_func_t)(struct pickle_ctx *ctx, PyObject *obj,
                             SDL_RWops *stream);
...
static bool pickle_obj(struct pickle_ctx *ctx, PyObject *obj,
                       SDL_RWops *stream);
...
static bool emit_get(const struct pickle_ctx *ctx, PyObject *obj,
                     SDL_RWops *rw);
static bool emit_put(const struct pickle_ctx *ctx, PyObject *obj,
                     SDL_RWops *rw);
...
static int weakref_callable_proxy_pickle(struct pickle_ctx *, PyObject *,
                                         SDL_RWops *);
...
static int class_method_descr_pickle(struct pickle_ctx *, PyObject *,
                                     SDL_RWops *);
...
static int newclass_instance_pickle(struct pickle_ctx *, PyObject *,
                                    SDL_RWops *);
...
static int oper_methodcaller_pickle(struct pickle_ctx *, PyObject *,
                                    SDL_RWops *);
static int placeholder_inst_pickle(struct pickle_ctx *, PyObject *,
                                   SDL_RWops *);
static int exception_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw);
...
static struct pickle_entry s_type_dispatch_table[] = {
    ...
    {.type = NULL,
     /*&PyByteArray_Type,*/ .picklefunc = bytearray_pickle}, /* bytearray() */
    {.type = NULL, /*&PyList_Type*/ .picklefunc = list_pickle},   /* list() */
    {.type = NULL, /*&PySuper_Type*/ .picklefunc = super_pickle}, /* super() */
    {.type = NULL,
     /*&PyBaseObject_Type*/ .picklefunc = base_obj_pickle},       /* object() */
...
#ifdef Py_USING_UNICODE
    {.type = NULL,
     /*&PyUnicode_Type*/ .picklefunc = unicode_pickle}, /* unicode() */
#endif
    {.type = NULL, /*&PySlice_Type*/ .picklefunc = slice_pickle}, /* slice() */
    {.type = NULL,
     /*&PyStaticMethod_Type*/ .picklefunc =
         static_method_pickle}, /* staticmethod() */
#ifndef WITHOUT_COMPLEX
    {.type = NULL,
     /*&PyComplex_Type*/ .picklefunc = complex_pickle}, /* complex() */
#endif
    {.type = NULL, /*&PyFloat_Type*/ .picklefunc = float_pickle}, /* float() */
    {.type = NULL,
     /*&PyBuffer_Type*/ .picklefunc = buffer_pickle},           /* buffer() */
    {.type = NULL, /*&PyLong_Type*/ .picklefunc = long_pickle}, /* long() */
    {.type = NULL, /*&PyInt_Type*/ .picklefunc = int_pickle},   /* int() */
    {.type = NULL,
     /*&PyFrozenSet_Type*/ .picklefunc = frozen_set_pickle}, /* frozenset() */
    {.type = NULL,
     /*&PyProperty_Type*/ .picklefunc = property_pickle}, /* property() */
    {.type = NULL,
     /*&PyMemoryView_Type*/ .picklefunc =
         memory_view_pickle}, /* memoryview() */
    {.type = NULL, /*&PyTuple_Type*/ .picklefunc = tuple_pickle}, /* tuple() */
    {.type = NULL,
     /*&PyEnum_Type*/ .picklefunc = enum_pickle}, /* enumerate() */
    {.type = NULL,
     /*&PyReversed_Type*/ .picklefunc = reversed_pickle}, /* reversed(()) */
    {.type = NULL,
     /*&PyMethod_Type*/ .picklefunc =
         method_pickle}, /* indirectly: instance methods */
    {.type = NULL,
     /*&PyFunction_Type*/ .picklefunc =
         function_pickle}, /* indirectly: function */
    {.type = NULL,
     /*&PyClass_Type*/ .picklefunc =
         class_pickle}, /* indirectly: Old-style class */
    {.type = NULL,
     /*&PyGen_Type*/ .picklefunc =
         gen_pickle}, /* indirectly: return value from generator function */
    {.type = NULL,
     /*&PyInstance_Type*/ .picklefunc = instance_pickle},       /* instance() */
    {.type = NULL, /*&PyFile_Type*/ .picklefunc = file_pickle}, /* open() */
    {.type = NULL,
     /*&PyClassMethod_Type*/ .picklefunc =
         class_method_pickle}, /* classmethod() */

    {.type = NULL,
     /*&PyCell_Type*/ .picklefunc =
         cell_pickle}, /* indirectly: used for closures */
    {
        .type = NULL,
        /*&PyModule_Type*/ .picklefunc = module_pickle,
    }, /* indirectly: via import */

    /* These are from accessing the attributes of built-in types; created via
       PyDescr_ API*/
    {.type = NULL,
     /*&PyGetSetDescr_Type*/ .picklefunc =
         get_set_descr_pickle}, /* Wrapper around PyGetSetDef */
    {.type = NULL,
     /*&PyWrapperDescr_Type*/ .picklefunc =
         wrapper_descr_pickle}, /* Wrapper around slot (slotdef) */
    {.type = NULL,
     /*&PyMemberDescr_Type*/ .picklefunc =
         member_descr_pickle}, /* Wrapper around PyMemberDef */
    /* PyClassMethodDescr_Type and PyMethodDescr_Type can only be instantiated
     * during initialization time for builtin types using PyMemberDefs to
     * implement methods in C. Sublcasses of these types will re-use the same
     * descriptor objects. Thus all descriptor objects can be indexed as
     * builtins, and no new instances will be created. */
    {.type = NULL,
     /* &PyClassMethodDescr_Type */ .picklefunc =
         class_method_descr_pickle}, /* Wrapper around PyMethodDef */
    {.type = NULL,
     /* &PyMethodDescr_Type */ .picklefunc =
         method_descr_pickle}, /* Wrapper around PyMethodDef with METH_CLASS set
                                */
    {.type = NULL,
     /* &wrappertype */ .picklefunc =
         method_wrapper_pickle}, /* A PyMethodDescrObject bound to an instance
                                  */
    ...
    /* The following are a result of calling the PyWeakref API with an existing
     * object.
     */
    {.type = NULL, /*&_PyWeakref_RefType*/ .picklefunc = weakref_ref_pickle},
    {.type = NULL,
     /*&_PyWeakref_CallableProxyType*/ .picklefunc =
         weakref_callable_proxy_pickle},
    {.type = NULL,
     /*&_PyWeakref_ProxyType*/ .picklefunc = weakref_proxy_pickle},

    {.type = NULL, /*&PySTEntry_Type*/ .picklefunc = st_entry_pickle},

    /* This is derived from an existing dictionary object using the PyDictProxy
     * API. The only way to get a dictproxy object via scripting is to access
     * the __dict__ attribute of a type object. */
    ...
    {.type = NULL,
     /*&PyByteArrayIter_Type*/ .picklefunc = byte_array_iter_pickle},
    {.type = NULL,
     /*&PyDictIterItem_Type*/ .picklefunc = dict_iter_item_pickle},
    {.type = NULL, /*&PyDictIterKey_Type*/ .picklefunc = dict_iter_key_pickle},
    {.type = NULL,
     /*&PyDictIterValue_Type*/ .picklefunc = dict_iter_value_pickle},
    ...
    {.type = NULL,
     /* &PyFieldNameIter_Type */ .picklefunc = field_name_iter_pickle},
    {.type = NULL,
     /* &PyFormatterIter_Type */ .picklefunc = formatter_iter_pickle},
    ...
    {.type = NULL,
     /* methodcaller_type */ .picklefunc = oper_methodcaller_pickle},

    /* A PyCObject cannot be instantiated directly, but may be exported by C
     * extension modules. As it is a wrapper around a raw memory address
     * exported by some module, we cannot reliablly save and restore it
     */
    ...
    {.type = NULL /* PyExc_KeyboardInterrupt */,
     .picklefunc = exception_pickle},
    ...
    {.type = NULL /* PyExc_NotImplementedError */,
     .picklefunc = exception_pickle},
    {.type = NULL /* PyExc_NameError */, .picklefunc = exception_pickle},
    {.type = NULL /* PyExc_UnboundLocalError */,
     .picklefunc = exception_pickle},
    ...
    {.type = NULL /* PyExc_DeprecationWarning */,
     .picklefunc = exception_pickle},
    {.type = NULL /* PyExc_PendingDeprecationWarning */,
     .picklefunc = exception_pickle},
...
static struct pickle_entry s_tc_dispatch_table[] = {
...
static unpickle_func_t s_ext_op_dispatch_table[256] = {
    [TC_BUILTIN] = op_ext_builtin,
    [TC_TYPE] = op_ext_type,
    [TC_GETATTR] = op_ext_getattr,
    [TC_CODE] = op_ext_code,
    [TC_FUNCTION] = op_ext_function,
    [TC_EMPTY_CELL] = op_ext_empty_cell,
    [TC_CELL] = op_ext_cell,
    [TC_TRUE] = op_ext_true,
    [TC_FALSE] = op_ext_false,
    [TC_BYTEARRAY] = op_ext_bytearray,
    [TC_SUPER] = op_ext_super,
    [TC_POPMARK] = op_ext_popmark,
    [TC_EMPTYFUNC] = op_ext_emptyfunc,
    [TC_BASEOBJ] = op_ext_baseobj,
    [TC_SETATTRS] = op_ext_setattrs,
    [TC_NOTIMPL] = op_ext_notimpl,
    [TC_ELLIPSIS] = op_ext_ellipsis,
    [TC_SYSLONGINFO] = op_ext_syslonginfo,
    [TC_NULLIMPORTER] = op_ext_nullimporter,
    [TC_SYSFLOATINFO] = op_ext_sysfloatinfo,
    [TC_SET] = op_ext_set,
    [TC_FROZENSET] = op_ext_frozenset,
    [TC_CLASS] = op_ext_class,
    [TC_INST] = op_ext_inst,
    [TC_GETSETDESC] = op_ext_getsetdesc,
    [TC_MODULE] = op_ext_module,
    [TC_NEWINST] = op_ext_newinst,
    [TC_CLSMETHOD] = op_ext_clsmethod,
    [TC_INSTMETHOD] = op_ext_instmethod,
    [TC_MEMDESC] = op_ext_memdesc,
    [TC_METHWRAP] = op_ext_method_wrapper,
    [TC_RANGE] = op_ext_range,
    [TC_SLICE] = op_ext_slice,
    [TC_STATMETHOD] = op_ext_staticmethod,
    [TC_BUFFER] op_ext_buffer,
    [TC_MEMVIEW] = op_ext_memview,
    [TC_PROPERTY] = op_ext_property,
    [TC_ENUMERATE] = op_ext_enumerate,
    [TC_LISTITER] = op_ext_listiter,
#ifndef WITHOUT_COMPLEX
    [TC_COMPLEX] = op_ext_complex,
#endif
    [TC_DICTPROXY] = op_ext_dictproxy,
    [TC_REVERSED] = op_ext_reversed,
    [TC_GEN] = op_ext_gen,
    [TC_FRAME] = op_ext_frame,
    [TC_NULLVAL] = op_ext_nullval,
    [TC_TRACEBACK] = op_ext_traceback,
    [TC_EMPTYFRAME] = op_ext_emptyframe,
    [TC_WEAKREF] = op_ext_weakref,
    [TC_EMPTYMOD] = op_ext_emptymod,
    [TC_PROXY] = op_ext_weakproxy,
    [TC_STENTRY] = op_ext_stentry,
    [TC_DICTKEYS] = op_ext_dictkeys,
    [TC_DICTVALS] = op_ext_dictvalues,
    [TC_DICTITEMS] = op_ext_dictitems,
    [TC_CALLITER] = op_ext_calliter,
    [TC_SEQITER] = op_ext_seqiter,
    [TC_BYTEARRITER] = op_ext_bytearriter,
    [TC_TUPLEITER] = op_ext_tupleiter,
    [TC_LISTREVITER] = op_ext_revlistiter,
    [TC_DICTKEYITER] = op_ext_dictkeyiter,
    [TC_DICTVALITER] = op_ext_dictvaliter,
    [TC_DICTITEMITER] = op_ext_dictitemiter,
    [TC_SETITER] = op_ext_setiter,
    [TC_FIELDNAMEITER] = op_ext_fieldnameiter,
    [TC_FORMATITER] = op_ext_formatiter,
    [TC_EXCEPTION] = op_ext_exception,
    [TC_METHOD_DESC] = op_ext_method_desc,
    [TC_BI_METHOD] = op_ext_bi_method,
    [TC_OP_ITEMGET] = op_ext_oper_itemgetter,
    [TC_OP_ATTRGET] = op_ext_oper_attrgetter,
    [TC_OP_METHODCALL] = op_ext_oper_methodcaller,
    [TC_CUSTOM] = op_ext_custom,
    [TC_ALLOC] = op_ext_alloc,
};

/* Statically-linked builtin modules not imported on initialization which also
 * contain C builtins */
/* (i.e. sys.builtin_module_names) */
static const char *s_extra_indexed_mods[] = {
    "array",     "_collections", "_heapq",   "exceptions", "gc",       "imp",
    "itertools", "math",         "operator", "_warnings",  "_weakref",
};

/* Create a 'dummy' subclass of every user-subclassable builtin and maintain a
 * mapping of builtin : subclass. The subclass can be used for creating
 * instances of user-defined subclasses of builtins. This is the same as using
 * the actual user-created type object for construction, except we avoid any
 * side-effects that may be present in __new__ or __init__. */
struct sc_map_entry {
...
static pickle_func_t picklefunc_for_type(PyObject *obj) {
  ...
  for (int i = 0; i < ARR_SIZE(s_tc_dispatch_table); i++) {

    if (obj->ob_type == s_tc_dispatch_table[i].type)
      return s_tc_dispatch_table[i].picklefunc;
  }
...
static bool type_is_builtin(PyObject *type) {
  ...
  for (int i = 0; i < ARR_SIZE(s_tc_dispatch_table); i++) {
    ...
    if (s_tc_dispatch_table[i].type == (PyTypeObject *)type)
      return true;
  }
  return false;
}
...
static int dispatch_idx_for_picklefunc(pickle_func_t tc) {
  for (int i = 0; i < ARR_SIZE(s_type_dispatch_table); i++) {
    if (s_type_dispatch_table[i].picklefunc == tc)
      return i;
  }
  return -1;
}
...
static void load_private_type_refs(void) {
  ...
  assert(string);
  PyObject *tuple =
      PyObject_CallMethod(string, "_formatter_field_name_split", "()");
  assert(tuple && PyTuple_GET_SIZE(tuple) > 1);
  s_type_dispatch_table[idx].type = PyTuple_GET_ITEM(tuple, 1)->ob_type;
  assert(
      !strcmp(s_type_dispatch_table[idx].type->tp_name, "fieldnameiterator"));
  Py_DECREF(tuple);
  Py_DECREF(string);
  ...
  s_type_dispatch_table[idx].type = iter->ob_type;
  assert(
      !strcmp(s_type_dispatch_table[idx].type->tp_name, "formatteriterator"));
  Py_DECREF(string);
  ...
  PyObject *reversed =
      PyObject_CallFunction((PyObject *)&PyReversed_Type, "(O)", tmp);
  Py_DECREF(tmp);
  assert(reversed);
  ...
  s_type_dispatch_table[idx].type = iter->ob_type;
  assert(
      !strcmp(s_type_dispatch_table[idx].type->tp_name, "listreverseiterator"));
  Py_DECREF(reversed);
  ...
  s_type_dispatch_table[idx].type =
      (PyTypeObject *)PyObject_GetAttrString(op_mod, "itemgetter");
  assert(
      !strcmp(s_type_dispatch_table[idx].type->tp_name, "operator.itemgetter"));

  /* operator.attrgetter */
  idx = dispatch_idx_for_picklefunc(oper_attrgetter_pickle);
  s_type_dispatch_table[idx].type =
      (PyTypeObject *)PyObject_GetAttrString(op_mod, "attrgetter");
  assert(
      !strcmp(s_type_dispatch_table[idx].type->tp_name, "operator.attrgetter"));

  /* operator.methodcaller */
  idx = dispatch_idx_for_picklefunc(oper_methodcaller_pickle);
  s_type_dispatch_table[idx].type =
      (PyTypeObject *)PyObject_GetAttrString(op_mod, "methodcaller");
  assert(!strcmp(s_type_dispatch_table[idx].type->tp_name,
                 "operator.methodcaller"));
...
static void load_subclassable_builtin_refs(void) {
  int base_idx = 0;
  PyObject *tcmod = PyDict_GetItemString(PySys_GetObject("modules"), "tc");
  assert(tcmod); /* borrowed ref */
  ...
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "Entity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "AnimEntity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "CombatableEntity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "Window");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "Tile");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "Camera");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "Task");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "BuildableEntity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "BuilderEntity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "ResourceEntity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "HarvesterEntity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "StorageSiteEntity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "MovableEntity");
  s_subclassable_builtin_map[base_idx++].builtin =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "Region");
...
static void load_exception_types(void) {
  ...
  s_type_dispatch_table[base_idx++].type =
      (PyTypeObject *)PyExc_KeyboardInterrupt;
  s_type_dispatch_table[base_idx++].type = (PyTypeObject *)PyExc_ImportError;
  s_type_dispatch_table[base_idx++].type =
      (PyTypeObject *)PyExc_EnvironmentError;
  ...
  s_type_dispatch_table[base_idx++].type =
      (PyTypeObject *)PyExc_NotImplementedError;
  s_type_dispatch_table[base_idx++].type = (PyTypeObject *)PyExc_NameError;
  s_type_dispatch_table[base_idx++].type =
      (PyTypeObject *)PyExc_UnboundLocalError;
  ...
  s_type_dispatch_table[base_idx++].type =
      (PyTypeObject *)PyExc_ArithmeticError;
  s_type_dispatch_table[base_idx++].type =
      (PyTypeObject *)PyExc_FloatingPointError;
  ...
  s_type_dispatch_table[base_idx++].type =
      (PyTypeObject *)PyExc_DeprecationWarning;
  s_type_dispatch_table[base_idx++].type =
      (PyTypeObject *)PyExc_PendingDeprecationWarning;
...
static void load_engine_builtin_types(void) {
  ...
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "Camera");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UIButtonStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UIHeaderStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UISelectableStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UIComboStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UIToggleStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UIScrollbarStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UIEditStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UIPropertyStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UISliderStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "UIProgressStyle");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "Task");
  s_tc_dispatch_table[idx++].type =
      (PyTypeObject *)PyObject_GetAttrString(tcmod, "BuildableEntity");
  ...
  for (int i = 0; i < ARR_SIZE(s_tc_dispatch_table); i++) {
    assert(s_tc_dispatch_table[i].type);
  }
  assert(!PyErr_Occurred());
}
...
static void post_build_index(void) {
  ...
  for (int i = 0; i < ARR_SIZE(s_extra_indexed_mods); i++) {
    ...
    assert(PyDict_GetItemString(sysmods, s_extra_indexed_mods[i])->ob_refcnt ==
           1);
...
static int reference_all_types(void) {
  ...
  for (int i = 0; i < ARR_SIZE(s_type_dispatch_table); i++) {
    ...
    tc_strlcpy(name, ((PyTypeObject *)type)->tp_name, sizeof(name));
    while (*curr) {
...
static void create_builtin_subclasses(void) {
  ...
  PyTypeObject *bi;
  PyObject *tcmod =
      PyDict_GetItemString(PySys_GetObject("modules"), "tc"); /* borrowed */
  ...
  for (int idx = 0; idx < ARR_SIZE(s_subclassable_builtin_map); idx++) {
    ...
    char name[256];
    tc_snprintf(name, sizeof(name), "__%s_subclass__", bi->tp_name);
...
static PyObject *qualname_new_ref(const char *qualname) {
  ...
  if (!mod && strarr_contains(s_extra_indexed_mods,
                              ARR_SIZE(s_extra_indexed_mods), modname)) {
    mod = PyImport_ImportModule(modname);
  }

  if (!mod) {
    SET_RUNTIME_EXC("Could not find module %s for qualified name %s", modname,
                    qualname);
    return NULL;
  }
  ...
  while (curr) {
    ...
    if (!PyObject_HasAttrString(parent, curr)) {
      Py_DECREF(parent);
      SET_RUNTIME_EXC("Could not look up attribute %s in qualified name %s",
                      curr, qualname);
      return NULL;
    }
 ...
 * the attribute. An example is static methods of a class. 'getting' the
 * attribute returns a function, but setting this attribute to a function object
 * will cause Python to wrap it in a method. Thus, when we read back the
 * attribute, it will be an 'unbound method' rather than a function object. This
 * routine handles such special cases, setting the attribute such that its'
 * original value is preserved. */
static int setattr_nondestructive(PyObject *obj, PyObject *name,
                                  PyObject *val) {
...
static int builtin_pickle(struct pickle_ctx *ctx, PyObject *obj,
                          SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char xtend = TC_EXTEND;
  const char builtin = TC_BUILTIN;
...
static int exception_pickle(struct pickle_ctx *ctx, PyObject *obj,
  ...
  const char ops[] = {TC_EXTEND, TC_EXCEPTION};
...
static int custom_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  PyObject *umeth =
      PyObject_GetAttrString((PyObject *)obj->ob_type, "__unpickle__");
  ...
  PyObject *kwargs = Py_BuildValue("{s:s#}", "__ctx__", (void *)&user,
                                   sizeof(struct py_pickle_ctx));
  ...
  if (!ret || !PyString_Check(ret)) {
    SET_RUNTIME_EXC("Error pickling %s instance (%p)", obj->ob_type->tp_name,
                    obj);
    Py_XDECREF(ret);
    goto fail;
  }
  ...
  const char ops[] = {TC_EXTEND, TC_CUSTOM};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static PyObject *method_funcs(PyObject *obj) {
  ...
  for (int i = 0; i < PyList_Size(attrs); i++) {
    ...
    if (meth->im_self && meth->im_self->ob_type != (PyTypeObject *)obj) {
...
static int type_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_TYPE};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int bool_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char true_ops[] = {TC_EXTEND, TC_TRUE};
  const char false_ops[] = {TC_EXTEND, TC_FALSE};
...
static int bytearray_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  CHK_TRUE(pickle_obj(ctx, (PyObject *)obj->ob_type, rw), fail);
  ...
  PyObject *str =
      PyUnicode_EncodeUTF7(PyUnicode_AS_UNICODE(uniobj),
                           PyUnicode_GET_SIZE(uniobj), true, true, "strict");
  ...
  const char ops[] = {TC_EXTEND, TC_BYTEARRAY};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int super_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_SUPER};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int base_obj_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);

  CHK_TRUE(pickle_obj(ctx, (PyObject *)obj->ob_type, rw), fail);

  const char ops[] = {TC_EXTEND, TC_BASEOBJ};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int range_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_RANGE};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int set_elems_pickle(struct pickle_ctx *ctx, PyObject *anyset,
                            SDL_RWops *rw) {
...
static int set_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_SET};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
#ifdef Py_USING_UNICODE
static int unicode_pickle(struct pickle_ctx *ctx, PyObject *obj,
                          SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  PyObject *str = PyUnicode_EncodeUTF7(
      PyUnicode_AS_UNICODE(obj), PyUnicode_GET_SIZE(obj), true, true, "strict");
  assert(strlen(PyString_AS_STRING(str)) == PyString_GET_SIZE(str));

  int nwritten =
      rw->write(rw, PyString_AS_STRING(str), 1, PyString_GET_SIZE(str));
...
static int slice_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_SLICE};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int static_method_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_STATMETHOD};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
#ifndef WITHOUT_COMPLEX
static int complex_pickle(struct pickle_ctx *ctx, PyObject *obj,
                          SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_COMPLEX};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int buffer_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  if (buff->b_ptr) {
    assert(!buff->b_base);
    SET_RUNTIME_EXC(
        "Picking raw memory buffer objects is not supported. Only buffer "
  ...
  const char ops[] = {TC_EXTEND, TC_BUFFER};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int frozen_set_pickle(struct pickle_ctx *ctx, PyObject *obj,
                             SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_FROZENSET};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int property_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_PROPERTY};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int memory_view_pickle(struct pickle_ctx *ctx, PyObject *obj,
                              SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_MEMVIEW};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int enum_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_ENUMERATE};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int reversed_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_REVERSED};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int method_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_INSTMETHOD};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int function_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char emptyfunc[] = {TC_EXTEND, TC_EMPTYFUNC};
  ...
  const char ops[] = {TC_EXTEND, TC_FUNCTION};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int class_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_CLASS};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int gen_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_GEN};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int instance_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_INST};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int file_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  if (file->f_fp == stdin ||
      !strcmp(PyString_AS_STRING(file->f_name), "<stdin>") ||
      file->f_fp == stdout ||
      !strcmp(PyString_AS_STRING(file->f_name), "<stdout>") ||
      file->f_fp == stderr ||
      !strcmp(PyString_AS_STRING(file->f_name), "<stderr>")) {
    return builtin_pickle(ctx, obj, rw);
  }

  TRACE_PICKLE(obj);
  SET_RUNTIME_EXC("Could not pickle file: %s. Only stdin, stdout, and stderr "
                  "are supported.",
...
static int cell_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ec_ops[] = {TC_EXTEND, TC_EMPTY_CELL};
  const char ops[] = {TC_EXTEND, TC_CELL};
...
static int module_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
   ...
   * memoize it */
  const char emptymod[] = {TC_EXTEND, TC_EMPTYMOD};
  ...
  const char ops[] = {TC_EXTEND, TC_MODULE};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int get_set_descr_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                SDL_RWops *rw) {
  ...
  const char getattr[] = {TC_EXTEND, TC_GETSETDESC};
...
static int wrapper_descr_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                SDL_RWops *rw) {
  return builtin_pickle(ctx, obj, rw);
}

static int member_descr_pickle(struct pickle_ctx *ctx, PyObject *obj,
                               SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_MEMDESC};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int dict_proxy_pickle(struct pickle_ctx *ctx, PyObject *obj,
                             SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_DICTPROXY};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int long_info_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  const char ops[] = {TC_EXTEND, TC_SYSLONGINFO};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int float_info_pickle(struct pickle_ctx *ctx, PyObject *obj,
                             SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  const char ops[] = {TC_EXTEND, TC_SYSFLOATINFO};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int sys_flags_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw) {
  return builtin_pickle(ctx, obj, rw);
}

static int sys_version_pickle(struct pickle_ctx *ctx, PyObject *obj,
                              SDL_RWops *rw) {
  return builtin_pickle(ctx, obj, rw);
}

static int cfunction_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw) {
  assert(obj->ob_type == &PyCFunction_Type);
  PyCFunctionObject *func = (PyCFunctionObject *)obj;

  /* Instances of unbounded built-in functions are never re-created. It is
   * sufficient to pickle them by reference. */
  ...
  const char ops[] = {TC_EXTEND, TC_BI_METHOD};
...
static int code_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char ops[] = {TC_EXTEND, TC_CODE};
  CHK_TRUE(rw->write(rw, ops, 2, 1), fail);
  return 0;
...
static int traceback_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_TRACEBACK};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static Py_ssize_t frame_extra_size(PyFrameObject *frame) {
  ...
  extras =
      frame->f_code->co_stacksize + frame->f_code->co_nlocals + ncells + nfrees;
  return extras;
}
...
static int frame_pickle(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char emptyframe[] = {TC_EXTEND, TC_EMPTYFRAME};
   ...
   */
  size_t nvals =
      (f->f_stacktop == NULL) ? 0 : (f->f_stacktop - f->f_valuestack);
  ...
  for (int i = nextra - 1; i >= 0; i--) {
    ...
    if (!curr) {
      const char ops[] = {TC_EXTEND, TC_NULLVAL};
      CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
    } else {
  ...
  const char ops[] = {TC_EXTEND, TC_FRAME};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int null_importer_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  const char ops[] = {TC_EXTEND, TC_NULLIMPORTER};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
fail:
  DEFAULT_ERR(PyExc_IOError, "Error writing to pickle stream");
...
static int not_implemented_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                  SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  assert(obj == Py_NotImplemented);
  const char ops[] = {TC_EXTEND, TC_NOTIMPL};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
fail:
  DEFAULT_ERR(PyExc_IOError, "Error writing to pickle stream");
...
static int ellipsis_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  assert(obj == Py_Ellipsis);
  const char ops[] = {TC_EXTEND, TC_ELLIPSIS};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
fail:
  DEFAULT_ERR(PyExc_IOError, "Error writing to pickle stream");
...
static int weakref_ref_pickle(struct pickle_ctx *ctx, PyObject *obj,
                              SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  assert(PyWeakref_CheckRef(obj));
  PyWeakReference *ref = (PyWeakReference *)obj;

  assert(ref->wr_object == Py_None ||
         PyType_SUPPORTS_WEAKREFS(Py_TYPE(ref->wr_object)));
  ...
  const char ops[] = {TC_EXTEND, TC_WEAKREF};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int weakref_callable_proxy_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                         SDL_RWops *rw) {
  return weakref_proxy_pickle(ctx, obj, rw);
}

static int weakref_proxy_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  PyWeakReference *ref = (PyWeakReference *)obj;
  ...
  const char ops[] = {TC_EXTEND, TC_PROXY};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int st_entry_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_STENTRY};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int class_method_descr_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                     SDL_RWops *rw) {
  return builtin_pickle(ctx, obj, rw);
}

static int class_method_pickle(struct pickle_ctx *ctx, PyObject *obj,
                               SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_CLSMETHOD};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int dict_items_pickle(struct pickle_ctx *ctx, PyObject *obj,
                             SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_DICTITEMS};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int dict_keys_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_DICTKEYS};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int dict_values_pickle(struct pickle_ctx *ctx, PyObject *obj,
                              SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_DICTVALS};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int method_descr_pickle(struct pickle_ctx *ctx, PyObject *obj,
                               SDL_RWops *rw) {
  ...
  const char getattr[] = {TC_EXTEND, TC_METHOD_DESC};
  CHK_TRUE(rw->write(rw, getattr, ARR_SIZE(getattr), 1), fail);
  return 0;
...
static int method_wrapper_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                 SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_METHWRAP};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int call_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_CALLITER};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int seq_iter_pickle_with_op(struct pickle_ctx *ctx, PyObject *obj,
                                   SDL_RWops *rw, char ext_op) {
  ...
  const char ops[] = {TC_EXTEND, ext_op};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int dict_iter_pickle_with_op(struct pickle_ctx *ctx, PyObject *obj,
                                    SDL_RWops *rw, char ext_op) {
...
static int seq_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  return seq_iter_pickle_with_op(ctx, obj, rw, TC_SEQITER);
}

static int byte_array_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                  SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  return seq_iter_pickle_with_op(ctx, obj, rw, TC_BYTEARRITER);
}

static int dict_iter_item_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                 SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  return dict_iter_pickle_with_op(ctx, obj, rw, TC_DICTITEMITER);
}

static int dict_iter_key_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  return dict_iter_pickle_with_op(ctx, obj, rw, TC_DICTKEYITER);
}

static int dict_iter_value_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                  SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  return dict_iter_pickle_with_op(ctx, obj, rw, TC_DICTVALITER);
}

static int field_name_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                  SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  assert(iter->it_field.str.ptr >= raw &&
         iter->it_field.str.ptr < raw + rawlen);
  assert(iter->it_field.str.end >= raw &&
         iter->it_field.str.end <= raw + rawlen);
  ...
  const char ops[] = {TC_EXTEND, TC_FIELDNAMEITER};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int formatter_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                 SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  assert(iter->it_markup.str.ptr >= raw &&
         iter->it_markup.str.ptr < raw + rawlen);
  assert(iter->it_markup.str.end >= raw &&
         iter->it_markup.str.end <= raw + rawlen);
  ...
  const char ops[] = {TC_EXTEND, TC_FORMATITER};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int list_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                            SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  return seq_iter_pickle_with_op(ctx, obj, rw, TC_LISTITER);
}

static int list_rev_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  return seq_iter_pickle_with_op(ctx, obj, rw, TC_LISTREVITER);
}

static int set_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                           SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_SETITER};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int tuple_iter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                             SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  return seq_iter_pickle_with_op(ctx, obj, rw, TC_TUPLEITER);
}

static int newclass_instance_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                    SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  pickle_func_t tc = picklefunc_for_type(obj);
  assert(tc);
  Py_TYPE(obj) = type;
  ...
  const char ops[] = {TC_EXTEND, TC_NEWINST};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int oper_itemgetter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                  SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_OP_ITEMGET};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int oper_attrgetter_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                  SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_OP_ATTRGET};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int oper_methodcaller_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                    SDL_RWops *rw) {
  TRACE_PICKLE(obj);
  ...
  const char ops[] = {TC_EXTEND, TC_OP_METHODCALL};
  CHK_TRUE(rw->write(rw, ops, ARR_SIZE(ops), 1), fail);
  return 0;
...
static int placeholder_inst_pickle(struct pickle_ctx *ctx, PyObject *obj,
                                   SDL_RWops *rw) {
  TRACE_PICKLE(obj);
...
static int op_int(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (errno || !isspace(*endptr)) {
    SET_RUNTIME_EXC("Bad int in pickle stream [offset: %ld]",
                    (long)rw->seek(rw, RW_SEEK_CUR, 0));
    goto fail;
  }
...
static int op_long(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (errno || !isspace(*endptr)) {
    SET_RUNTIME_EXC("Bad long in pickle stream [offset: %ld]",
                    (long)rw->seek(rw, RW_SEEK_CUR, 0));
    goto fail;
  }
...
static int op_put(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!idx && !isspace(*end)) {
    SET_RUNTIME_EXC("Bad index in pickle stream: [offset: %ld]",
                    (long)rw->seek(rw, RW_SEEK_CUR, 0));
    return -1;
  }
...
static int op_ext_type(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(meta)) {
    SET_RUNTIME_EXC("TC_TYPE: 'type' (metatype) not found at TOS");
    goto fail_typecheck;
  }

  if (!PyDict_Check(dict)) {
    SET_RUNTIME_EXC("TC_TYPE: Dict not found at TOS1");
    goto fail_typecheck;
  }

  if (!PyTuple_Check(bases)) {
    SET_RUNTIME_EXC("TC_TYPE: (bases) tuple not found at TOS2");
    goto fail_typecheck;
  }

  if (!PyString_Check(name)) {
    SET_RUNTIME_EXC("TC_TYPE: Name not found at TOS3");
    goto fail_typecheck;
  }
...
static int op_ext_getattr(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  int ret = -1;
  if (!PyString_Check(name)) {
    SET_RUNTIME_EXC("TC_GETATTR: Expecting string (name) at TOS");
    goto fail_typecheck;
  }
...
static int op_ext_code(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyInt_Check(argcount) || !PyInt_Check(nlocals) ||
      !PyInt_Check(stacksize) || !PyInt_Check(flags) ||
      !PyInt_Check(firstlineno)) {
    SET_RUNTIME_EXC("TC_CODE: argcount, nlocals, stacksize, flags, "
                    "firstlinenoe must be an integers");
    goto fail;
  }

  PyObject *ret = (PyObject *)PyCode_New(
      PyInt_AS_LONG(argcount), PyInt_AS_LONG(nlocals), PyInt_AS_LONG(stacksize),
      PyInt_AS_LONG(flags), code, consts, names, varnames, freevars, cellvars,
      filename, name, PyInt_AS_LONG(firstlineno), lnotab);
...
static int op_ext_bytearray(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyString_Check(encoded)) {
    SET_RUNTIME_EXC("TC_BYTEARRAY: Expecting string at TOS");
    goto fail_typecheck;
  }
  ...
  if (!PyType_Check(type) ||
    ...
    SET_RUNTIME_EXC(
        "TC_BYTEARRAY: Expecting bytearray type of subtype at TOS1");
    goto fail_typecheck;
  }
  ...
  PyObject *raw = PyUnicode_DecodeUTF7(PyString_AS_STRING(encoded),
                                       PyString_GET_SIZE(encoded), "strict");
...
static int op_ext_super(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(metatype) &&
    ...
    SET_RUNTIME_EXC("TC_SUPER: Expecting 'super' type or subtype at TOS2");
    goto fail_typecheck;
  }
...
static void del_extra_attrs(PyObject *obj, PyObject **attrs_base,
                            size_t npairs) {
...
static int op_ext_set(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type) ||
    ...
    SET_RUNTIME_EXC("TC_SET: Expecting a 'set' type or subtype on TOS");
    goto fail_typecheck;
  }

  if (!PyTuple_Check(items)) {
    SET_RUNTIME_EXC("TC_SET: Expecting a tuple of set items on TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_frozenset(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type) ||
    ...
    SET_RUNTIME_EXC("TC_FROZENSET: Expecting a tuple of set items on TOS");
    goto fail_typecheck;
  }
...
static int op_ext_class(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyDict_Check(dict)) {
    SET_RUNTIME_EXC("TC_CLASS: Dict not found at TOS");
    goto fail_typecheck;
  }

  if (!PyTuple_Check(bases)) {
    SET_RUNTIME_EXC("TC_CLASS: (bases) tuple not found at TOS1");
    goto fail_typecheck;
  }

  if (!PyString_Check(name)) {
    SET_RUNTIME_EXC("TC_CLASS: Name not found at TOS2");
    goto fail_typecheck;
  }
...
static int op_ext_inst(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyDict_Check(dict)) {
    SET_RUNTIME_EXC("TC_INST: dict not found at TOS");
    goto fail_typecheck;
  }

  if (!PyClass_Check(klass)) {
    SET_RUNTIME_EXC("TC_INST: classobj not found at TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_getsetdesc(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyString_Check(name)) {
    SET_RUNTIME_EXC("TC_GETSETDESC: Expecting string at TOS");
    goto fail_typecheck;
  }

  if (!PyType_Check(type)) {
    SET_RUNTIME_EXC("TC_GETSETDESC: Expecting type at TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_module(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyDict_Check(dict)) {
    SET_RUNTIME_EXC("TC_MODULE: Expecting dict on TOS");
    goto fail_typecheck;
  }

  if (!PyModule_Check(emptymod)) {
    SET_RUNTIME_EXC("TC_MODULE: Expecting module instance on TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_newinst(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type)) {
    SET_RUNTIME_EXC("TC_NEWINST: Expecting type on TOS");
    goto fail_typecheck;
  }
...
static int op_ext_clsmethod(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyCallable_Check(callable)) {
    SET_RUNTIME_EXC("TC_CLSMETHOD: Expecting callable object on TOS");
    goto fail;
  }
...
static int op_ext_instmethod(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  PyObject *newmeth =
      PyMethod_New(func, (self == Py_None ? NULL : self), klass);
  CHK_TRUE(newmeth, fail);
  vec_pobj_push(&ctx->stack, newmeth);
  ret = 0;
...
static int op_ext_memdesc(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyString_Check(name)) {
    SET_RUNTIME_EXC("TC_MEMDESC: Expecting string at TOS");
    goto fail_typecheck;
  }

  if (!PyType_Check(type)) {
    SET_RUNTIME_EXC("TC_MEMDESC: Expecting type at TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_method_wrapper(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (desc->ob_type != &PyWrapperDescr_Type) {
    SET_RUNTIME_EXC("TC_METHWRAP: Expecting wrapper_descriptor at TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_range(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyLong_Check(ilow) || !PyLong_Check(ihigh) || !PyLong_Check(step)) {

    SET_RUNTIME_EXC("TC_RANGE: Expecting long objects as the top 3 TOS items");
    goto fail_typecheck;
  }

  PyObject *retval = PyObject_CallFunction((PyObject *)&PyRange_Type, "(OOO)",
                                           ilow, ihigh, step);
...
static int op_ext_staticmethod(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type) ||
    ...
    SET_RUNTIME_EXC(
        "TC_STATMETHOD: Expecting 'staticmethod' type or subtype on TOS");
    goto fail_typecheck;
  }

  if (!PyCallable_Check(callable)) {
    SET_RUNTIME_EXC("TC_STATMETHOD: Expecting callable object on TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_buffer(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyLong_Check(readonly) || !PyLong_Check(offset) || !PyLong_Check(size)) {
    SET_RUNTIME_EXC("TC_BUFFER: Expecting long objects as top 3 stack items");
    goto fail_typecheck;
  }
  ...
  if (PyLong_AsLong(readonly)) {
    retval =
        PyBuffer_FromObject(base, PyLong_AsLong(offset), PyLong_AsLong(size));
  } else {
    retval = PyBuffer_FromReadWriteObject(base, PyLong_AsLong(offset),
                                          PyLong_AsLong(size));
  }
...
static int op_ext_property(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type) ||
    ...
    SET_RUNTIME_EXC("TC_PROPERTY: Expecting 'property' type or subtype on TOS");
    goto fail_typecheck;
  }
...
static int op_ext_enumerate(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type) ||
    ...
    SET_RUNTIME_EXC(
        "TC_ENUMERATE: Expecting 'enumerate' type or subtype at TOS");
    goto fail_typecheck;
  }

  if (longindex != Py_None && !_PyAnyInt_Check(longindex)) {
    SET_RUNTIME_EXC(
        "TC_ENUMERATE: expecting integer (int,long) or None type on TOS1");
    goto fail_typecheck;
  }

  if (result != Py_None && !PyTuple_Check(result)) {
    SET_RUNTIME_EXC("TC_ENUMERATE: expecting tuple or None on TOS2");
    goto fail_typecheck;
  }

  if (!PyIter_Check(sit)) {
    SET_RUNTIME_EXC("TC_ENUMERATE: expecting iterator on TOS2");
    goto fail_typecheck;
  }

  if (!PyLong_Check(index)) {
    SET_RUNTIME_EXC("TC_ENUMERATE: expecting long on TOS3");
    goto fail_typecheck;
  }
  ...
  enumobject *en =
      (enumobject *)((PyTypeObject *)ctype)->tp_alloc((PyTypeObject *)ctype, 0);
  CHK_TRUE(en, fail_en);
...
static int op_ext_seqiter_with_type(struct unpickle_ctx *ctx, SDL_RWops *rw,
                                    PyTypeObject *type) {
  int ret = -1;
...
static int op_ext_complex(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type) ||
    ...
    SET_RUNTIME_EXC("TC_COMPLEX: Expecting 'complex' type or subtype at TOS");
    goto fail_typecheck;
  }

  if (!PyFloat_Check(imag) || !PyFloat_Check(real)) {
    SET_RUNTIME_EXC("TC_COMPLEX: Expecting float objects at TOS1 and TOS2");
    goto fail_typecheck;
  }
...
static int op_ext_dictproxy(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyDict_Check(dict)) {
    SET_RUNTIME_EXC("TC_DICTPROXY: Expecting dict on TOS");
    goto fail_typecheck;
  }
...
static int op_ext_reversed(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type) ||
    ...
    SET_RUNTIME_EXC("TC_REVERSED: Expecting 'reversed' type or subtype at TOS");
    goto fail_typecheck;
  }

  if (seq != Py_None && !PySequence_Check(seq)) {
    SET_RUNTIME_EXC("TC_REVERSED: TOS1 item must be None or a sequence");
    goto fail_typecheck;
  }

  if (!PyLong_Check(index)) {
    SET_RUNTIME_EXC("TC_REVERSED: Expecting long object on TOS2");
    goto fail_typecheck;
  }
  ...
  reversedobject *rev = (reversedobject *)((PyTypeObject *)ctype)
                            ->tp_alloc((PyTypeObject *)ctype, 0);
...
static int op_ext_gen(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyCode_Check(top) && !PyFrame_Check(top)) {
    SET_RUNTIME_EXC("TC_GEN: Expecting code or frame object on TOS");
    goto fail_typecheck;
  }
...
static int op_ext_frame(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyFrame_Check((PyObject *)frame)) {
    SET_RUNTIME_EXC("TC_FRAME: Expecting frame on TOS");
    goto fail_typecheck;
  }

  if (locals != Py_None && !PyMapping_Check(locals)) {
    SET_RUNTIME_EXC("TC_FRAME: Expecting mapping or None on TOS1");
    goto fail_typecheck;
  }

  if (!PyDict_Check(globals)) {
    SET_RUNTIME_EXC("TC_FRAME: Expecting dict on TOS2");
    goto fail_typecheck;
  }

  if (!PyCode_Check((PyObject *)code)) {
    SET_RUNTIME_EXC("TC_FRAME: Expecting code object on TOS3");
    goto fail_typecheck;
  }

  if (back != Py_None && !PyFrame_Check(back)) {
    SET_RUNTIME_EXC("TC_FRAME: Expecting frame or None on TOS4");
    goto fail_typecheck;
  }

  if (!PyInt_Check(lineno) || !PyInt_Check(lasti)) {
    SET_RUNTIME_EXC("TC_FRAME: Expecting int objects on TOS5 and TOS6");
    goto fail_typecheck;
  }

  /* Forcefully set attrs */
  PyObject_GC_UnTrack(frame);
  CHK_TRUE(0 == convert_frame(frame, code, globals,
  ...
  if (nitems % 3 != 0) {
    SET_RUNTIME_EXC("TC_FRAME: Number of stack items for the block stack not "
                    "divisible by 3");
    goto fail_frame;
  }
  nitems /= 3;
  ...
  for (int i = 0; i < nitems; i++) {
    ...
    if (!PyInt_Check(level) || !PyInt_Check(handler) || !PyInt_Check(type)) {
      SET_RUNTIME_EXC("TC_FRAME: Got non-int blockstack item fields");
      goto fail_inner;
    }

    PyFrame_BlockSetup(frame, PyInt_AS_LONG(type), PyInt_AS_LONG(handler),
                       PyInt_AS_LONG(level));
  ...
  if (PyInt_Check(sent) && PyInt_AS_LONG(sent) != nitems) {
    ...
    SET_RUNTIME_EXC("TC_FRAME: Sentinel reports incorrect number of items on "
                    "valuestack [exp: %u, act: %ld]",
...
static int op_ext_traceback(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyInt_Check(lasti) || !PyInt_Check(lineno)) {
    SET_RUNTIME_EXC("TC_TRACEBACK: Expecting int objects on TOS and TOS1");
    goto fail_typecheck;
  }

  if (!PyFrame_Check(frame)) {
    SET_RUNTIME_EXC("TC_TRACEBACK: Expecting frame object on TOS2");
    goto fail_typecheck;
  }

  if (next != Py_None && !PyTraceBack_Check(next)) {
    SET_RUNTIME_EXC("TC_TRACEBACK: Expecting tracebakc or None object on TOS3");
    goto fail_typecheck;
  }
...
static int op_ext_emptyframe(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyInt_Check(valsize)) {
    SET_RUNTIME_EXC("TC_EMPTYFRAME: Expecting integer object on TOS");
    goto fail_typecheck;
  }

  PyObject *code =
      (PyObject *)PyCode_NewEmpty("__placeholder__", "__placeholder__", 0);
  ...
  PyThreadState *tstate = PyThreadState_GET();
  PyFrameObject *frame =
      PyFrame_New(tstate, (PyCodeObject *)code, globals, locals);
...
static int op_ext_weakref(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (callback != Py_None && !PyCallable_Check(callback)) {
    SET_RUNTIME_EXC("TC_WEAKREF: Expecting callable or none on TOS");
    goto fail_typecheck;
  }

  if (!PyType_SUPPORTS_WEAKREFS(Py_TYPE(referent)) && referent != Py_None) {
    SET_RUNTIME_EXC(
        "TC_WEAKREF: Expecting object of type that supports weakrefs on TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_weakproxy(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (callback != Py_None && !PyCallable_Check(callback)) {
    SET_RUNTIME_EXC("TC_PROXY: Expecting callable or none on TOS");
    goto fail_typecheck;
  }
...
static int op_ext_stentry(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyInt_Check(ste_id) && !PyLong_Check(ste_id)) {
    SET_RUNTIME_EXC("TC_STENTRY: Expecting int or long object on TOS15");
    goto fail_typecheck;
  }

  if (!PyDict_Check(ste_symbols)) {
    SET_RUNTIME_EXC("TC_STENTRY: Expecting dict object on TOS14");
    goto fail_typecheck;
  }

  if (!PyString_Check(ste_name)) {
    SET_RUNTIME_EXC("TC_STENTRY: Expecting string object on TOS13");
    goto fail_typecheck;
  }

  if (!PyList_Check(ste_varnames) && !PyList_Check(ste_children)) {
    SET_RUNTIME_EXC("TC_STENTRY: Expecting list objects on TOS12 and TOS11");
    goto fail_typecheck;
  }

  if (!PyInt_Check(ste_type) && !PyInt_Check(ste_unoptimized) &&
      !PyInt_Check(ste_nested) && !PyInt_Check(ste_free) &&
      !PyInt_Check(ste_child_free) && !PyInt_Check(ste_generator) &&
      !PyInt_Check(ste_varargs) && !PyInt_Check(ste_returns_value) &&
      !PyInt_Check(ste_lineno) && !PyInt_Check(ste_tmpname)) {
    SET_RUNTIME_EXC("TC_STENTRY: Expecting int objects on TOS10 through TOS");
    goto fail_typecheck;
  }
...
static int op_ext_dictkeys(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyDict_Check(dict)) {
    SET_RUNTIME_EXC("TC_DICTKEYS: Expecting dict object at TOS");
    goto fail_typecheck;
  }
...
static int op_ext_dictvalues(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyDict_Check(dict)) {
    SET_RUNTIME_EXC("TC_VALUES: Expecting dict object at TOS");
    goto fail_typecheck;
  }
...
static int op_ext_dictiter_with_type(struct unpickle_ctx *ctx, SDL_RWops *rw,
  ...
  if (!PyLong_Check(len) || !PyLong_Check(di_pos) || !PyLong_Check(di_pos)) {
...
static int op_ext_setiter(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyLong_Check(len) || !PyLong_Check(si_pos) || !PyLong_Check(si_used)) {
    SET_RUNTIME_EXC(
        "TC_SETITER: Expecting long objects for the top 3 TOS items");
    goto fail_typecheck;
  }

  if (!PySet_Check(si_set)) {
    SET_RUNTIME_EXC("TC_SETITER: Expecting set object at TOS3");
    goto fail_typecheck;
  }
...
static int op_ext_fieldnameiter(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyLong_Check(str_end) || !PyLong_Check(str_ptr) || !PyLong_Check(ptr)) {
    SET_RUNTIME_EXC(
        "TC_FIELDNAMEITER: Expecting long objects as top 3 TOS items");
    goto fail_typecheck;
  }

  if (!PyString_Check(str)) {
    SET_RUNTIME_EXC("TC_FIELDNAMEITER: Expecting string object at TOS3");
    goto fail_typecheck;
  }
...
static int op_ext_formatiter(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyLong_Check(str_end) || !PyLong_Check(str_ptr)) {
    SET_RUNTIME_EXC(
        "TC_FIELDNAMEITER: Expecting long objects as top 2 TOS items");
    goto fail_typecheck;
  }

  if (!PyString_Check(str)) {
    SET_RUNTIME_EXC("TC_FIELDNAMEITER: Expecting string object at TOS2");
    goto fail_typecheck;
  }
...
static int op_ext_exception(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyType_Check(type) || !PyExceptionClass_Check(type)) {
    SET_RUNTIME_EXC("TC_EXCEPTION: Expecting type that is a non-strict "
                    "subclass of Exception on TOS");
    goto fail_typecheck;
  }
...
static int op_ext_method_desc(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyString_Check(name)) {
    SET_RUNTIME_EXC("TC_METHOD_DESC: Expecting string at TOS");
    goto fail_typecheck;
  }

  if (!PyType_Check(type)) {
    SET_RUNTIME_EXC("TC_METHOD_DESC: Expecting type at TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_bi_method(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyString_Check(name)) {
    SET_RUNTIME_EXC("TC_BI_METHOD: Expecting string at TOS");
    goto fail_typecheck;
  }

  if (!PyType_Check(type)) {
    SET_RUNTIME_EXC("TC_BI_METHOD: Expecting type at TOS1");
    goto fail_typecheck;
  }
  ...
  if (!found && (bases = tp_type->tp_bases)) {
    ...
    for (int i = 0; i < PyTuple_GET_SIZE(bases); i++) {
      ...
      for (PyMethodDef *curr = tp_type->tp_methods; curr && curr->ml_name;
           curr++) {
...
static int op_ext_oper_itemgetter(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyInt_Check(nitems)) {
    SET_RUNTIME_EXC("TC_OP_ITEMGET: Expecting int at TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_oper_attrgetter(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyInt_Check(nattrs)) {
    SET_RUNTIME_EXC("TC_OP_ATTRGET: Expecting int at TOS1");
    goto fail_typecheck;
  }
...
static int op_ext_oper_methodcaller(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (kwds != Py_None && !PyDict_Check(kwds)) {
    SET_RUNTIME_EXC("TC_OP_METHODCALL: Expecting dictionary of None at TOS");
    goto fail_typecheck;
  }

  if (!PyTuple_Check(args)) {
    SET_RUNTIME_EXC("TC_OP_METHODCALL: Expecting tuple at TOS1");
    goto fail_typecheck;
  }

  if (!PyString_Check(name)) {
    SET_RUNTIME_EXC("TC_OP_METHODCALL: Expecting string at TOS2");
    goto fail_typecheck;
  }

  methodcallerobject *rval =
      PyObject_GC_New(methodcallerobject, methodcallertype);
  Py_INCREF(name);
  rval->name = name;
  Py_INCREF(args);
  rval->args = args;
...
static int op_ext_custom(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  if (!PyString_Check(str)) {
    SET_RUNTIME_EXC("TC_CUSTOM: Expecting string at TOS");
    goto fail_typecheck;
  }

  if (!PyType_Check(klass)) {
    SET_RUNTIME_EXC("TC_CUSTOM: Expecting type at TOS1");
    goto fail_typecheck;
  }
  PyObject *ctype = constructor_type((PyTypeObject *)klass);
  assert(ctype);

  struct py_unpickle_ctx user = (struct py_unpickle_ctx){.stack = &ctx->stack};

  PyObject *pmeth = PyObject_GetAttrString(klass, "__unpickle__");
  PyObject *args = Py_BuildValue("(O)", str);
  PyObject *kwargs =
      Py_BuildValue("{s:s#}", "__ctx__", (void *)&user, sizeof(user));
...
static int op_ext_alloc(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  ...
  PyObject *rval =
      ((PyTypeObject *)klass)->tp_alloc((struct _typeobject *)klass, 0);
  CHK_TRUE(rval, fail_inst);
...
static int op_ext_nullimporter(struct unpickle_ctx *ctx, SDL_RWops *rw) {
  TRACE_OP(TC_NULLIMPORTER, ctx);

  PyObject *args = PyTuple_Pack(
      1, PyString_FromString(
             "__test__")); /* Pass any invalid path; it's not saved */
...
static bool emit_get(const struct pickle_ctx *ctx, PyObject *obj,
                     SDL_RWops *rw) {
  char str[32];
  tc_snprintf(str, ARR_SIZE(str), "%c%d\n", GET, memo_idx(ctx, obj));
...
static bool emit_alloc(const struct pickle_ctx *ctx, SDL_RWops *rw) {
  const char ops[] = {TC_EXTEND, TC_ALLOC};
...
static bool pickle_attrs(struct pickle_ctx *ctx, PyObject *obj, SDL_RWops *rw) {
  ...
  const char setattrs[] = {TC_EXTEND, TC_SETATTRS};
...
bool S_Pickle_Init(PyObject *module) {
  ...
  PyObject *args = Py_BuildValue("(s(O){})", "__placeholder__",
...
void S_Pickle_Shutdown(void) {
  const char *curr;
  kh_foreach(s_id_qualname_map, (uint64_t){0}, curr, { TC_FREE(curr); });
  kh_destroy(str, s_id_qualname_map);
}
...
PyObject *S_UnpickleObjgraph(SDL_RWops *stream) {
  ...
  while (!ctx.stop) {
    ...
    unpickle_func_t utc =
        xtend ? s_ext_op_dispatch_table[op] : s_op_dispatch_table[op];
    if (!utc) {
      SET_RUNTIME_EXC("Bad %sopcode %c[%d]", (xtend ? "extended " : ""), op,
                      (int)op);
      goto err;
    }
    CHK_TRUE(utc(&ctx, stream) == 0, err);
  }

  if (vec_size(&ctx.stack) != 1) {
    SET_RUNTIME_EXC("Unexpected stack size [%u] after 'STOP'",
                    (unsigned)vec_size(&ctx.stack));
    goto err;
  }

  if (vec_size(&ctx.mark_stack) != 0) {
    SET_RUNTIME_EXC("Unexpected mark stack size [%u] after 'STOP'",
