...
typedef struct {
  PyObject_HEAD struct camera *cam;
  enum cam_mode mode;
} PyCameraObject;

static PyObject *PyCamera_new(PyTypeObject *type, PyObject *args,
                              PyObject *kwds);
static void PyCamera_dealloc(PyCameraObject *self);
static int PyCamera_init(PyCameraObject *self, PyObject *args, PyObject *kwds);

static PyObject *PyCamera_center_over_location(PyCameraObject *self,
                                               PyObject *args);
static PyObject *PyCamera_pickle(PyCameraObject *self, PyObject *args,
                                 PyObject *kwargs);
...
static int PyCamera_set_pos(PyCameraObject *self, PyObject *value,
                            void *closure);
...
static int PyCamera_set_pitch(PyCameraObject *self, PyObject *value,
                              void *closure);
static PyObject *PyCamera_get_yaw(PyCameraObject *self, void *closure);
static int PyCamera_set_yaw(PyCameraObject *self, PyObject *value,
                            void *closure);
static PyObject *PyCamera_get_speed(PyCameraObject *self, void *closure);
static int PyCamera_set_speed(PyCameraObject *self, PyObject *value,
                              void *closure);
static PyObject *PyCamera_get_sensitivity(PyCameraObject *self, void *closure);
static int PyCamera_set_sensitivity(PyCameraObject *self, PyObject *value,
                                    void *closure);
...
static PyMethodDef PyCamera_methods[] = {
    {"center_over_location", (PyCFunction)PyCamera_center_over_location,
     METH_VARARGS,
     "Position the camera over the specified (X, Z) map location, by only "
     "shifting it in the XZ plane."},

    {"__pickle__", (PyCFunction)PyCamera_pickle, METH_KEYWORDS,
     "Serialize a Permafrost Engine camera object to a string."},

    {"__unpickle__", (PyCFunction)PyCamera_unpickle,
     METH_VARARGS | METH_KEYWORDS | METH_CLASS,
     "Create a new tc.Camera instance from a string earlier returned from a "
     "__pickle__ method."
...
static PyGetSetDef PyCamera_getset[] = {
    {"mode", (getter)PyCamera_get_mode, NULL,
     "The mode determines which controller is installed when the camera is "
     "activated. "
     "Can be one of tc.CAM_MODE_RTS, tc.CAM_MODE_FPS, tc.CAM_MODE_FREE",
     NULL},
    {"position", (getter)PyCamera_get_pos, (setter)PyCamera_set_pos,
     "The current worldspace position of the camera", NULL},
    {"direction", (getter)PyCamera_get_dir, NULL,
     "The current worldspace direction of the camera", NULL},
    {"pitch", (getter)PyCamera_get_pitch, (setter)PyCamera_set_pitch,
     "The camera's pitch (in degrees)", NULL},
    {"yaw", (getter)PyCamera_get_yaw, (setter)PyCamera_set_yaw,
     "The camera's yaw (in degrees)", NULL},
    {"speed", (getter)PyCamera_get_speed, (setter)PyCamera_set_speed,
     "The camera's speed (in OpenGL units / ms)", NULL},
    {"sensitivity", (getter)PyCamera_get_sensitivity,
     (setter)PyCamera_set_sensitivity,
     "The camera's sensitivity (how fast direction can be changed)", NULL},
    {NULL} /* Sentinel */
};

static PyTypeObject PyCamera_type = {
    PyVarObject_HEAD_INIT(NULL, 0) "tc.Camera", /* tp_name */
    ...
    "in its' constructor:                                           \n"
    "                                                               \n"
    "  - mode {tc.CAM_MODE_RTS, tc.CAM_MODE_FPS, tc.CAM_MODE_FREE}  \n"
...
static PyObject *PyCamera_new(PyTypeObject *type, PyObject *args,
  ...
  static char *kwlist[] = {"mode",  "position",    "pitch", "yaw",
                           "speed", "sensitivity", NULL};
  enum cam_mode mode = CAM_MODE_FREE;
  PyObject *obj;
  float a, b, c, d;

  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|iOffff", kwlist, &mode, &obj,
                                   &a, &b, &c, &d)) {
    assert(PyErr_Occurred());
    return NULL;
  }
...
static int PyCamera_init(PyCameraObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|iOffff", kwlist, &mode,
                                   &pos_tuple, &pitch, &yaw, &speed, &sens)) {
    assert(PyErr_Occurred());
    return -1;
  }

  if (pos_tuple &&
      !(PyTuple_Check(pos_tuple) || PyTuple_GET_SIZE(pos_tuple) != 3 ||
    ...
    PyErr_SetString(PyExc_TypeError,
                    "position keyword argument must be a tuple of 3 floats.");
    return -1;
  }
...
static PyObject *PyCamera_center_over_location(PyCameraObject *self,
  ...
  if (!PyArg_ParseTuple(args, "(ff)", &target.x, &target.z)) {
    PyErr_SetString(
        PyExc_TypeError,
        "Argument must be a tuple of two floats (XZ map position).");
    return NULL;
  }
  ...
  if (!hit) {
    ...
    if (dir.y >= 0.0f) {
      PyErr_SetString(
          PyExc_RuntimeError,
          "The camera is facing upwards. Unable ot center over map position.");
      return NULL;
    }
    float t = fabs(pos.y / dir.y);
    map_intersect = (vec3_t){pos.x + t * dir.x, 0.0f, pos.z + t * dir.z};
  }

  vec2_t delta_xz;
  vec2_t map_intersect_xz = (vec2_t){map_intersect.x, map_intersect.z};
  TCM_Vec2_Sub(&target, &map_intersect_xz, &delta_xz);

  vec3_t newpos;
  vec3_t delta = (vec3_t){delta_xz.x, 0.0f, delta_xz.z};
  TCM_Vec3_Add(&pos, &delta, &newpos);
...
static PyObject *PyCamera_unpickle(PyObject *cls, PyObject *args,
  ...
  if (!active || !mode || !position || !pitch || !yaw || !speed ||
      !sensitivity) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Could not unpickle internal state of tc.Camera instance");
    goto fail_unpickle;
  }

  if (!PyInt_Check(active)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Unpickled 'active' field must be an integer type");
    goto fail_unpickle;
  }
  ...
  if (PyInt_AS_LONG(active)) {
    ...
    PyObject *cam_kwargs = Py_BuildValue(
        "{s:O,s:O,s:O,s:O,s:O,s:O}", "mode", mode, "position", position,
        "pitch", pitch, "yaw", yaw, "speed", speed, "sensitivity", sensitivity);
    if (!cam_kwargs) {
      Py_DECREF(cam_args);
      goto fail_unpickle;
    }
...
static bool camera_check_readonly(PyCameraObject *self) {
  if (self->mode != CAM_MODE_FREE) {
    PyErr_SetString(PyExc_RuntimeError, "This attribute is readonly except for "
                                        "cameras in the tc.CAM_MODE_FREE mode");
    return true;
  }
  return false;
}
...
static int PyCamera_set_pitch(PyCameraObject *self, PyObject *value,
  ...
  Camera_SetPitchAndYaw(self->cam, PyFloat_AS_DOUBLE(value),
                        Camera_GetYaw(self->cam));
  return 0;
}
...
static int PyCamera_set_yaw(PyCameraObject *self, PyObject *value,
  ...
  Camera_SetPitchAndYaw(self->cam, Camera_GetPitch(self->cam),
...
void S_Camera_Shutdown(void) { /* No-op for now */
}

void S_Camera_Clear(void) { Py_CLEAR(s_active_cam); }
...
bool S_Camera_SetActive(PyObject *cam) {
  ...
  if (!PyType_IsSubtype(cam->ob_type, &PyCamera_type)) {
    PyErr_SetString(
        PyExc_TypeError,
        "The active camera must be a subclass of the tc.Camera type");
    return false;
  }
