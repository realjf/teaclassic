...
typedef struct {
  PyObject_HEAD PyObject *runfunc;
...
static PyObject *PyTask_call(PyTaskObject *self, PyObject *args,
                             PyObject *kwargs);
...
static PyObject *PyTask_pickle(PyTaskObject *self, PyObject *args,
                               PyObject *kwargs);
...
static PyObject *PyTask_who_is(PyTaskObject *self, PyObject *args,
                               PyObject *kwargs);
...
static PyGetSetDef PyTask_getset[] = {
    {"completed", (getter)PyTask_get_completed, NULL,
     "Returns True if the task has ran to completion", NULL},
    {NULL} /* Sentinel */
};

static PyMethodDef PyTask_methods[] = {
    {"__pickle__", (PyCFunction)PyTask_pickle, METH_KEYWORDS,
     "Serialize a Permafrost Engine task object to a string."},

    {"__unpickle__", (PyCFunction)PyTask_unpickle,
     METH_VARARGS | METH_KEYWORDS | METH_CLASS,
     "Create a new tc.Task instance from a string earlier returned from a "
     "__pickle__ method."
     "Returns a tuple of the new instance and the number of bytes consumed "
     "from the stream."},

    {"run", (PyCFunction)PyTask_run, METH_NOARGS,
     "Start the task, invoking its' __run__ method in a fiber context."},

    {"wait", (PyCFunction)PyTask_wait, METH_VARARGS,
     "Block until the completion of another tc.Task instance."},

    {"yield_", (PyCFunction)PyTask_yield, METH_NOARGS,
     "Yield to the scheduler, allowing other tasks to run. The task may be "
     "suspended until "
     "the next frame. Yielding periodically is a means to make sure that "
     "long-running tasks "
     "don't exceed the frame's time budget."},

    {"send", (PyCFunction)PyTask_send, METH_VARARGS,
     "Send a message to another tc.Task instance, becoming blocked until it "
     "replies."},

    {"receive", (PyCFunction)PyTask_receive, METH_NOARGS,
     "Become blocked, waiting until a message is received from the specified "
     "task."},

    {"reply", (PyCFunction)PyTask_reply, METH_VARARGS,
     "Respond to a sent message from another task, unblocking it."},

    {"await_event", (PyCFunction)PyTask_await_event, METH_VARARGS,
     "Become blocked unitl a particular event takes place."},

    {"sleep", (PyCFunction)PyTask_sleep, METH_VARARGS,
     "Become blocked for a period of time specified in milliseconds."},

    {"register", (PyCFunction)PyTask_register, METH_VARARGS,
     "Register this task for a specific name."},

    {"who_is", (PyCFunction)PyTask_who_is, METH_VARARGS | METH_KEYWORDS,
...
PyTypeObject PyTask_type = {
    PyVarObject_HEAD_INIT(NULL, 0) "tc.Task", /* tp_name */
...
static PyObject *pytask_call_method(void *func, PyTaskObject *self,
...
static int pytask_tracefunc(PyTaskObject *self, PyFrameObject *frame, int what,
                            PyObject *arg) {
...
static struct result py_task(void *arg) {
  ...
  if (self->state == PYTASK_STATE_RUNNING) {
    ...
    if (req_result) {
      ...
      const unsigned char *bytecode =
          (unsigned char *)PyString_AS_STRING(self->ts->frame->f_code->co_code);
      ...
      assert(opcode == CALL_FUNCTION || opcode == CALL_FUNCTION_VAR ||
             opcode == CALL_FUNCTION_KW || opcode == CALL_FUNCTION_VAR_KW);
      ...
      *(self->ts->frame->f_stacktop++) = req_result;
       ...
       * cases when this can occur: when the task was send-blocked or
       * reply-blocked on another task that has subsequently finished running.
       * Effectively this puts the resumed task into a state where it can never
       * be unblocked. Such a "hung" task is definitely an example of sloppy
       * scripting but, nonetheless, shouldn't prevent us from resuming our
       * session.
       */
      PyErr_Clear();
    }
    ...
    ret = PyEval_EvalCodeEx((PyCodeObject *)PyFunction_GET_CODE(self->runfunc),
                            PyFunction_GET_GLOBALS(self->runfunc), NULL,
                            (PyObject **)&self, 1, NULL, 0, NULL, 0,
  ...
  if (PyErr_Occurred()) {
    PyObject *exc_info = Py_BuildValue(
        "OOOO", self, self->ts->curexc_type ? self->ts->curexc_type : Py_None,
...
static void pytask_req_set(PyTaskObject *task, PyObject *args, PyObject *kwargs,
                           int type) {
  Py_XINCREF(args);
  Py_XINCREF(kwargs);
...
static PyObject *PyTask_getattro(PyTaskObject *self, PyObject *name) {
  if (PyString_Check(name) && !strcmp(PyString_AS_STRING(name), "__run__")) {
    PyErr_SetString(
        ...
        "The __run__ method of the task cannot be accessed directly. "
...
static PyObject *PyTask_new(PyTypeObject *type, PyObject *args,
                            PyObject *kwds) {
  ...
  if (!func) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Task class must implement a __run__ method.");
    goto fail_run;
  }

  if (!PyFunction_Check(func)) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The task class's __run__ attribute must be a function.");
    goto fail_run;
  }
...
static PyObject *PyTask_pickle(PyTaskObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|s#", kwlist, (char *)&ctx,
                                   &len)) {
    assert(PyErr_Occurred());
    return NULL;
  }
  if (!ctx) {
    PyErr_SetString(
        PyExc_TypeError,
        "Expecting __ctx__ keyword argument with pointer to pickling context.");
    return NULL;
  }
  ...
  if (self->state == PYTASK_STATE_RUNNING) {
    ...
    self->ts->frame->f_stacktop =
        self->ts->frame->f_valuestack + self->stack_depth;
  }

  PyObject *ts = Py_BuildValue(
      "OiOOOOOOO", self->ts->frame ? (PyObject *)self->ts->frame : Py_None,
  ...
  PyObject *request = Py_BuildValue(
      "OOi", self->req.args ? self->req.args : Py_None,
      self->req.kwargs ? self->req.kwargs : Py_None, self->req.type);
...
static PyObject *PyTask_unpickle(PyObject *cls, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s#|s#", kwlist, &str, &slen,
                                   &ctx, &len)) {
    assert(PyErr_Occurred());
    goto fail_args;
  }
  ...
  if (!ctx) {
    PyErr_SetString(PyExc_TypeError, "Expecting __ctx__ keyword argument with "
                                     "pointer to unpickling context.");
    return NULL;
  }
  ...
  CHK_TRUE(PyInt_AS_LONG(state) >= PYTASK_STATE_NOT_STARTED &&
               PyInt_AS_LONG(state) <= PYTASK_STATE_FINISHED,
           fail_unpickle);
  ...
  if (!PyTuple_Check(ts) ||
      !PyArg_ParseTuple(ts, "OiOOOOOOO", &frame, &recursion_depth, &curexc_type,
                        &curexc_value, &curexc_traceback, &exc_type, &exc_value,
                        &exc_traceback, &dict)) {
    goto fail_unpickle;
  }
  ...
  if (!PyTuple_Check(request) ||
      !PyArg_ParseTuple(request, "OOi", &reqargs, &reqkwargs, &reqtype))
    goto fail_unpickle;
  ...
  ret = (PyTaskObject *)((PyTypeObject *)cls)
            ->tp_alloc((struct _typeobject *)cls, 0);
  ...
  if (regname == Py_None) {
    ...
    ret->regname = tc_strdup(PyString_AS_STRING(regname));
  }
  ...
  ret->ts->frame =
      (frame == Py_None) ? NULL : (Py_INCREF(frame), (PyFrameObject *)frame);
  ret->ts->recursion_depth = recursion_depth;
  ret->ts->dict = (dict == Py_None) ? NULL : (Py_INCREF(dict), dict);

  ret->ts->curexc_type =
      (curexc_type == Py_None) ? NULL : (Py_INCREF(curexc_type), curexc_type);
  ret->ts->curexc_value = (curexc_value == Py_None)
                              ? NULL
                              : (Py_INCREF(curexc_value), curexc_value);
  ret->ts->curexc_traceback =
      (curexc_traceback == Py_None)
          ? NULL
          : (Py_INCREF(curexc_traceback), curexc_traceback);

  ret->ts->exc_type =
      (exc_type == Py_None) ? NULL : (Py_INCREF(exc_type), exc_type);
  ret->ts->exc_value =
      (exc_value == Py_None) ? NULL : (Py_INCREF(exc_value), exc_value);
  ret->ts->exc_traceback = (exc_traceback == Py_None)
                               ? NULL
                               : (Py_INCREF(exc_traceback), exc_traceback);

  ret->req.args = (reqargs == Py_None) ? NULL : (Py_INCREF(reqargs), reqargs);
  ret->req.kwargs =
      (reqkwargs == Py_None) ? NULL : (Py_INCREF(reqkwargs), reqkwargs);
...
fail_unpickle:
  if (!ret && !PyErr_Occurred()) {
    PyErr_SetString(PyExc_RuntimeError, "Could not unpickle tc.Task instance");
  }
...
static PyObject *PyTask_wait(PyTaskObject *self, PyObject *args) {
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'wait' method can only be called from the context of "
                    "the __run__ method.");
    return NULL;
  }
  ...
  if (!PyArg_ParseTuple(args, "O", &task) ||
    ...
    PyErr_SetString(
        PyExc_TypeError,
        "Expecting one argument: a tc.Task instance (task to wait on).");
    return NULL;
  }
...
static PyObject *PyTask_yield(PyTaskObject *self) {
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'yield_' method can only be called from the context "
                    "of the __run__ method.");
    return NULL;
  }
...
static PyObject *PyTask_send(PyTaskObject *self, PyObject *args) {
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'send' method can only be called from the context of "
                    "the __run__ method.");
    return NULL;
  }
  ...
  if (!PyArg_ParseTuple(args, "OO", &recepient, &message) ||
    ...
    PyErr_SetString(PyExc_TypeError,
                    "Expecting two arguments: a tc.Task instance (repecient) "
  ...
  if (((PyTaskObject *)recepient)->state != PYTASK_STATE_RUNNING) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Can only send messages to a running task.");
    return NULL;
  }
...
static PyObject *PyTask_receive(PyTaskObject *self) {
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'receive' method can only be called from the context "
                    "of the __run__ method.");
    return NULL;
  }
...
static PyObject *PyTask_reply(PyTaskObject *self, PyObject *args) {
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'reply' method can only be called from the context of "
                    "the __run__ method.");
    return NULL;
  }
...
static PyObject *PyTask_await_event(PyTaskObject *self, PyObject *args) {
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'await_event' method can only be called from the "
                    "context of the __run__ method.");
    return NULL;
  }
  ...
  if (!PyArg_ParseTuple(args, "i", &event)) {
    PyErr_SetString(PyExc_TypeError,
                    "Expecting one integer argument (event to wait on)");
    return NULL;
  }
...
static PyObject *PyTask_sleep(PyTaskObject *self, PyObject *args) {
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'sleep' method can only be called from the context of "
                    "the __run__ method.");
    return NULL;
  }
  ...
  if (!PyArg_ParseTuple(args, "i", &ms)) {
    PyErr_SetString(PyExc_TypeError,
                    "Expecting one integer argument (number of milliseconds)");
    return NULL;
  }
...
static PyObject *PyTask_register(PyTaskObject *self, PyObject *args) {
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'register' method can only be called from the context "
                    "of the __run__ method.");
    return NULL;
  }
  ...
  if (!PyArg_ParseTuple(args, "s", &name)) {
    PyErr_SetString(
        PyExc_TypeError,
        "Expecting one string argument (the name to register under)");
    return NULL;
  }

  self->regname = tc_strdup(name);
...
static PyObject *PyTask_who_is(PyTaskObject *self, PyObject *args,
  ...
  if (self->state != PYTASK_STATE_RUNNING || Sched_ActiveTID() != self->tid) {
    PyErr_SetString(PyExc_RuntimeError,
                    "The 'who_is' method can only be called from the context "
                    "of the __run__ method.");
    return NULL;
  }
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|i", kwlist, &name,
                                   &blocking)) {
    assert(PyErr_Occurred());
    return NULL;
  }
...
void S_Task_Clear(void) {
  PyTaskObject *curr;
  kh_foreach(s_tid_task_map, (uint32_t){0}, curr, { Py_DECREF(curr); });
