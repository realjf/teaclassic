...
#define __KHASH_IMPL(name, SCOPE, khkey_t, khval_t, kh_is_map, __hash_func, __hash_equal)                       \
    ...
    SCOPE int kh_resize_##name(kh_##name##_t *h, khint_t new_n_buckets) {                                       \
        ...
        {                                                                                                       \
            ...
            if (h->size >= (khint_t)(new_n_buckets * __ac_HASH_UPPER + 0.5))                                    \
                ...
                if (h->n_buckets < new_n_buckets) { /* expand */                                                \
                    ...
                    h->keys = new_keys;                                                                         \
                    if (kh_is_map) {                                                                            \
                        khval_t *new_vals = (khval_t *)krealloc((void *)h->vals,                                \
        ...
        if (j) { /* rehashing is needed */                                                                      \
            ...
            if (h->n_buckets > new_n_buckets) { /* shrink the hash table */                                     \
                ...
                if (kh_is_map) h->vals = (khval_t *)krealloc((void *)h->vals,                                   \
                                                             new_n_buckets * sizeof(khval_t));                  \
            }                                                                                                   \
    ...
    SCOPE khint_t kh_put_##name(kh_##name##_t *h, khkey_t key, int *ret) {                                      \
        ...
        if (h->n_occupied >= h->upper_bound) {   /* update the hash table */                                    \
            if (h->n_buckets > (h->size << 1)) { /* clear "deleted" elements */                                 \
...
/*! @function
  ...
  @param  ...   Block of code to execute
 */
#define kh_foreach_value(h, vvar, ...)                     \
    {                                                      \
        khint_t __i;                                       \
        for (__i = kh_begin(h); __i != kh_end(h); ++__i) { \
            if (!kh_exist(h, __i)) continue;               \
