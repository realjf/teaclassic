    ...
    static void _pq_##name##_balance(pq(name) * pqueue, int root_idx) {                                                            \
        while (root_idx != pqueue->size + 1) {                                                                                     \
            ...
            int right_child_idx = left_child_idx + 1;                                                                              \
                                                                                                                                   \
            if (left_child_idx <= pqueue->size && pqueue->nodes[left_child_idx].priority < pqueue->nodes[target_idx].priority) {   \
                target_idx = left_child_idx;                                                                                       \
            }                                                                                                                      \
                                                                                                                                   \
            if (right_child_idx <= pqueue->size && pqueue->nodes[right_child_idx].priority < pqueue->nodes[target_idx].priority) { \
