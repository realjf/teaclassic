...
static bool sett_parse_line(char *line, int *out_prio,
                            struct named_val *out_val) {
  char *saveptr;
  char *token = tc_strtok_r(line, " \t", &saveptr);
  ...
  if (!strcmp(token, "string")) {
    ...
    if (!sscanf(token, "%f %f\n", &out_val->val.as_vec2.x,
                &out_val->val.as_vec2.y))
      goto fail;
...
ss_e Settings_Create(struct setting sett) {
  ...
  if ((k != kh_end(s_settings_table)) &&
    ...
    const char *key = tc_strdup(sett.name);
...
ss_e Settings_SetNoPersist(const char *name, const struct sval *new_val) {
  ...
  sett_set_priv(sett->name, (struct setting_priv){.flags = SETT_FLAGS_NOPERSIST,
                                                  .prev = prev});
  return ret;
}
...
ss_e Settings_LoadFromFile(void) {
  ...
  for (int i = 0; i < SETT_MAX_PRIO; i++) {
    ...
    while (AL_ReadLine(stream, line)) {
      ...
      khiter_t k;
      if ((k = kh_get(setting, s_settings_table, nv.name)) !=
          kh_end(s_settings_table)) {
