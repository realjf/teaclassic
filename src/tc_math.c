...
void TCM_Mat4x4_RotFromEuler(GLfloat deg_x, GLfloat deg_y, GLfloat deg_z, mat4x4_t *out)
{
    ...
    TCM_Mat4x4_MakeRotX(DEG_TO_RAD(deg_x), &x);
    TCM_Mat4x4_MakeRotY(DEG_TO_RAD(deg_y), &y);
    TCM_Mat4x4_MakeRotZ(DEG_TO_RAD(deg_z), &z);

    TCM_Mat4x4_Mult4x4(&y, &z, &tmp);
    TCM_Mat4x4_Mult4x4(&x, &tmp, out);
}
                           ...
                           vec3_t *up, mat4x4_t *out)
{
    ...
    TCM_Vec3_Sub(camera_pos, target_pos, &camera_dir);
    TCM_Vec3_Normal(&camera_dir, &camera_dir);
    TCM_Vec3_Cross(&camera_dir, up, &right);

    mat4x4_t axes, trans;
    TCM_Mat4x4_Identity(&axes);
    ...
    TCM_Mat4x4_MakeTrans(-camera_pos->x, -camera_pos->y, -camera_pos->z, &trans);
    TCM_Mat4x4_Mult4x4(&axes, &trans, out);
}
...
GLfloat TCM_Quat_PitchDiff(quat_t *op1, quat_t *op2)
{
    ...
    TCM_Mat4x4_RotFromQuat(op1, &mat);
    TCM_Mat4x4_Mult4x1(&mat, &front, &dir_homo);
    ...
    TCM_Mat4x4_RotFromQuat(op2, &mat);
    TCM_Mat4x4_Mult4x1(&mat, &front, &dir_homo);
