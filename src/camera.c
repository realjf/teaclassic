...
static bool camera_pos_in_bounds(const struct camera *cam) {
  ...
  return (cam->pos.x <= cam->bounds.x &&
          cam->pos.x >= cam->bounds.x - cam->bounds.w) &&
         (cam->pos.z >= cam->bounds.z &&
          cam->pos.z <= cam->bounds.z + cam->bounds.h);
}
...
void Camera_Free(struct camera *cam) { free(cam); }
...
void Camera_SetPitchAndYaw(struct camera *cam, float pitch, float yaw) {
  ...
  TCM_Vec3_Normal(&front, &cam->front);
...
void Camera_SetSpeed(struct camera *cam, float speed) { cam->speed = speed; }

void Camera_SetSens(struct camera *cam, float sensitivity) {
  cam->sensitivity = sensitivity;
}

float Camera_GetSpeed(const struct camera *cam) { return cam->speed; }

float Camera_GetSens(const struct camera *cam) { return cam->sensitivity; }

float Camera_GetYaw(const struct camera *cam) { return cam->yaw; }

float Camera_GetPitch(const struct camera *cam) { return cam->pitch; }

float Camera_GetHeight(const struct camera *cam) { return cam->pos.y; }

vec3_t Camera_GetPos(const struct camera *cam) { return cam->pos; }
...
void Camera_MoveLeftTick(struct camera *cam) {
  ...
  TCM_Vec3_Cross(&cam->front, &cam->up, &left);
  TCM_Vec3_Normal(&left, &left);

  TCM_Vec3_Scale(&left, tdelta * cam->speed, &vdelta);
  TCM_Vec3_Add(&cam->pos, &vdelta, &cam->pos);
...
void Camera_MoveRightTick(struct camera *cam) {
  ...
  TCM_Vec3_Scale(&left, tdelta * cam->speed, &vdelta);
  TCM_Vec3_Sub(&cam->pos, &vdelta, &cam->pos);
...
void Camera_MoveFrontTick(struct camera *cam) {
  ...
  TCM_Vec3_Scale(&cam->front, tdelta * cam->speed, &vdelta);
  TCM_Vec3_Add(&cam->pos, &vdelta, &cam->pos);
...
void Camera_MoveDirectionTick(struct camera *cam, vec3_t dir) {
  ...
  TCM_Vec3_Scale(&dir, tdelta * cam->speed, &vdelta);
  TCM_Vec3_Add(&cam->pos, &vdelta, &cam->pos);
...
void Camera_TickFinishPerspective(struct camera *cam) {
  ...
  vec3_t target;
  TCM_Vec3_Add(&cam->pos, &cam->front, &target);
  TCM_Mat4x4_MakeLookAt(&cam->pos, &target, &cam->up, &view);
  ...
  TCM_Mat4x4_MakePerspective(DEG_TO_RAD(45.0f), ((GLfloat)w) / h,
...
void Camera_TickFinishOrthographic(struct camera *cam, vec2_t bot_left,
                                   vec2_t top_right) {
  mat4x4_t view, proj;
  ...
  /* Set the projection matrix for the vertex shader */
  TCM_Mat4x4_MakeOrthographic(bot_left.raw[0], top_right.raw[0],
                              bot_left.raw[1], top_right.raw[1],
                              CAM_Z_NEAR_DIST, CONFIG_DRAWDIST, &proj);
  R_PushCmd((struct rcmd){
      .func = R_GL_SetProj,
      .nargs = 1,
...
bool Camera_PosIsRestricted(const struct camera *cam) { return cam->bounded; }

void Camera_MakeViewMat(const struct camera *cam, mat4x4_t *out) {
  vec3_t target;
  TCM_Vec3_Add((vec3_t *)&cam->pos, (vec3_t *)&cam->front, &target);
  TCM_Mat4x4_MakeLookAt((vec3_t *)&cam->pos, &target, (vec3_t *)&cam->up, out);
}

void Camera_MakeProjMat(const struct camera *cam, mat4x4_t *out) {
  int w, h;
  Engine_WinDrawableSize(&w, &h);
  TCM_Mat4x4_MakePerspective(CAM_FOV_RAD, ((GLfloat)w) / h, 0.1f,
                             CONFIG_DRAWDIST, out);
}
