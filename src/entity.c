...
void Entity_ModelMatrix(const struct entity *ent, mat4x4_t *out) {
  ...
  TCM_Mat4x4_MakeTrans(pos.x, pos.y, pos.z, &mtrans);
  TCM_Mat4x4_MakeScale(scale.x, scale.y, scale.z, &mscale);
  TCM_Mat4x4_RotFromQuat(&rot, &mrot);

  TCM_Mat4x4_Mult4x4(&mscale, &mrot, &mtmp);
  TCM_Mat4x4_Mult4x4(&mtrans, &mtmp, out);
}

uint32_t Entity_NewUID(void) { return s_next_uid++; }

void Entity_SetNextUID(uint32_t uid) { s_next_uid = uid; }
...
void Entity_CurrentOBB(const struct entity *ent, struct obb *out,
  ...
  vec4_t identity_center_homo = (vec4_t){
      (aabb->x_min + aabb->x_max) / 2.0f, (aabb->y_min + aabb->y_max) / 2.0f,
      (aabb->z_min + aabb->z_max) / 2.0f, 1.0f};
  ...
  for (int i = 0; i < 8; i++) {
    TCM_Mat4x4_Mult4x1(&model, identity_verts_homo + i, obb_verts_homo + i);
  ...
  TCM_Mat4x4_Mult4x1(&model, &identity_center_homo, &obb_center_homo);
  out->center = (vec3_t){
  ...
  TCM_Vec3_Sub(&out->corners[4], &out->corners[0], &axis0);
  TCM_Vec3_Sub(&out->corners[2], &out->corners[0], &axis1);
  TCM_Vec3_Sub(&out->corners[1], &out->corners[0], &axis2);

  TCM_Vec3_Normal(&axis0, &out->axes[0]);
  TCM_Vec3_Normal(&axis1, &out->axes[1]);
  TCM_Vec3_Normal(&axis2, &out->axes[2]);
}
...
vec3_t Entity_TopCenterPointWS(const struct entity *ent) {
  ...
  vec4_t top_center_homo =
      (vec4_t){(aabb->x_min + aabb->x_max) / 2.0f, aabb->y_max,
               (aabb->z_min + aabb->z_max) / 2.0f, 1.0f};
  ...
  Entity_ModelMatrix(ent, &model);
  TCM_Mat4x4_Mult4x1(&model, &top_center_homo, &out_ws_homo);
...
void Entity_FaceTowards(struct entity *ent, vec2_t point) {
  vec2_t delta;
  vec2_t pos = G_Pos_GetXZ(ent->uid);
  TCM_Vec2_Sub(&point, &pos, &delta);

  float radians = atan2(delta.x, delta.z);

  mat4x4_t rotmat;
  TCM_Mat4x4_MakeRotY(radians, &rotmat);
...
void Entity_Ping(const struct entity *ent) {
  uint32_t tid =
      Sched_Create(1, ping_task, (void *)ent, NULL, TASK_MAIN_THREAD_PINNED);
  Sched_RunSync(tid);
}
...
bool Entity_MaybeAdjacentFast(const struct entity *a, const struct entity *b,
                              float buffer) {
  ...
  vec2_t diff;
  TCM_Vec2_Sub(&apos, &bpos, &diff);
  return (TCM_Vec2_Len(&diff) < len + buffer);
}
...
size_t Entity_EntsForTag(const char *tag, size_t maxout,
                         uint32_t out[static maxout]) {
  khiter_t k = kh_get(ents, s_tag_ent_map, tag);
  if (k == kh_end(s_tag_ent_map))
    return 0;
...
size_t Entity_TagsForEnt(uint32_t uid, size_t maxout,
...
void Entity_DisappearAnimated(struct entity *ent, const struct map *map,
