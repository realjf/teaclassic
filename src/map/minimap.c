...
{
    vec2_t point_to_src;
    TCM_Vec2_Sub(&src, &point, &point_to_src);
    ...
    vec2_t ret;
    TCM_Vec2_Add(&rotated, &point, &ret);
    return ret;
}
...
static vec2_t m_minimap_mouse_coords_to_world(const struct map *map, vec2_t virt_screen_coords)
{
    ...
    vec2_t ap, ab, ad;
    TCM_Vec2_Sub(&virt_screen_coords, &curr_bounds.a, &ap);
    TCM_Vec2_Sub(&curr_bounds.b, &curr_bounds.a, &ab);
    TCM_Vec2_Sub(&curr_bounds.d, &curr_bounds.a, &ad);

    float x_frac = TCM_Vec2_Dot(&ap, &ab) / TCM_Vec2_Dot(&ab, &ab);
    float z_frac = TCM_Vec2_Dot(&ap, &ad) / TCM_Vec2_Dot(&ad, &ad);
...
bool M_InitMinimap(struct map *map, vec2_t center_pos)
{
    ...
    for(int c = 0; c < map->width;  c++) {
        ...
        const struct tcchunk *curr = &map->chunks[r * map->width + c];
...
void M_RenderMinimap(const struct map *map, const struct camera *cam)
{
    ...
    vec2_t center;
    TCM_Vec2_Add(&curr_bounds.a, &curr_bounds.b, &center);
    TCM_Vec2_Add(&center, &curr_bounds.c, &center);
    TCM_Vec2_Add(&center, &curr_bounds.d, &center);
    TCM_Vec2_Scale(&center, 0.25f, &center);

    vec2_t ab;
    TCM_Vec2_Sub(&curr_bounds.b, &curr_bounds.a, &ab);
    int len = TCM_Vec2_Len(&ab);
