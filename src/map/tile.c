...
float M_Tile_HeightAtPos(const struct tile *tile, float frac_width, float frac_height)
{
    if(tile->type == TILETYPE_FLAT) {
        ...
        if(C_PointInsideTriangle2D(
            ...
            TCM_Vec3_Sub(&first_tri[1], &first_tri[0], &edge1);
            TCM_Vec3_Sub(&first_tri[2], &first_tri[0], &edge2);
            tri_point = first_tri[0];

        }else{

            TCM_Vec3_Sub(&second_tri[1], &second_tri[0], &edge1);
            TCM_Vec3_Sub(&second_tri[2], &second_tri[0], &edge2);
            tri_point = second_tri[0];
        }

        TCM_Vec3_Cross(&edge2, &edge1, &tri_normal);
        TCM_Vec3_Normal(&tri_normal, &tri_normal);
        ...
        TCM_Vec3_Add(&ray_origin, &ray_dir, &intersec_point);
                                     ...
                                     struct line_seg_2d line, struct tile_desc out[], size_t maxout)
{
    ...
    TCM_Vec2_Normal(&line_dir, &line_dir);
