...
static void sched_init_ctx(struct task *task, void *code) {
  const size_t stack_size =
      (task->flags & TASK_BIG_STACK) ? BIG_STACK_SZ : STACK_SZ;
  ...
  *(uint64_t *)stack_base = (uintptr_t)code;
   ...
   */
  task->ctx.fpucw =
      (((uint16_t)(0x0 & 0x1)) << 12) | (((uint16_t)(0x0 & 0x3)) << 10) |
      (((uint16_t)(0x2 & 0x3)) << 8) | (((uint16_t)(0x0 & 0x1)) << 7) |
   ...
   */
  task->ctx.mxcsr =
      (((uint32_t)(0x0 & 0x1)) << 15) | (((uint32_t)(0x0 & 0x3)) << 13) |
      (((uint32_t)(0x3f & 0x3f)) << 7) | (((uint32_t)(0x0 & 0x1)) << 6);
}
...
static void sched_task_init(struct task *task, int prio, uint32_t flags,
                            void *code, void *arg, struct future *future,
                            uint32_t parent) {
  task->prio = prio;
...
static void sched_send(struct task *task, uint32_t tid, void *msg,
                       size_t msglen) {
...
static void sched_receive(struct task *task, uint32_t *out_tid, void *msg,
                          size_t msglen) {
...
static void sched_reply(struct task *task, uint32_t tid, void *reply,
                        size_t replylen) {
...
static uint64_t sched_retain_arg(struct task *task, int event, int event_source,
                                 void *arg) {
  void *ret = NULL;
  if (event_source == ES_SCRIPT) {
    ...
    if (event < SDL_LASTEVENT) {
      ...
      /* We'd better be sure that the pointer won't have been 'free'd or
       * something by the time the waiting task finally runs... */
      ret = arg;
      task->erelease = NULL;
    }
  }
...
static uint32_t sched_create(int prio, task_func_t code, void *arg,
...
static bool sched_wait(struct task *task, uint32_t child_tid) {
  ...
  if (child->parent_tid != task->tid || (child->flags & TASK_DETACHED))
    return false;
...
static void sched_task_run(struct task *task) {
  ...
  char name[64];
  tc_snprintf(name, sizeof(name), "Task %03u", task->tid);
  PERF_PUSH(name);
  ...
  if (SDL_ThreadID() == g_main_thread_id) {
    ...
    int id = sched_curr_thread_worker_id();
    sched_switch_ctx(&s_worker_contexts[id], &task->ctx, task->retval,
                     task->arg);
  }
...
static void sched_task_service_request(struct task *task) {
  ...
  case SCHED_REQ_CREATE:
    task->retval = sched_create(
        (int)task->req.argv[0], (task_func_t)task->req.argv[1],
        (void *)task->req.argv[2], (struct future *)task->req.argv[3],
        (int)task->req.argv[4], task->tid);
    sched_reactivate(task);
    break;
  ...
  case SCHED_REQ_YIELD:
    ...
    break;
  case SCHED_REQ_SEND:
    sched_send(task, (uint32_t)task->req.argv[0], (void *)task->req.argv[1],
               (size_t)task->req.argv[2]);
    break;
  case SCHED_REQ_RECEIVE:
    sched_receive(task, (uint32_t *)task->req.argv[0],
                  (void *)task->req.argv[1], (size_t)task->req.argv[2]);
    break;
  case SCHED_REQ_REPLY:
    sched_reply(task, (uint32_t)task->req.argv[0], (void *)task->req.argv[1],
                (size_t)task->req.argv[2]);
    break;
...
bool Sched_Init(void) {
  ...
  for (int i = 0; i < s_nworkers; i++) {
    ...
    char threadname[128];
    tc_snprintf(threadname, sizeof(threadname), "worker-%d", i);
    s_worker_threads[i] =
        SDL_CreateThread(worker_threadfn, threadname, (void *)((uintptr_t)i));
...
void Sched_Shutdown(void) {
  ...
  kh_foreach(s_event_queues, key, curr, { queue_tid_destroy(&curr); });
  kh_destroy(tqueue, s_event_queues);
...
void Sched_Tick(void) {
  ...
  /* Use a do-while to ensure we're always making at least _some_ forward
   * progress */
  do {
    int nwaiters = 0;
    ...
    SDL_LockMutex(s_ready_lock);
    while (!pq_size(&s_ready_queue_main) && !pq_size(&s_ready_queue) &&
    ...
    /* When the ready queue is empty and all the workers are in a state of
     * waiting, there is no more work to be done. In that case, let's not waste
     * any more time.
     */
...
uint32_t Sched_Create(int prio, task_func_t code, void *arg,
  ...
  SDL_LockMutex(s_request_lock);
  uint32_t ret =
      sched_create(prio, code, arg, result, flags | TASK_DETACHED, NULL_TID);
  SDL_UnlockMutex(s_request_lock);
...
bool Sched_RunSync(uint32_t tid) {
  ...
  if (sched_curr_thread_tid() == NULL_TID) {
    ...
    Sched_Request((struct request){.type = _SCHED_REQ_RUN_SYNC, .argv = {tid}});
...
void Sched_ClearState(void) {
  ...
  for (khiter_t k = kh_begin(s_event_queues); k != kh_end(s_event_queues);
       k++) {
...
uint32_t Sched_ActiveTID(void) { return sched_curr_thread_tid(); }
...
uint32_t Sched_CreateBlocking(int prio, task_func_t code, void *arg,
  ...
  do {
    ...
    if (ret == NULL_TID) {
      ...
      SDL_LockMutex(s_ready_lock);
      status = pq_task_pop(&s_ready_queue_main, &task) ||
               pq_task_pop(&s_ready_queue, &task);
      SDL_UnlockMutex(s_ready_lock);
      ...
      if (status) {
        ...
        /* All the tasks are blocked. Nothing more we can do until the next
         * event pump */
        break;
      }
    }
...
bool Sched_HasBlocked(void) {
  ...
  SDL_LockMutex(s_ready_lock);
  bool ret =
      (pq_size(&s_ready_queue) > 0) || (pq_size(&s_ready_queue_main) > 0);
  SDL_UnlockMutex(s_ready_lock);
  return ret;
}
